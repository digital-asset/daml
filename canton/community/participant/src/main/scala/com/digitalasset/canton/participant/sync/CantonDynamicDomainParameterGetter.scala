    ...
    def getToleranceForDomain(
      ...
      for {
        topoClient <- EitherT.fromOption[Future](
          ...
          s"Domain $domainId has no snapshot",
        )
        snapshot = topoClient.currentSnapshotApproximation(traceContext)
        paramsWithValidity <- EitherT(snapshot.findDynamicDomainParameters()(traceContext))
      } yield paramsWithValidity.parameters.ledgerTimeRecordTimeTolerance
