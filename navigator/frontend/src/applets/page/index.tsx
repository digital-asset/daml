...
export const toTemplateContracts = (action: TemplateContracts.Action): Action =>
  ({ type: 'TO_TEMPLATECONTRACTS', action });
...
export const reduce = (page?: State, action?: Action): State => {
  ...
  } else if (action.type === 'TO_TEMPLATECONTRACTS'
...
const Component = ({ configSource, page, user, config, toConfig, toSelf, toWatcher }: Props) => {
  switch (page.type) {
    ...
    case 'configsource':
      return (
        <ConfigSource.UI
          toSelf={(action: ConfigSource.Action) =>
            toConfig(action) }
          state={configSource}
          config={config}
          user={user}
