...
export type Contract = ContractsByTemplateQuery_node_Template_contracts_edges_node;
...
query ContractsByTemplateQuery($templateId: ID!, $filter: [FilterCriterion!], $search: String!,
  $count: Int!, $sort: [SortCriterion!], $includeArchived: Boolean!) {
  node(typename: "Template", id: $templateId) {
    ... on Template {
      id
      choices { name }
...
export function makeQueryVariables(config: TableConfig): ContractsByTemplateQueryVariables {
  return {
...
export function makeParamQueryVariables<
  P extends {state: State}
>({state}: P): {variables: ContractsByTemplateParamQueryVariables} {
  return {
    variables: {
      templateId: state.id,
    },
