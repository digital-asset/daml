...
import { DamlLfDefDataType, DamlLfIdentifier, DamlLfRecord, DamlLfTypeCon } from '../api/DamlLfType';
...
import { ContractIdProvider, default as ParameterForm, ParameterFormContract, TypeProvider } from './index';
...
const contractIdProvider: ContractIdProvider = {
  fetchContracts(filter: string, onResult: (result: ParameterFormContract[]) => void): void {
    onResult(contracts.filter((c) => c.id.indexOf(filter) >= 0
    || c.template.id.indexOf(filter) >= 0));
  },
}

const exampleRecordId: DamlLfIdentifier = { name: 'exampleRecord', module: 'module', package: 'package'};
