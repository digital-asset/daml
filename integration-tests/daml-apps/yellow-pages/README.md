...
```bash
daml build
daml codegen js .daml/dist/yellow-pages-0.1.0.dar -o ui/daml.js
cd ui
npm install
npm run-script build
zip -r ../yellow-pages-ui.zip build
```

Next you need to create a ledger on [Daml Hub], upload the files
`.daml/dist/yellow-pages-0.1.0.dar` (created by the `daml build` command)
and `yellow-pages-ui.zip` (created by the `zip` command based on the result
