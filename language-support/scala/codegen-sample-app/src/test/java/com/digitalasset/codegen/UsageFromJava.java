...
final class UsageFromJava {
    ...
    static <T> T matchingVariants(Option<T, T> opt) {
        if (opt instanceof Option.Call)
            return ((Option.Call<T, T>) opt).body();
        else if (opt instanceof Option.Put)
            return ((Option.Put<T, T>) opt).body();
    ...
    static DomainCommand invokeTransfer1(Contract<CallablePayout> cp, String controller, String toParty) {
    ...
    */
    ...
    private static <T> T parseTemplate(TemplateCompanion<? extends T> tc, Value v) {
        return tc.the$u0020template$u0020Value().read(v.sum())
                .getOrElse(() -> {throw new RuntimeException("not a " + tc.id());});
    }

    // and likewise DEL-5482
    private static Contract<?> parseAnyContractFromModule(EventDecoderApi ed, CreatedEvent v) {
        return ed.createdEventToContractRef(v).toOption().getOrElse(
                () -> {
