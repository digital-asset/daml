...
public class LedgerViewFlowable {
  ...
  abstract static class LedgerViewFlowableInput {}
  ...
  static class CompletionFailure extends LedgerViewFlowableInput {
    ...
    @Override
    public String toString() {
      return "CompletionFailure{" + "commandId='" + commandId + '\'' + ", status=" + status + '}';
    }
  }
  ...
  static class WorkflowEventWrapper extends LedgerViewFlowableInput {
    ...
    @Override
    public String toString() {
      return "WorkflowEventWrapper{" + "workflowEvent=" + workflowEvent + '}';
    }
  ...
  private static class CommandIdAndPendingSet extends LedgerViewFlowableInput {
    final String commandId;
    ...
    public CommandIdAndPendingSet(
        String commandId, PMap<Identifier, PSet<String>> templateToContracts) {
      this.commandId = commandId;
    ...
    public static CommandIdAndPendingSet from(CommandsAndPendingSet commandsAndPendingSet) {
      return new CommandIdAndPendingSet(
          commandsAndPendingSet.getSubmitCommandsRequest().getCommandId(),
          commandsAndPendingSet.getContractIdsPendingIfSucceed());
    }
  }

  public static <R> Flowable<LedgerView<R>> of(
      LedgerView<R> initialLedgerView,
    ...
    Flowable<LedgerViewFlowableInput> workflowEventsWrapper =
        workflowEvents.map(WorkflowEventWrapper::new);
    Flowable<LedgerViewFlowableInput> commandIdAndPendingsSet =
        commandsAndPendingsSet.map(CommandIdAndPendingSet::from);
    Flowable<LedgerViewFlowableInput> all =
        Flowable.merge(
            submissionFailures, completionFailures, workflowEventsWrapper, commandIdAndPendingsSet);

    boolean shouldEmitInitial = !initialLedgerView.activeContractSet.isEmpty();
    StateWithShouldEmit<R> initialState =
        new StateWithShouldEmit<>(initialLedgerView, 0, shouldEmitInitial);

    return all.scan(
            initialState, (input, state) -> LedgerViewFlowable.accumulate(input, state, transform))
        .filter(s -> s.shouldEmit)
        .map(s -> s.ledgerView);
  }

  public static <R> Flowable<LedgerView<R>> of(
      Flowable<SubmissionFailure> submissionFailuresCommandIds,
  ...
  static <R> StateWithShouldEmit<R> accumulate(
      StateWithShouldEmit<R> stateWithShouldEmit,
      LedgerViewFlowableInput ledgerViewFlowableInput,
      Function<CreatedContract, R> transform) {
    ...
    if (ledgerViewFlowableInput instanceof SubmissionFailure) {
      ...
      LedgerView<R> newLedgerView =
          stateWithShouldEmit.ledgerView.unsetPendingForContractsOfCommandId(
              submissionFailure.commandId);
      ...
      if (workflowEvent instanceof Transaction) {
            ...
            R r =
                transform.apply(
                    new CreatedContract(
                        ...
                        transactionContext));
            newLedgerView =
                newLedgerView.addActiveContract(
                    createdEvent.getTemplateId(), createdEvent.getContractId(), r);
          } else if (event instanceof ArchivedEvent) {
            ArchivedEvent archivedEvent = (ArchivedEvent) event;
            newLedgerView =
                newLedgerView.archiveContract(
                    archivedEvent.getTemplateId(), archivedEvent.getContractId());
          }
        }
        ...
        LedgerView<R> newLedgerView = stateWithShouldEmit.ledgerView;
        GetActiveContractsResponseContext context =
            new GetActiveContractsResponseContext(activeContracts.getWorkflowId());
        for (CreatedEvent createdEvent : activeContracts.getCreatedEvents()) {
          R r =
              transform.apply(
                  new CreatedContract(
                      createdEvent.getTemplateId(), createdEvent.getArguments(), context));
          newLedgerView =
              newLedgerView.addActiveContract(
                  createdEvent.getTemplateId(), createdEvent.getContractId(), r);
        }
      ...
      CommandIdAndPendingSet commandIdAndPendingSet =
          (CommandIdAndPendingSet) ledgerViewFlowableInput;
      LedgerView<R> newLedgerView = stateWithShouldEmit.ledgerView;
      for (Identifier templateId : commandIdAndPendingSet.templateToContracts.keySet()) {
        ...
        for (String contractId : contractIds) {
          newLedgerView =
              newLedgerView.setContractPending(
                  commandIdAndPendingSet.commandId, templateId, contractId);
        }
      ...
      if (isBoundary) {
        // if we found a boundary, we emit a value only in case there have been commands in between
        // this and the
        // previous boundary:
        ...
        return new StateWithShouldEmit<>(
            newLedgerView, stateWithShouldEmit.commandsCounter + 1, false);
      }
    }
    //        return stateWithShouldEmit;
  ...
  static <R> Single<Pair<LedgerView<R>, LedgerOffset>> ledgerViewAndOffsetFromACS(
      Flowable<GetActiveContractsResponse> responses, Function<CreatedContract, R> transform) {
    Single<Pair<StateWithShouldEmit<R>, Optional<LedgerOffset>>> lastViewAndOffset =
        responses.reduce(
            ...
            (viewAndOffset, response) -> {
              ...
              StateWithShouldEmit<R> newLedgerView =
                  accumulate(ledgerView, new WorkflowEventWrapper(response), transform);
              @NonNull
              Optional<LedgerOffset> offset =
                  response.getOffset().map(off -> new LedgerOffset.Absolute(off));
              return new Pair<>(newLedgerView, offset);
            });
    return lastViewAndOffset.map(
        m ->
            new Pair<>(
                m.getFirst().ledgerView,
                m.getSecond().orElse(LedgerOffset.LedgerBegin.getInstance())));
  }

  static <R> Single<LedgerView<R>> ledgerViewFromFlowable(
      LedgerView<R> initial,
      Flowable<WorkflowEvent> events,
      Function<CreatedContract, R> transform) {
    return events
        .reduce(
            new StateWithShouldEmit<>(initial, 0, false),
            (ledgerView, event) ->
                accumulate(ledgerView, new WorkflowEventWrapper(event), transform))
        .map(m -> m.ledgerView);
  }

  /**
   * The local view of the Ledger. This view is created and updated every time a new event is
  ...
  public static class LedgerView<R> {
    ...
    LedgerView(
        PMap<String, PMap<Identifier, PSet<String>>> commandIdToContractIds,
    ...
    LedgerView<R> addActiveContract(Identifier templateId, String contractId, R r) {
      ...
      PMap<String, R> oldContractIdsToR =
          this.activeContractSet.getOrDefault(templateId, HashTreePMap.empty());
      PMap<String, R> newContractIdsToR = oldContractIdsToR.plus(contractId, r);
      PMap<Identifier, PMap<String, R>> newActiveContractSet =
          this.activeContractSet.plus(templateId, newContractIdsToR);
      return new LedgerView<R>(
          this.commandIdToContractIds,
          this.contractIdsToCommandIds,
          this.pendingContractSet,
          newActiveContractSet);
    }
    ...
    LedgerView<R> setContractPending(String commandId, Identifier templateId, String contractId) {
      ...
      PMap<String, R> oldActiveContracts =
          this.activeContractSet.getOrDefault(templateId, HashTreePMap.empty());
      ...
      PMap<String, R> oldPendingContracts =
          this.pendingContractSet.getOrDefault(templateId, HashTreePMap.empty());
      PMap<String, R> newPendingContracts = oldPendingContracts.plus(contractId, r);
      PMap<Identifier, PMap<String, R>> newPendingContractSet =
          this.pendingContractSet.plus(templateId, newPendingContracts);

      // add to commandId -> contractIds
      PMap<Identifier, PSet<String>> oldTemplateIdToContracts =
          this.commandIdToContractIds.getOrDefault(commandId, HashTreePMap.empty());
      PSet<String> oldContracts =
          oldTemplateIdToContracts.getOrDefault(templateId, HashTreePSet.empty());
      PSet<String> newContracts = oldContracts.plus(contractId);
      PMap<Identifier, PSet<String>> newTemplateIdToContracts =
          oldTemplateIdToContracts.plus(templateId, newContracts);
      ...
      PMap<String, PSet<String>> oldContractToCommandIds =
          this.contractIdsToCommandIds.getOrDefault(templateId, HashTreePMap.empty());
      PSet<String> oldCommandIds =
          oldContractToCommandIds.getOrDefault(contractId, HashTreePSet.empty());
      PSet<String> newCommandIds = oldCommandIds.plus(commandId);
      PMap<String, PSet<String>> newContractToCommandIds =
          oldContractToCommandIds.plus(contractId, newCommandIds);
      ...
      LedgerView<R> result =
          new LedgerView<R>(
              newCommandIdToContracts,
              newContractIdsToCommandIds,
              newPendingContractSet,
              newActiveContractSet);
      logger.debug(
          "ledgerView.setContractPending({}, {}, {})).result: ",
          commandId,
          templateId,
          contractId,
          result);
    ...
    LedgerView<R> unsetPendingForContractsOfCommandId(String commandId) {
      ...
      final PMap<String, PMap<Identifier, PSet<String>>> newCommandIdToContractIds =
          this.commandIdToContractIds.minus(commandId);

      // we go over each contract, see if this is the only command locking it and if the answer is
      // yes we move
      // the contract from pending to active
      final PMap<Identifier, PSet<String>> contractsToUnset =
          this.commandIdToContractIds.getOrDefault(commandId, HashTreePMap.empty());
      ...
      for (Identifier templateId : contractsToUnset.keySet()) {
        ...
        PMap<String, PSet<String>> contractIdToCommandIds =
            newContractIdsToCommandIds.getOrDefault(templateId, HashTreePMap.empty());
        PMap<String, R> pendingContractSet =
            newPendingContractSet.getOrDefault(templateId, HashTreePMap.empty());
        PMap<String, R> activeContractSet =
            newActiveContractSet.getOrDefault(templateId, HashTreePMap.empty());

        for (String contractId : oldContractIds) {
          final PSet<String> oldCommandIds =
              contractIdToCommandIds.getOrDefault(contractId, HashTreePSet.empty());
    ...
    private static <R> PMap<Identifier, PMap<String, R>> removeContract(
        Identifier templateId, String contractId, PMap<Identifier, PMap<String, R>> from) {
    ...
    LedgerView<R> archiveContract(Identifier templateId, String contractId) {
      ...
      PMap<Identifier, PMap<String, R>> newPendingContractSet =
          removeContract(templateId, contractId, this.pendingContractSet);
      PMap<Identifier, PMap<String, R>> newActiveContractSet =
          removeContract(templateId, contractId, this.activeContractSet);

      PMap<String, PMap<Identifier, PSet<String>>> newCommandIdToContractIds =
          this.commandIdToContractIds;
      if (this.contractIdsToCommandIds.containsKey(templateId)) {
        PMap<String, PSet<String>> contractIdToCommandIds =
            this.contractIdsToCommandIds.get(templateId);
        if (contractIdToCommandIds.containsKey(contractId)) {
          PSet<String> commandIds = contractIdToCommandIds.get(contractId);
          for (String commandId : commandIds) {
            PMap<Identifier, PSet<String>> oldContracts =
                newCommandIdToContractIds.getOrDefault(commandId, HashTreePMap.empty());
            PSet<String> oldContractIds =
                oldContracts.getOrDefault(templateId, HashTreePSet.empty());
     ...
     * @return A map contractId to contract containing all the contracts with template id equal to
    ...
    public int hashCode() {
      ...
      return Objects.hash(
          commandIdToContractIds, contractIdsToCommandIds, pendingContractSet, activeContractSet);
    }
  }
