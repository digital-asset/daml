...
public final class TimeClientImpl implements TimeClient {
    ...
    private Single<Empty> setTime(Instant currentTime, Instant newTime, Optional<String> accessToken) {
        ...
        return Single.fromFuture(StubHelper.authenticating(this.serviceFutureStub, accessToken).setTime(request));
    }
    ...
    private Flowable<Instant> getTime(Optional<String> accessToken) {
        ...
        return ClientPublisherFlowable
                .create(request, StubHelper.authenticating(this.serviceStub, accessToken)::getTime, sequencerFactory)
