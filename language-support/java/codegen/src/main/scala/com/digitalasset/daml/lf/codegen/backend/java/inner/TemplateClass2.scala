...
package com.daml
package lf.codegen.backend.java.inner

import ledger.javaapi
import lf.codegen.TypeWithContext
import lf.codegen.backend.java.ObjectMethods
import lf.data.ImmArray.ImmArraySeq
import lf.data.Ref.{ChoiceName, PackageId, QualifiedName}
import lf.iface._
...
private[inner] object TemplateClass2 extends StrictLogging {
  ...
  def generate(
    ...
    TrackLineage.of("interface", interfaceName.simpleName()) {
      ...
      val templateType = TypeSpec
        ...
        .addType(generateContractClass(interfaceName, template.key, packagePrefixes))
        .addFields(RecordFields(fields).asJava)
        .addMethods(RecordMethods(fields, interfaceName, IndexedSeq.empty, packagePrefixes).asJava)
        .build()
      logger.debug("End")
      templateType
    }
