...
import tests.numerictest.*;
import tests.numerictest.nestednumericbox.*;
...
public class NumericTest {
    ...
    void deserializableFromRecord() {
        ...
        checkRecord(record);
        assertTrue("to value uses original daml lf names for fields", record.toValue().getFieldsMap().get("numeric37").asNumeric().isPresent());
    }
    ...
    void outerRecordRoundtrip() {
        ...
        NumericBox fromRoundTrip = NumericBox.fromValue(Value.fromProto(fromConstructor.toValue().toProto()));
