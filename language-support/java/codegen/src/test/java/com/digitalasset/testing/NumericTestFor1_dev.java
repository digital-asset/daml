// Copyright (c) 2019 The DAML Authors. All rights reserved.
// SPDX-License-Identifier: Apache-2.0

package com.digitalasset.testing;


import com.daml.ledger.javaapi.data.Decimal;
import com.daml.ledger.javaapi.data.Party;
import com.daml.ledger.javaapi.data.Record;
import org.junit.jupiter.api.Test;
import org.junit.platform.runner.JUnitPlatform;
import org.junit.runner.RunWith;
import test.numeric.Box;

import java.math.BigDecimal;

import static org.junit.jupiter.api.Assertions.*;

@RunWith(JUnitPlatform.class)
public class NumericTestFor1_dev {

    @Test
    void numeric2Value2Numeric() {

        Box b = new Box(
                new BigDecimal(0),
                new BigDecimal(10),
                new BigDecimal(17),
                new BigDecimal("0.38"),
                "alice"
        );
        assertEquals(Box.fromValue(b.toValue()), b);
    }

    @Test
    void value2Decimal2value() {
        Record record = new Record(
                new Record.Field("x0", new Decimal(new BigDecimal(0))),
                new Record.Field("x10", new Decimal(new BigDecimal(10))),
                new Record.Field("x17", new Decimal(new BigDecimal(17))),
                new Record.Field("x38", new Decimal(new BigDecimal("0.38"))),
                new Record.Field("party", new Party("alice"))
            );
        assertEquals(Box.fromValue(record).toValue(), record);
    }

}
