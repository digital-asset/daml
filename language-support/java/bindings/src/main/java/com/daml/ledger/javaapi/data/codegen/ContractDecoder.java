...
public class ContractDecoder {
    private final Map<Identifier, ? extends ContractCompanion<? extends Contract<?, ?>, ?, ?>> companions;

    public ContractDecoder(Iterable<? extends ContractCompanion<? extends Contract<?, ?>, ?, ?>> companions) {
        this.companions = StreamSupport.stream(companions.spliterator(), false)
    ...
    public Contract<?, ?> fromCreatedEvent(CreatedEvent event) throws IllegalArgumentException {
        ...
        ContractCompanion<? extends Contract<?, ?>, ?, ?> companion = getContractCompanion(templateId).orElseThrow(() -> new IllegalArgumentException("No template found for identifier " + templateId));
