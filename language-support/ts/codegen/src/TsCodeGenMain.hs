...
configConsts sdkVersion = ConfigConsts
  { pkgDependencies = HMS.fromList
      ...
      , (NpmPackageName "@daml/types"
          -- If the SDK version is 0.0.0 (CI testing), we assume
          -- daml-types is provided as a local package by the parent
          -- projet.
        , NpmPackageVersion (if sdkVersion == "0.0.0" then "file:../daml-types" else sdkVersion))
      ]
...
buildPackages sdkVersion optScope optOutputDir dependencies = do
  ...
  where
    ...
    config = configConsts $ versionToText sdkVersion
