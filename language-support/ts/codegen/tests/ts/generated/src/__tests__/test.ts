...
test('create + fetch & exercise', async () => {
  ...
  // the new contract.
  const [result6, events6] = await ledger.exercise(Main.Person.Birthday, alice5Contract.contractId, {});
  expect(result6).not.toEqual(alice5Contract.contractId);
  expect(events6).toHaveLength(2);
  expect(events6[0]).toHaveProperty('archived');
  expect(events6[1]).toHaveProperty('created');
  const alice5Archived = (events6[0] as {archived: ArchiveEvent<Main.Person>}).archived;
  const alice6Contract = (events6[1] as {created: CreateEvent<Main.Person, PersonKey>}).created;
  expect(alice5Archived.contractId).toEqual(alice5Contract.contractId);
  expect(alice6Contract.contractId).toEqual(result6);
