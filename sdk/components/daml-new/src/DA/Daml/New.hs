...
--
ociMain :: IO ()
ociMain = withProgName "dpm" $ withComponentVersions $ do
  ...
  where
    commandParser :: ComponentVersioned => Parser (IO ())
    commandParser = 
      let templateHelpStr = "Name of the template used to create the project (default: " <> defaultProjectTemplate <> ")"
          ...
          runNewDpm allowExisting oTargetFolder templateNameM = do
            mAssemblyVersion <- getAssemblyVersionMaybe
            ...
            let -- Temporary solution to find version from resolution file, pull from the path. This will not work with local path overrides
                ...
                extractVersionFromComponentPath componentName =
                  either (\err -> error $ "Failed to lookup version of " <> componentName <> ". " <> err) id $ do
                    ...
                    componentData <- maybeToEither ("Couldn't find resolution data for " <> componentName <> ". Something is wrong with your DPM installation.")
                      $ Map.lookup componentName $ components validDefaultResolution
                    let versionString = componentVersion componentData
