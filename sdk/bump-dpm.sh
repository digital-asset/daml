...
if [ -z "$DPM_VERSION" ]; then
  ...
  bazel run @dpm_binary//:dpm -- repo resolve-tags --registry europe-docker.pkg.dev/da-images/public dpm:latest > /dev/null
  DPM_VERSION=$(bazel run @dpm_binary//:dpm -- repo resolve-tags --registry europe-docker.pkg.dev/da-images/public dpm:latest 2> /dev/null)
fi
...
for platform in "${platforms[@]}"; do
  sha=$(bazel run @dpm_binary//:oras -- manifest fetch europe-docker.pkg.dev/da-images/public/components/dpm:$DPM_VERSION.$platform  2> /dev/null | jq -r '.layers[0].digest | split(":")[1]')
  shas+=("\"$platform\": \"$sha\"")
done
