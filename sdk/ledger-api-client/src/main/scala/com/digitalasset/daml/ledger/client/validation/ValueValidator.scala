...
package com.digitalasset.daml.ledger.client.validation
...
/** Self-contained SDK-side replacement for the canton ledger API `ValueValidator`.
  *
  * Translates Ledger API values back to [[com.digitalasset.daml.lf.value.Value]] values. Unlike the
  * canton original, it carries no dependency on the canton error framework or logging: validation
  * failures are reported as plain gRPC `INVALID_ARGUMENT` errors.
  */
object ValueValidator {

  private val validNumericPattern =
    """[+-]?\d{1,38}(\.\d{0,37})?""".r.pattern

  private def validateNumeric(s: String): Option[Numeric] =
    if (validNumericPattern.matcher(s).matches())
      Numeric.fromUnscaledBigDecimal(new java.math.BigDecimal(s)).toOption
    else
      None

  private def invalidArgument(message: String): StatusRuntimeException =
    Status.INVALID_ARGUMENT.withDescription(s"Invalid argument: $message").asRuntimeException()

  private def missingField(fieldName: String): StatusRuntimeException =
    Status.INVALID_ARGUMENT.withDescription(s"Missing field: $fieldName").asRuntimeException()

  private def invalidField(fieldName: String, message: String): StatusRuntimeException =
    Status.INVALID_ARGUMENT
      .withDescription(s"Invalid field $fieldName: $message")
      .asRuntimeException()

  private def validateRecordFields(
      recordFields: Seq[api.RecordField]
  ): Either[StatusRuntimeException, ImmArray[(Option[Ref.Name], Lf)]] =
    recordFields
      .foldLeft[Either[StatusRuntimeException, BackStack[(Option[Ref.Name], Lf)]]](
  ...
  def validateValue(v0: api.Value): Either[StatusRuntimeException, Lf] = v0.sum match {
    ...
    case Sum.List(api.List(elems)) =>
      elems
        .foldLeft[Either[StatusRuntimeException, BackStack[Lf]]](Right(BackStack.empty))(
          (valuesE, v) =>
            for {
              values <- valuesE
              validatedValue <- validateValue(v)
    ...
    case Sum.Optional(o) =>
      o.value.fold[Either[StatusRuntimeException, Lf]](Right(Lf.ValueNone))(
        validateValue(_).map(v => Lf.ValueOptional(Some(v)))
      )
    case Sum.TextMap(textMap0) =>
      val map = textMap0.entries
        .foldLeft[Either[StatusRuntimeException, FrontStack[(String, Lf)]]](
    ...
    case Sum.GenMap(genMap0) =>
      val genMap = genMap0.entries
        .foldLeft[Either[StatusRuntimeException, BackStack[(Lf, Lf)]]](
          Right(BackStack.empty)
  ...
  private def requireIdentifier(s: String): Either[StatusRuntimeException, Ref.Name] =
  ...
  private def validateTemplateQualifiedName(
      moduleName: String,
      entityName: String,
