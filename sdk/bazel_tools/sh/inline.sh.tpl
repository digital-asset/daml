#!/usr/bin/env bash
# By default, running a script using `bazel run <script>` its working directory
# is below `$HOME/.cache/bazel/...`.
#
# But the scripts in dev-env/bin require that they are called from within the
# same worktree the dev-env scripts were sourced from, and fail:
#
#   Fatal: Using dev-env from $DADE_BASE_ROOT, but running under $PWD.
#
# Change to the workspace directory to avoid this problem.
...
canonicalize_rlocation() {
  # Note (MK): This is a fun one: Let’s say %workspace% is "compatibility"
