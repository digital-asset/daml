...
class ReinterpretTest(majorLanguageVersion: LanguageMajorVersion)
  ...
  "exercise by interface pull interfaceId package " in  {
    ...
    val testCases = Table(
      ...
      ReplayCommand.Exercise(templateId, None, toContractId("cid"),  "myChoice", ValueUnit)  -> List(templatePkgId),
      ReplayCommand.Exercise(templateId, Some(interfaceId), toContractId("cid"),  "myChoice", ValueUnit)  -> List(templatePkgId,interfacePkgId),
      ReplayCommand.ExerciseByKey(templateId, ValueUnit, "MyChoide", ValueUnit) -> List(templatePkgId),
    ...
    forEvery(testCases) { (cmd, pkgIds) =>
      val emptyPackage = Package(Map.empty, Set.empty, LanguageMajorVersion.V1.maxStableVersion, None)
      var queriedPackageIds  = Set.empty[Ref.PackageId]
      val trackPackageQueries: PartialFunction[Ref.PackageId, Package] = {
        pkgId =>
