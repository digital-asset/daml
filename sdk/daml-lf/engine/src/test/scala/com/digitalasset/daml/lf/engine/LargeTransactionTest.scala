...
class LargeTransactionTest(majorLanguageVersion: LanguageMajorVersion)
  ...
  /** Tiny wrapper around ScriptLedger that provides
    * a mutable API for ease of use in tests.
    */
  class MutableLedger {

    import ScriptLedger.{initialLedger => _, _}

    private var ledger: ScriptLedger = initialLedger()

    private def initialLedger(): ScriptLedger = ScriptLedger.initialLedger(Time.Timestamp.now())
