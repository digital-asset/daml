...
import com.digitalasset.daml.lf.language.LazyTypeModule
...
private[lf] final class ValueTranslator(
  ...
  private[this] val LazyType = language.LazyTypeModule(pkgInterface)
  private[this] type LazyType = LazyType.LazyType
  ...
  private[preprocessing] def unsafeTranslateValue(
    ...
    def go(ty0: LazyType, value0: Value, nesting: Int): SValue =
      if (nesting > Value.MAXIMUM_NESTING) {
        throw Error.Preprocessing.ValueNesting(value)
      } else {
        import language.TypeF._
        val newNesting = nesting + 1
        def typeError(msg: String = s"mismatching type: ${ty0.typ.pretty} and value: $value0") =
          throw Error.Preprocessing.TypeMismatch(ty0.typ, value0, msg)
        def destruct(typ: LazyType) =
          typ.destruct match {
            case Right(value) => value
            case Left(LazyTypeModule.Error.TypeError(err)) =>
              typeError(err)
            case Left(LazyTypeModule.Error.LookupError(err)) =>
        ...
        (destruct(ty0), value0) match {
          ...
          case (
            ...
            def addMissingField(lbl: Ref.Name, ty: LazyType): (Option[Ref.Name], Value) =
              destruct(ty) match {
