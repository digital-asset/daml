...
private[daml] class EncodeCommon(languageVersion: LV) {

  import EncodeCommon._
  ...
  private[this] class PackageEncoder(selfPkgId: PackageId) {
    ...
    private[this] val stringsTable =
      new EncodeCommon.TableBuilder[String, String] {
        override def toProto(x: String): String = x
      }
    private[this] val dottedNameTable =
      new EncodeCommon.TableBuilder[DottedName, PLF.InternedDottedName] {
    ...
    private[this] val typeTable = new EncodeCommon.TableBuilder[Type, PLF.Type] {
    ...
    private val builtinFunctionInfos =
      DecodeCommon.builtinFunctionInfos
        .filter(info =>
