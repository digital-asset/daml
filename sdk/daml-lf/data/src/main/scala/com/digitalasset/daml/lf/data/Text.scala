...
sealed abstract class TextModule {
  ...
  private[data] def unsafeFromString(s: String): Text

  final def concat(a: Text, b: Text): Text = unsafeFromString(a + b)

  final def fromBool(b: Boolean): Text = unsafeFromString(b.toString)

  final def fromLong(i: Long): Text = unsafeFromString(i.toString)

  final def Empty: Text = unsafeFromString("")
}
...
private[data] object TextModuleImpl extends TextModule {
      ...
      if (i >= length) Right(new Text(s))
        ...
        // Check for valid surrogate pairs
          ...
          if (i + 1 >= length || !Character.isLowSurrogate(s.charAt(i + 1))) {
            Left("String contains unpaired high surrogate")
          } else {
            loop(i + 2) // Skip the low surrogate
          }
        } else if (Character.isLowSurrogate(ch)) {
          Left("String contains unpaired low surrogate")
        } else {
          loop(i + 1)
        }
      }
  ...
  override private[data] def unsafeFromString(s: String): Text = s
}
