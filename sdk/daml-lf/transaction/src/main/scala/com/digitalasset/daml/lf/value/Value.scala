...
object Value {
  ...
  // Unserializable, rejected by normalizer outside the below runValueFunction and runPureValueFunction
  final class ValueBlackbox(content: Any)
      extends ValueCidlessLeaf
      with CidContainer[ValueBlackbox] {
    // Usually generated by case class
    def canEqual(that: Any): Boolean = that.isInstanceOf[ValueBlackbox]
    def productArity: Int = 1
    def productElement(n: Int): Any =
      if (n == 0) content else throw new IndexOutOfBoundsException(s"Index out of range: $n")

    override def nonVerboseWithoutTrailingNones: Value = this
    private[lf] def getContent: Any = content
  }
