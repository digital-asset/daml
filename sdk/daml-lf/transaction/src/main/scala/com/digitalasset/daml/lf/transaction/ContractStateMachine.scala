...
object ContractStateMachine {
  ...
  /** @param locallyCreated
    ...
    */
  final case class NonUniqueContractKeyImpl[Nid] private[lf] (
    ...
    override def mode: ContractKeyUniquenessMode = ContractKeyUniquenessMode.Off
    ...
    override private[lf] def resolveKey(
      ...
      ContractId
    ] => (KeyMapping, NonUniqueContractKeyImpl[Nid]), (KeyMapping, NonUniqueContractKeyImpl[Nid])] = {
      lookupActiveKey(gkey) match {
        ...
        case None =>
          ...
          ): (KeyMapping, NonUniqueContractKeyImpl[Nid]) = {
    ...
    private def witnessContractId(contractId: ContractId): NonUniqueContractKeyImpl[Nid] =
    ...
    private def assertKeyMapping(
        ...
        mbKey: Option[GlobalKey],
    ): Either[InconsistentContractKey, NonUniqueContractKeyImpl[Nid]] =
      mbKey match {
  ...
  /** @param locallyCreated
    ...
    * and similarly for all [[ActiveLedgerState]]s in [[rollbackStack]].
    */
  final case class UniqueContractKeyImpl[Nid] private[lf] (
      override val locallyCreated: Set[ContractId],
      override val inputContractIds: Set[ContractId],
      override val globalKeyInputs: Map[GlobalKey, KeyInput],
      override val activeState: ContractStateMachine.ActiveLedgerState[Nid],
    ...
    override def mode: ContractKeyUniquenessMode = ContractKeyUniquenessMode.Strict
    ...
    override private[lf] def resolveKey(
      ...
      ContractId
    ] => (KeyMapping, UniqueContractKeyImpl[Nid]), (KeyMapping, UniqueContractKeyImpl[Nid])] = {
      lookupActiveKey(gkey) match {
        case Some(keyMapping) => Right(keyMapping -> this)
        case None =>
          // if we cannot find it here, send help, and make sure to update keys after
          // that.
          def handleResult(result: Option[ContractId]): (KeyMapping, UniqueContractKeyImpl[Nid]) = {
            // Update key inputs. Create nodes never call this method,
            // so NegativeKeyLookup is the right choice for the global key input.
            val keyInput = result match {
              case None => NegativeKeyLookup
    ...
    private def assertKeyMapping(
        cid: Value.ContractId,
        mbKey: Option[GlobalKey],
    ): Either[InconsistentContractKey, UniqueContractKeyImpl[Nid]] =
      mbKey match {
        case None => Right(this)
        case Some(gk) =>
          val (keyMapping, next) = resolveKey(gk) match {
  ...
  object State {
    private[ContractStateMachine] def empty[Nid](mode: ContractKeyUniquenessMode): State[Nid] =
      mode match {
        case ContractKeyUniquenessMode.Off =>
          NonUniqueContractKeyImpl(
        ...
        case ContractKeyUniquenessMode.Strict =>
          UniqueContractKeyImpl(
            locallyCreated = Set.empty,
            inputContractIds = Set.empty,
            globalKeyInputs = Map.empty,
            activeState = ContractStateMachine.ActiveLedgerState.empty,
