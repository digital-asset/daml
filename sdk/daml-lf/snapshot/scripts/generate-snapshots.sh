...
for ((i=0;i<$SIZE;++i)); do
  ...
  for ((j=0;j<$SCRIPTS_SIZE;++j)); do
    ...
    SNAPSHOT_DIR="$SNAPSHOT_DIR" \
      bazel run --sandbox_debug --test_env="HOME=$HOME" //daml-lf/snapshot:generate-snapshots || true
    DAR_FILE="$REPO/$DAR_DIR/$DAR_NAME" \
    SCRIPT_NAME="$SCRIPT" \
    SNAPSHOT_DIR="$SNAPSHOT_DIR" \
      bazel run --sandbox_debug --test_env="HOME=$HOME" //daml-lf/snapshot:extract-snapshot-choices || true
  done
done
