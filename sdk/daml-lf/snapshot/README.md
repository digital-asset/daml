...
A sample ledger entries file can be generated by running:
     ...
     bazel run --sandbox_debug //daml-lf/snapshot:replay-benchmark-test

This will save snapshotting information (i.e. the ledger entries file) to a
bazel based temporary file named `snapshot-participant.bin`. The location of
the Dar file `ReplayBenchmark-v2.dar` and the snapshot file are printed to
the console output. Move these files into a non-bazel controlled directory - say `$SAVED`.

A choice may now be benchmarked by JMH using:

    bazel run //daml-lf/snapshot:replay-benchmark --   \
      -p entriesFile=$SAVED/snapshot-participant.bin   \
      -p choiceName=ReplayBenchmark:T:Add              \
      -p darFile=$SAVED/ReplayBenchmark-v2.dar
