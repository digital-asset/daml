...
class PartialTransactionSpec extends AnyWordSpec with Matchers with Inside {
  ...
  private[this] val pkgNameVersion =
    Some(
      (
        data.Ref.PackageName.assertFromString("-package-name-"),
        data.Ref.PackageVersion.assertFromString("1.0.0"),
      )
    )
  private[this] def pkgName = pkgNameVersion.map(_._1)
  ...
  private[this] implicit class PartialTransactionExtra(val ptx: PartialTransaction) {
    ...
    def rollbackTry_ : PartialTransaction = ptx.rollbackTry
  }
