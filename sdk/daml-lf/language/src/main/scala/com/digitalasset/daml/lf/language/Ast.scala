...
object Ast {
  ...
  final case class GenPackage[E](
      ...
      // imports is an Either type, because in the lf, the imports field is set
      // or unset. If its unset it is not nothing, but a string is given _why_
      // it is unset.
      //
      // if imports is set in lf, the field below will be Right(Set[PackageId])
      // and contains the imports from lf. If its unset in lf, we have
      // Left(String, Set[PackageId]) where the String is the reason included in
      // lf why the imports are unset, and the Set is GENERATED by the
      // PackageDependencyTracker
      imports: Either[(String, Set[PackageId]), Set[PackageId]],
    ...
    def importsFromEither(withStablePackages: Boolean) = {
