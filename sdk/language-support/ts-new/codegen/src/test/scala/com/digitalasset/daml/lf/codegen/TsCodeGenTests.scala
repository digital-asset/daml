...
final class TsCodeGenTests extends AsyncWordSpec with Assertions {
    ...
    "generate the index tree" in {
      withTestDir { implicit testDir =>
        ...
        for (outputDir <- runGen(testDar)) yield {
          ...
          def reexportIndex(name: String): String =
            s"""|import * as $name from './$name';
                |export { $name };""".stripMargin

          val reexportModule = "export * from './module';"

          assertFilesContain(libDir)(
            "index.d.ts" -> Seq(reexportIndex("A")),
            "A/index.d.ts" -> Seq(reexportIndex("B"), reexportModule),
            "A/B/index.d.ts" -> Seq(reexportIndex("C"), reexportIndex("D")),
            "A/B/C/index.d.ts" -> Seq(reexportModule),
            "A/B/D/index.d.ts" -> Seq(reexportModule),
          )
        }
      }
    }
  ...
  private def runGen(dars: Path*)(implicit dir: TestDir): Future[Path] = {
    ...
    TsCodeGen.run(dars, outputDir, scope = "daml2js", damlVersion = "0.0.0").map(_ => outputDir)
  }
