...
/** @param darFiles The [[Set]] of Daml-LF [[Path]]s to convert into code. It MUST contain
  ...
  */
final case class TsCodeGenConf(
    darFiles: Seq[Path],
...
object TsCodeGenConf {
  def parse(args: Array[String], parserName: String = "codegen"): Option[TsCodeGenConf] =
    parser(parserName).parse(
      args,
      TsCodeGenConf(darFiles = Seq.empty, outputDirectory = Paths.get(".")),
    )

  def parser(parserName: String): OptionParser[TsCodeGenConf] =
    new scopt.OptionParser[TsCodeGenConf](parserName) {
