...
import com.digitalasset.daml.lf.codegen.ts._
...
private final class TsCodeGen(
  ...
  def writePackage(packageId: PackageId, packageSig: Ast.PackageSignature): Unit = {
    ...
    Files.write(srcDir.resolve("tsconfig.json"), tsConfig.getBytes)
    val dependencies = moduleGens.flatMap(_.externalImports).distinctBy(_._1).map(_._2)
    val _ = Files.write(
      srcDir.resolve("package.json"),
  ...
  private def renderPackageJson(
    ...
    s"""|{
        ...
        |  "description": "Generated by `daml codegen js` from SDK $damlVersion",
        |  "dependencies": {
        |    ${deps.mkString(",\n    ")}
        |  },
        |  "peer-dependencies": {
  ...
  private def genModule(
    ...
    ModuleGen(
      ...
      internalImports.map(_.moduleName).toSeq.sorted,
...
object TsCodeGen extends StrictLogging {

  def run(conf: TsCodeGenConf, damlVersion: String): Unit = {
    LoggerFactory
  ...
  def run(darFiles: Seq[Path], outputDir: Path, scope: String, damlVersion: String)(implicit
    ...
    for {
      ...
      codegen = new TsCodeGen(outputDir, scope, damlVersion, allPackages)
