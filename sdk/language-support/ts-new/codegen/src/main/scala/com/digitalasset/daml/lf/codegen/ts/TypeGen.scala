...
package com.digitalasset.daml.lf.codegen.ts

import com.digitalasset.daml.lf.data.Ref.{ModuleId, TypeConId}
...
private object TypeGen {
  ...
  def renderSerializable(currentModule: ModuleId, tpe: Ast.Type): String = {
    def rec(tpe: Ast.Type): String =
      tpe match {
        ...
        // TApp
        case Util.TNumeric(n: Ast.TNat) => s"damlTypes.Numeric($n)"
  ...
  def renderType(currentModule: ModuleId, typeCon: TypeConId): String =
    if (currentModule.pkg != typeCon.pkg)
      s"${typeCon.pkg}.${typeCon.qualifiedName.qualifiedName}"
