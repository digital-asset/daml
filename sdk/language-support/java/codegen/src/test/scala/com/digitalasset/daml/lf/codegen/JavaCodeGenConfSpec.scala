...
class JavaCodeGenConfSpec extends AnyFlatSpec with Matchers with OptionValues {

  behavior of "JavaCodeGenConf.parse"

  it should "return error when no arguments are passed" in {
    JavaCodeGenConf.parse(Array.empty) shouldBe empty
  }

  it should "return error when only inputs are passed" in {
    JavaCodeGenConf.parse(Array("foo")) shouldBe empty
  }

  it should "return error when only output is passed" in {
    JavaCodeGenConf.parse(Array("-o", "bar")) shouldBe empty
  }

  it should "return error when only inputs and decoder class are passed" in {
    JavaCodeGenConf.parse(Array("-d", "package.ClassName", "input")) shouldBe empty
  }

  it should "return a JavaCodeGenConf when input, output and a known backend are passed" in {
  ...
  it should "return a JavaCodeGenConf with expected single unmapped input and output" in {
    val conf = JavaCodeGenConf.parse(Array("-o", "output", "input")).value
  ...
  it should "return error when illegal Decoder class is passed" in {
    JavaCodeGenConf.parse(Array("-o", "output", "-d", "$illegal")) shouldBe empty
  }

  it should "return a JavaCodeGenConf with expected single mapped input, output and backend" in {
    val conf = JavaCodeGenConf.parse(Array("-o", "output", "input=input.prefix")).value
    conf.darFiles should contain theSameElementsAs Map(Paths.get("input") -> Some("input.prefix"))
  }

  it should "return a JavaCodeGenConf with expected multiple mapped inputs, output and backend" in {
    val conf = JavaCodeGenConf
  ...
  it should "return a JavaCodeGenConf with expected multiple mixed inputs, output and backend" in {
    val conf =
      JavaCodeGenConf.parse(Array("-o", "output", "input1=input1.prefix", "input2")).value
    conf.darFiles should contain theSameElementsAs Map(
      Paths.get("input1") -> Some("input1.prefix"),
