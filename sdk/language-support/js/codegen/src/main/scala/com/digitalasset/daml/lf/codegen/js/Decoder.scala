...
private[codegen] sealed trait Decoder {
  def render(currentModule: ModuleId): String
}

private[codegen] final case class OneOfDecoder(decoders: Seq[Decoder]) extends Decoder {
  override def render(currentModule: ModuleId): String = {
    val branches = decoders.map(_.render(currentModule)).mkString(", ")
    s"jtv.oneof($branches)"
  }
}
...
private[codegen] final case class ObjectDecoder(fields: Seq[(String, Decoder)]) extends Decoder {
  override def render(currentModule: ModuleId): String = {
    val decodedFields = fields
      .map { case (name, field) => s"$name: ${field.render(currentModule)}" }
      .mkString(", ")
    s"jtv.onbject({$decodedFields})"
  }
}
...
private[codegen] final case class LazyDecoder(underlying: Decoder) extends Decoder {
  override def render(currentModule: ModuleId): String =
    s"damlTypes.lazyMemo(function () { return ${underlying.render(currentModule)}; })"
}
