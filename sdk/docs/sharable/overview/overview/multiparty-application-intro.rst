...
Important requirements for real-world multi-party applications
--------------------------------------------------------------

Businesses operate in a complex environment with strong expectations around data privacy, security, regulatory
compliance and control. At the same time, business run millions of transactions in parallel across the globe.

Running multi-party applications on a distributed ledger therefore requires a highly scalable system
which provides the ability to tightly integrate individual workflows and processes, while still ensuring data sovereignty and privacy.

Therefore, such a system needs to be built around the following key requirements:
 - **Data sovereignty**: Each party must retain control over its own data, ensuring that no other party can access or modify it without explicit permission.
 ...
 - **Composability**: The system must allow for the composition of multiple applications and workflows, enabling complex business processes to be built on top of simpler ones.
 - **Self-Sovereignty**: Each party must retain control over its data and be able to operate independently, without delegating to a central authority or an anonymous group of operators.
