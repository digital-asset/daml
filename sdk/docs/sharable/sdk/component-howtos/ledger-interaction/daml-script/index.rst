...
.. note::
   This page needs to be restructured. The content needs to follow the structure like every component,
   omitting sections that don't apply.
   Download <- can be omitted
   Install <- same here
   Configure <- how to run with config file and external input
   Secure <- how to connect against a secured ledger
   Operate <- quite a few recipes will go there
   Observe <- how to do tracing? how to do logging and debug writing.
   Optimize <- all performance and contention related things
   Upgrade <- how does upgrading and script work
   Recover <- how to write resilient scripts?
   Decomission <- remove
   Troubleshoot <- debugging etc.
