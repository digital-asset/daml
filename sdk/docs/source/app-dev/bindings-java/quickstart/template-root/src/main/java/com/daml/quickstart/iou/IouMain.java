...
public class IouMain {
  ...
  private static <U> U submit(LedgerClient client, String party, Update<U> update) {
    var params = UpdateSubmission.create(APP_ID, randomUUID().toString(), update).withActAs(party);

    return client.getCommandClient().submitAndWaitForResult(params).blockingGet();
  }
}
