...
requirements. At the same time, business run millions of transactions in parallel across the globe. Therefore, a
distributed ledger that support real-world multi-party applications must be able to address a number of key requirements
simultaneously:
 ...
 - **Interconnectivity**: Each party must retain control over its connectivity and be able to operate independently,
    without delegating to a central authority or an anonymous group of operators.
