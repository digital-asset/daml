...
def main(argv):
    r = runfiles.Create()
    install_tree = r.Rlocation(argv[1])
    configure = r.Rlocation(argv[2])
    ...
    lock = {
        "_comment": "Generated by `bazel run //bazel/haskell/ghc:ghc_packages.update`. Do not hand-edit.",
        "ghc_version": "9.0.2",
        "packages": packages,
    }
    ...
    sys.stderr.write("wrote {} ({} packages: {})\n".format(
        out, len(packages), ", ".join(p["name"] for p in packages)))
