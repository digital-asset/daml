...
-- | A package where all references to `PSelf` have been rewritten
-- to `PImport`.
...
data LookupError
  = LEPackage !PackageId
  | LEModule !PackageImportOrSelf !ModuleName
...
instance Pretty LookupError where
  pPrint = \case
    ...
    LEModule PSelf modName -> "unknown module:" <-> pretty modName
    LEModule (PImport pkgId) modName -> "unknown module:" <-> pretty pkgId <> ":" <> pretty modName
