...
decodePackage version selfPackageRef (LF2.Package mods internedStringsV internedDottedNamesV mMetadata internedTypesV internedKindsV)
  ...
  | Just metadata <- mMetadata = do
      ...
      internedKinds <- V.constructNE (V.length internedTypesV) $ \prefix i ->
          runDecode env2{internedKinds = prefix} $ decodeKind (internedKindsV V.! i)
      let env3 = env2{internedKinds}
      runDecode env3 $ do
