...
def get_template_name(pkg): .tycon_interned_dname // 0 | resolve_interned_dname(pkg);

def get_module_name(pkg): .module.module_name_interned_dname // 0 | resolve_interned_dname(pkg);

def get_dotted_name(pkg): .name_interned_dname // 0 | resolve_interned_dname(pkg);

def get_field(pkg): .field_interned_str // 0 | resolve_interned_string(pkg);

def get_name(pkg): .name_interned_str // 0 | resolve_interned_string(pkg);
...
def norm_qualified_module(pkg; f):
    ...
    # we ignore the package id here since the only packages we could have in
    # damlc-integration tests are daml-prim, daml-stdlib, daml-script and <self>.
    { "module":
        .[1] |
        .struct.fields |
        map (
