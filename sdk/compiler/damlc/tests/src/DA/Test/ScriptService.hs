              ...
              testCase "queryContractId/Key" $ do
                rs <-
                  runScriptsInModule
                    ...
                    , "  optOnlyP1 <- queryByKey @T p1 (p1, p1)"
                    , "  optOnlyP1 === Some (onlyP1, T p1 p1)"
                    , "  optOnlyP1 <- queryContractId p2 onlyP1"
                    , "  optOnlyP1 === None"
                    , "  optBoth <- queryByKey @T p1 (p1, p2)"
                    , "  optBoth === Some (both, T p1 p2)"
                    , "  optOnlyP1 <- queryByKey @T p2 (p1, p1)"
                    , "  optOnlyP1 === None"
                    , "  optBoth <- queryByKey @T p2 (p1, p2)"
                    , "  optBoth === Some (both, T p1 p2)"
              ...
              testCase "multi-party query" $ do
                rs <-
                  runScriptsInModule
                    ...
                    , "  r === T p1 23"
                    , "  Some (r, _) <- queryByKey @T [p0, p1] p0"
                    , "  r === cid0"
                    , "  Some (r, _) <- queryByKey @T [p0, p1] p1"
                    , "  r === cid1"
                    , "  pure ()"
                    ]
                expectScriptSuccess rs "test" $ \r ->
