...
runDamlStudio replaceExt remainingArguments = do
    ...
    (exitCode, _out, err) <- runVsCodeCommand (path : remainingArguments) [("DAML_USING_DPM", "1")]
    when (exitCode /= ExitSuccess) $ do
        hPutStrLn stderr . unlines $
            [ err
            , "Failed to launch DPM studio. Make sure Visual Studio Code is installed."
