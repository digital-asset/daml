...
headerLines =
  ...
  , "# //compiler/damlc/stable-packages:stable-package-list-reader-hs and"
  , "# stable-package-list-reader-scala. Whilst the writer depends on the reader,"
  , "# there is NO bazel dependency of the reader on the writer. This it is used to"
  , "# break the dependency cycle between the core of the compiler that generates the"
  , "# stable packages and the logic within this core that needs to know what the"
  , "# stable package ids are (notably, DA.Daml.LF.Proto3.EncodeV2 needs to know if a"
  , "# package ref is stable to allow a direct ref, or ref via pkg import table, see"
