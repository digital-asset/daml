...
metadataEncodingTests = testGroup "MetadataEncoding"
    ...
    , roundtripTestsBy "type synonyms"
        ...
        , ( "type Num = Number"
          , let numberClass = LF.Qualified LF.PSelf (LF.ModuleName ["GHC", "Num"]) (LF.TypeSynName ["Number"])
            in
            ( LF.TypeSynName ["Num"]
            , True
            , LF.KStar `LF.KArrow` LF.KStar
        ...
        , ( "type Display a = Show a"
          , let
              ...
              showClass = LF.Qualified LF.PSelf (LF.ModuleName ["GHC", "Show"]) (LF.TypeSynName ["Show"])
            in
        ...
        , ( "class C a b; type D = C"
          , let
              cClass = LF.Qualified LF.PSelf (LF.ModuleName ["Main"]) (LF.TypeSynName ["C"])
            in
            ( LF.TypeSynName ["D"]
            , True
...
mkImport mPackage moduleComponents =
  LF.Qualified
    { qualPackage = maybe LF.PSelf (LF.PImport . LF.PackageId) mPackage
