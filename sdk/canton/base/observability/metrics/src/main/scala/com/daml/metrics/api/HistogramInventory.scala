...
/** A helper class to register histogram items
  ...
  * histogram items are registered before the actual metrics are created. This is necessary as open
  * telemetry needs to know about the histogram views before the actual metrics are created.
  *
...
object HistogramInventory {
  ...
  final case class Item(
    ...
    /** Builds the info aligning with the histogram definition
      *
      * Due to open telemetry insisting that histograms are defined before the metric is known, we
      * need to dance through a few hoops to keep things consistent
      */
    def info: MetricInfo =
      MetricInfo(
        name = name,
