...
case class MetricsContext(labels: Map[String, String]) {
  ...
  /** Merges the current metric context with the given context. The produced labels represent a
    * union of the labels defined by the two contexts, with the label value found in the given
    * context overriding any values with the same key in the current context.
    */
