...
/** Trait for deterministically serializing an object to a [[com.google.protobuf.ByteString]].
  *
  * A class should extend this trait to indicate that the `serialize` method yields the same
  * `ByteString` if invoked several times. Typical use cases of this behavior include:
  ...
  *   - Classes that are mere wrappers of ByteString (for convenience of the caller) e.g.
  *     AuthenticationToken.
  *
  * '''If a class merely represents content that is transmitted over a network, the class does not
  * need to extend this trait.'''
  *
  * It is strongly recommended to extend this trait by mixing in
  * [[ProtocolVersionedMemoizedEvidence]] or [[MemoizedEvidenceWithFailure]], instead of directly
  * extending this trait.
  *
  * Classes `C` implementing [[HasCryptographicEvidence]] must define a `fromByteString: ByteString
  * \=> C` method in their companion object that converts a serialization back into an equal object.
  * In particular, `c.fromByteString(byteString).toByteString` must equal `byteString`.
  */
trait HasCryptographicEvidence {

  /** Returns the serialization of the object into a [[com.google.protobuf.ByteString]]. In
    * particular, every instance `i` of this trait must equal `fromByteString(i.toByteString)`.
    *
...
/** Effectively immutable [[HasCryptographicEvidence]] classes can mix in this trait to implement
...
trait ProtocolVersionedMemoizedEvidence
  ...
  /** Returns the [[com.google.protobuf.ByteString]] from which this object has been deserialized,
    * if any. If defined, [[getCryptographicEvidence]] will use this as the serialization.
    */
  def deserializedFrom: Option[ByteString]

  /** Computes the serialization of the object as a [[com.google.protobuf.ByteString]].
    *
    * Must meet the contract of [[getCryptographicEvidence]] except that when called several times,
    * different [[com.google.protobuf.ByteString]]s may be returned.
    */
...
/** Thrown by [[MemoizedEvidenceWithFailure]] classes during object construction if the
  * serialization fails.
  *
...
/** Effectively immutable [[HasCryptographicEvidence]] classes can mix in this trait to implement
  * the memoization logic.
  *
  * Use this class if serialization may fail. This mix-in checks whenever an object is constructed
  * that either a serialization is given or that serialization will succeed. It also ensures that no
  * `copy` method is generated for case classes with this mixin.
  *
  * Make sure that `fromByteString(byteString).deserializedFrom` equals `Some(byteString)`.
  *
  * Make sure that every public constructor and apply method yields an instance with
...
trait MemoizedEvidenceWithFailure[SerializationError] extends HasCryptographicEvidence with NoCopy {
  ...
  /** Computes the serialization of the object as a [[com.google.protobuf.ByteString]] or produces a
    * `SerializationError` if serialization fails.
    *
    * Must meet the contract of [[getCryptographicEvidence]] except that different
    * [[com.google.protobuf.ByteString]]s may be returned when called several times and it may fail
    * at any time.
    */
