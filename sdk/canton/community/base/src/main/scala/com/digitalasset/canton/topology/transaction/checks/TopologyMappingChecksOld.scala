...
import com.digitalasset.canton.topology.processing.TopologyStateProcessorOld.MaybePending
...
import com.digitalasset.canton.topology.transaction.checks.TopologyMappingChecksOld.PendingChangesLookup
...
object TopologyMappingChecksOld {
  type PendingChangesLookup = scala.collection.Map[MappingHash, MaybePending]

  class All(fst: TopologyMappingChecksOld, rest: TopologyMappingChecksOld*)(implicit
      executionContext: ExecutionContext
  ) extends TopologyMappingChecksOld {
...
trait TopologyMappingChecksOld {
...
abstract class TopologyMappingChecksOldWithStore(
...
/** Topology mapping checks which verify invariants on the topology state
  ...
  */
class RequiredTopologyMappingChecksOld(
    maybeStore: MaybeEmptyTopologyStore,
    parameters: Option[StaticSynchronizerParameters],
    loggerFactory: NamedLoggerFactory,
)(implicit
    executionContext: ExecutionContext
) extends TopologyMappingChecksOldWithStore(maybeStore, loggerFactory) {
...
object RequiredTopologyMappingChecksOld {
  def apply(
      store: TopologyStore[TopologyStoreId],
      parameters: Option[StaticSynchronizerParameters],
      loggerFactory: NamedLoggerFactory,
  )(implicit executionContext: ExecutionContext): RequiredTopologyMappingChecksOld =
