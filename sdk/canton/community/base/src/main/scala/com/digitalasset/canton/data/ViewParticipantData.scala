  ...
  def rootAction(): RootAction =
    actionDescription match {
      ...
      case FetchActionDescription(
        ...
        // Template Id is not populated prior to ProtocolVersion.v6
        val templateId =
          templateIdO.getOrElse(inputContract.contract.contractInstance.unversioned.template)
        ...
        val packageIdPreference = if (interfaceId.isDefined) Set(templateId.packageId) else Set.empty[LfPackageId]
