...
trait SyncCryptoSigner extends NamedLogging {
  ...
  protected def verificationParallelismLimit: PositiveInt
  protected def signingAlgorithmSpec: Option[SigningAlgorithmSpec]
  protected def staticSynchronizerParameters: StaticSynchronizerParameters

  protected def signPublicApiWithLongTermKeys: SynchronizerCryptoPureApi

  /** The software-based crypto public API that is used to sign protocol messages and verify their
    * signatures with a session signing key (generated in software). Except for the signing scheme,
    * when signing with session keys is enabled, all other schemes are not needed. Therefore, we use
    * fixed schemes (i.e. placeholders) for the other crypto parameters.
    *
    * //TODO(#23731): Split up pure crypto into smaller modules and only use the signing module here
    */
  protected lazy val signPublicApiSoftwareBased: SynchronizerCryptoPureApi = {
    val pureCryptoForSessionKeys = new JcePureCrypto(
      Aes128Gcm, // not used
      signingAlgorithmSpec.getOrElse(Ed25519 /* not used */ ),
      signPublicApiWithLongTermKeys.supportedSigningAlgorithmSpecs,
      RsaOaepSha256, // not used
      NonEmpty.mk(Set, RsaOaepSha256), // not used
      Sha256, // not used
      PbkdfScheme.Argon2idMode1, // not used
      loggerFactory,
    )
  ...
  private def verifySignatureWithLongTermKey(
    ...
    (for {
      ...
      _ <- signPublicApiWithLongTermKeys.verifySignature(hash, keyToUse, signature, usage)
