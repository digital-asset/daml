...
object SeqUtil {
  ...
  /** Picks a random subset of indices of size `size` from `xs` and returns a random permutation of
    * the elements at these indices. Implements the Fisher-Yates shuffle
    * (https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle) with a separate output array.
    *
    * Note: The inside-out version of Fisher-Yates would not have to modify the input sequence, but
    * doesn't work for picking a proper subset because it would be equivalent to shuffling only the
    * prefix of length size.
    */
