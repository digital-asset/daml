...
/** A fanout signaller, representing a stream of external updates, that can be subscribed to
  * dynamically at a given point in the stream. Stream positions are given by the Index type, and
  * stream values are given by T. Subscribing to a point yields all values starting at that point.
...
trait Dispatcher[Index] {
  ...
  /** Triggers shutdown of the Dispatcher by completing all outstanding stream subscriptions. This
    * method ensures that all outstanding subscriptions have been notified with the latest signalled
    * head and waits for their graceful completion.
    */
  def shutdown(): Future[Unit]

  /** Triggers shutdown of the Dispatcher by eagerly failing all outstanding stream subscriptions
    * with a throwable.
    *
    * @param throwableBuilder
    *   Create a new throwable. It is important to create a new throwable for each failed
    *   subscription since the throwable closing the streams or parts of it can be mutable. (e.g.
    *   the [[io.grpc.Metadata]] provided as part of [[io.grpc.StatusRuntimeException]] is mutated
    *   in the gRPC layer)
    */
...
object Dispatcher {
  ...
  /** Construct a new Dispatcher. This will consume Pekko resources until closed.
    *
    * @param firstIndex
    *   the initial starting Index instance
    * @param headAtInitialization
    *   the head index at the time of creation
    * @tparam Index
    *   The index type
    * @return
    *   A new Dispatcher.
