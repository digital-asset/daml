...
class BestFittingBatcherTest extends AnyWordSpec with BaseTest {
  "BestFitter" should {
    "pack items optimally" in {
      ...
      fitter.add(TestItems(maxSize + 1)) shouldBe false
    }
  }
}
...
object BestFittingBatcherTest {
  ...
  final case class TestItems(override val size: Int) extends BestFittingBatcher.Sized {
    val data: Seq[Unit] = Vector.fill(size)(())
