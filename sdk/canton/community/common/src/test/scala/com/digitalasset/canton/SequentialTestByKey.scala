...
/** Our tests are typically run in parallel but sometimes we want to make sure specific tests are
  * not run in parallel. One example of this are tests that change the same DB table and it doesnt
  * make sense to change the table just for the sake of parallel testing ability. By adding this
  * mixin, semaphores will be used to only allow one test concurrently with the same semaphoreKey
  */
