...
import com.digitalasset.canton.topology.transaction.checks.OptionalTopologyMappingChecksOld
import com.digitalasset.canton.topology.{DefaultTestIdentities, MediatorId, SequencerId}

import scala.annotation.nowarn

@nowarn("msg=match may not be exhaustive")
class OptionalTopologyMappingChecksOldTest
    extends BaseTopologyMappingChecksOldTest[OptionalTopologyMappingChecksOld] {

  override protected def mkChecks(
      store: TopologyStore[SynchronizerStore]
  ): OptionalTopologyMappingChecksOld =
    new OptionalTopologyMappingChecksOld(
      store,
      loggerFactory,
    )
