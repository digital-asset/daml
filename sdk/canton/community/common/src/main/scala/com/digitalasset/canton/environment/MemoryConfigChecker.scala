  ...
  def check(config: StartupMemoryCheckConfig, logger: TracedLogger)(implicit
    ...
    val checkT = for {
      ...
      (check, config) match {
        ...
        case (false, StartupMemoryCheckConfig(ReportingLevel.Warn)) =>
          logger.warn(warnMessage)
        case (false, StartupMemoryCheckConfig(ReportingLevel.Ignore)) =>
          logger.debug(warnMessage)
        case (false, StartupMemoryCheckConfig(ReportingLevel.Crash)) =>
