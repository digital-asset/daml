...
/** Stand up a gRPC service stub which will direct to an instance set dynamically at runtime if
  * available, or return a UNAVAILBLE status if not set.
  *
  * Use setInstance and clear to update the backing service instance.
  *
  * There will be a performance penalty as the method on the backing service is looked up on each
  * call, however this is likely fine for services that are rarely used like admin commands.
  *
  * Currently works with typeless [[io.grpc.ServiceDescriptor]] instances. Validation could be added
  * to ensure full names of services and methods match.
  */
