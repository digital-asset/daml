...
import com.digitalasset.canton.store.{IndexedSynchronizer, SavePendingSendError, SendTrackerStore}
...
class DbSendTrackerStore_Unused(
      ...
      rowsUpdated <- EitherT.right(
        storage.update(
          sqlu"""insert into sequencer_client_pending_sends (synchronizer_idx, message_id, max_sequencing_time)
                 values ($indexedSynchronizer, $messageId, $maxSequencingTime)
                 on conflict do nothing""",
          operationName = s"${this.getClass}: save pending send",
        )
      )
      _ <-
        if (rowsUpdated == 1) EitherT.rightT[FutureUnlessShutdown, SavePendingSendError](())
        else {
          ...
          EitherT(for {
            existingMaxSequencingTimeO <- storage.query(
              sql"""select max_sequencing_time from sequencer_client_pending_sends
                    where synchronizer_idx = $indexedSynchronizer and message_id = $messageId"""
                .as[CantonTimestamp]
                .headOption,
              functionFullName,
            )
  ...
  override def fetchPendingSends(implicit
    ...
    for {
      items <- storage.query(
        sql"select message_id, max_sequencing_time from sequencer_client_pending_sends where synchronizer_idx = $indexedSynchronizer"
  ...
  override def removePendingSend(
    ...
    storage.update_(
      sqlu"delete from sequencer_client_pending_sends where synchronizer_idx = $indexedSynchronizer and message_id = $messageId",
      functionFullName,
    )
