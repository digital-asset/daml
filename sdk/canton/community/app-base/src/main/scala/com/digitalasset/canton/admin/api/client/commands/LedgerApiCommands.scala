      ...
      override def createRequest(): Either[String, CompletionStreamRequest] =
        Right(
          CompletionStreamRequest(
            applicationId = applicationId,
            parties = Seq(partyId),
            beginExclusive = Option.unless(beginOffsetExclusive.isEmpty)(
              ApiOffset.assertFromString(beginOffsetExclusive).toLong
            ),
          )
        )
    ...
    final case class Subscribe(
      ...
      override def createRequest(): Either[String, CompletionStreamRequest] = Right {
        CompletionStreamRequest(
          applicationId = applicationId,
          parties = parties,
          beginExclusive = Option.unless(offset.isEmpty)(
            ApiOffset.assertFromString(offset).toLong
          ),
        )
      }
