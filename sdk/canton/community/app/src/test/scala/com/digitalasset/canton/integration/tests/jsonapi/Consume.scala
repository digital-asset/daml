...
/** This example script reads two elements unconditionally, checking whether the first element is
  * "good" and that the second element has `true` as its first member, then performs a future, then
  * drains any remaining elements from the stream, yielding a result that uses elements of all three
  * reads.
  *
  * {{{
...
object Consume {
  ...
  /** Strictly speaking, this function really returns the following, these are just passed to
    * `Sink.foldAsync` for convenience:
    *
    * {{{
    *   {
    *     type O
