...
sealed trait KeyManagementIntegrationTest
  ...
  "A Canton operator" can {
    ...
    "upload a key pair where the private and public keys do not match" in { implicit env =>
      ...
      // this will not fail (but throws a warning) because the 'wrong' public key is ignored and will be derived from the private key
      loggerFactory.assertLogs(
        ...
        _.shouldBeCantonError(
          InvalidKeyPairAlarm,
          _ shouldBe s"""The derived public key (${publicSigningKeyB.id}) does not match the provided public key (${publicSigningKeyA.id}). This may indicate an invalid or mismatched key pair.""",
        ),
      )
