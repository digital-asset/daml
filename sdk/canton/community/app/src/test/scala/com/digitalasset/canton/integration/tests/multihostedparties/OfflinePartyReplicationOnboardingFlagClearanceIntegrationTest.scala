...
import com.digitalasset.canton.topology.{ExternalParty, Party, PartyId, PhysicalSynchronizerId}
...
sealed trait OfflinePartyReplicationOnboardingFlagClearanceIntegrationTest
  ...
  protected def checkOnboardingFlag(
    ...
    lastPTP.partyId shouldBe testedParty
  ...
  /** Gets the `validFrom` timestamp of the PTP transaction that onboarded the party to the target.
    */
  protected def getOnboardingEffectiveAt(
      daId: PhysicalSynchronizerId,
      testedParty: PartyId,
  ): CantonTimestamp =
    CantonTimestamp
      .fromInstant(
        source.topology.party_to_participant_mappings
          .list(
            synchronizerId = daId,
            filterParty = testedParty.filterString,
  ...
  protected def computeExpectedDecisionDeadline(
  ...
  protected def testOnboardingFlagClearance(alice: Party, bob: Party)(implicit
    ...
    alice match {
      case _: ExternalParty =>
        source.topology.party_to_participant_mappings.sign_and_update(
          party = alice.partyId,
          daId,
          ptp =>
            ptp.tryCopy(
              participants =
                ptp.participants :+ HostingParticipant(target.id, Submission, onboarding = true)
            ),
        )
      case _: PartyId =>
    ...
    checkOnboardingFlag(daId, setOnTarget = true, alice.partyId)
    ...
    val status = target.parties.clear_party_onboarding_flag(alice.partyId, daId, targetLedgerEnd)
    val expectedTimestamp = computeExpectedDecisionDeadline(
      getOnboardingEffectiveAt(daId, alice.partyId)
    )

    status shouldBe (false, Some(expectedTimestamp))

    // Advance time to allow the asynchronous onboarding flag clearance to complete
    clock.advanceTo(expectedTimestamp.plusSeconds(10))
...
final class OffPROnboardingFlagClearanceIntegrationTest
  ...
  "Party replication sets and clears the onboarding flag successfully for local parties" in {
    implicit env =>
      testOnboardingFlagClearance(alice, bob)
  }

  "Party replication sets and clears the onboarding flag successfully for external parties" in {
    implicit env =>
      import env.*
      val aliceE = participant1.parties.external.enable("alice")
      val bobE = participant2.parties.external.enable("bob")

      testOnboardingFlagClearance(aliceE, bobE)
  }
}
