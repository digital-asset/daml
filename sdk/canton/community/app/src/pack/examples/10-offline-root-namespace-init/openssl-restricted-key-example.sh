...
openssl pkeyutl -rawin -inkey "$PRIVATE_KEY" -keyform DER -sign -in "$RESTRICTED_KEY_NAMESPACE_PREFIX.hash" > "$RESTRICTED_KEY_NAMESPACE_PREFIX.signature"
# [end-docs-entry: sign restricted key cert]

# Assemble signature and transaction
# [start-docs-entry: assemble restricted key cert]
"$SCRIPTS_ROOT/assemble-cert.sh" --prepared-transaction "$RESTRICTED_KEY_NAMESPACE_PREFIX.prep" --signature "$RESTRICTED_KEY_NAMESPACE_PREFIX.signature" --signature-algorithm "$SIGNATURE_ALGORITHM" --output "$RESTRICTED_KEY_NAMESPACE_PREFIX.cert"
