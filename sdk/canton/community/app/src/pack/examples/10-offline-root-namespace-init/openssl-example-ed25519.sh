...
openssl genpkey -algorithm Ed25519 -outform DER -out "$PRIVATE_KEY"
...
openssl pkeyutl -rawin -inkey "$PRIVATE_KEY" -keyform DER -sign -in "$ROOT_NAMESPACE_PREFIX.hash" > "$ROOT_NAMESPACE_PREFIX.signature"
# [end-docs-entry: sign root cert]
# [start-docs-entry: sign delegation cert]
openssl pkeyutl -rawin -inkey "$PRIVATE_KEY" -keyform DER -sign -in "$INTERMEDIATE_NAMESPACE_PREFIX.hash" > "$INTERMEDIATE_NAMESPACE_PREFIX.signature"
# [end-docs-entry: sign delegation cert]

# Assemble signature and transaction
# [start-docs-entry: assemble root cert]
"$SCRIPTS_ROOT/assemble-cert.sh" --prepared-transaction "$ROOT_NAMESPACE_PREFIX.prep" --signature "$ROOT_NAMESPACE_PREFIX.signature" --signature-algorithm ed25519 --output "$ROOT_NAMESPACE_PREFIX.cert"
# [end-docs-entry: assemble root cert]
# [start-docs-entry: assemble delegation cert]
"$SCRIPTS_ROOT/assemble-cert.sh" --prepared-transaction "$INTERMEDIATE_NAMESPACE_PREFIX.prep" --signature "$INTERMEDIATE_NAMESPACE_PREFIX.signature" --signature-algorithm ed25519 --output "$INTERMEDIATE_NAMESPACE_PREFIX.cert"
