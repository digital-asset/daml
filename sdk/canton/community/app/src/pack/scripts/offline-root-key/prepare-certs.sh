...
usage() {
  echo "Usage: $0 [--root-delegation] --root-pub-key <path> --target-pub-key <path> --target-key-spec <ed25519|ecp256|ecp384|secp256k1> --output <prefix>"
  echo
  echo "Arguments:"
  echo "  --root-delegation              Optional. Indicates if the root public key is delegating to itself."
...
if [[ -n "$TARGET_PUB_KEY" ]]; then
  ...
  NAMESPACE_MAPPING=$(build_namespace_mapping "$ROOT_NAMESPACE_FINGERPRINT" "CRYPTO_KEY_FORMAT_DER" "$NAMESPACE_PUBLIC_KEY_BASE64" "$TARGET_KEY_SPEC" "$ROOT_DELEGATION")
else
  # Target pub key is a serialized "SigningPublicKey"
  if ! NAMESPACE_SIGNING_KEY=$(serialized_versioned_message_to_json "$CRYPTO_PROTO" "com.digitalasset.canton.crypto.v30.SigningPublicKey" < "$CANTON_TARGET_PUB_KEY"); then
  ...
  NAMESPACE_MAPPING=$(build_namespace_mapping_from_signing_key "$ROOT_NAMESPACE_FINGERPRINT" "$NAMESPACE_SIGNING_KEY" "$ROOT_DELEGATION" | jq .)
fi
