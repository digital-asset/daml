...
import com.digitalasset.canton.metrics.{BatchLoaderMetrics, Metrics}
        ...
        new PekkoStreamParallelBatchedLoader[
          ...
          batchLoad = { batch =>
            val (latestValidAtOffset, usedLoggingContext) = maxOffsetAndContextFromBatch(
              batch,
              metrics.daml.index.db.activeContracts.batchSize,
            )
          ...
          createQueue = () => createQueue(maxQueueSize, metrics.daml.index.db.activeContracts),
          maxBatchSize = maxBatchSize,
      ...
      .forReleasable(() =>
        new PekkoStreamParallelBatchedLoader[
          ...
          batchLoad = { batch =>
            ...
            val contractKeysF =
              dbDispatcher
                .executeSql(metrics.daml.index.db.lookupContractByKeyDbMetrics)(
          ...
          createQueue =
            () => ContractLoader.createQueue(maxQueueSize, metrics.daml.index.db.activeContracts),
          maxBatchSize = maxBatchSize,
          parallelism = parallelism,
          loggerFactory = loggerFactory,
        )
