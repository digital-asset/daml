...
class CachedEventsReader(
  ...
  override def getEventsByContractKey(
    ...
    val key = GlobalKey.assertBuild(templateId, contractKey)
    ...
    val processCached: KeyUpdates => Future[GetEventsByContractKeyResponse] = {
      case keyUpdates
    ...
    keyContinuationToken match {
      case NoToken => eventsByContractKeyCache.get(key).fold(fromDb())(processCached)
      case _ => fromDb()
    }
  }
}
