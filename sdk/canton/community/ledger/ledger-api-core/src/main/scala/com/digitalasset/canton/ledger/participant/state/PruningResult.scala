...
object PruningResult {
  ...
  /** Pruning has been performed. Commits the ledger api server to prune at the same offset as
    * passed to the SyncService.
    */
  ...
  /** Pruning was not performed. Indicates to ledger api server not to proceed with pruning either.
    *
    * @param grpcStatus
    *   grpcStatus created using error codes API (see [[com.daml.error.ErrorCode]]). Examples of
    *   gRPC status codes specific to pruning:
    *
    ...
    *
    * INTERNAL: If a severe error has been encountered, particularly indicating that pruning has
    * only been partially applied.
    */
