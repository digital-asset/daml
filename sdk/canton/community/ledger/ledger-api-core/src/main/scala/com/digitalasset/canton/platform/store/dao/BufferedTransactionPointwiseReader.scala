...
/** Generic class that helps serving Ledger API point-wise lookups
  * (UpdateService.{GetTransactionById, GetTransactionTreeById, GetTransactionByOffset,
  * GetTransactionTreeByOffset}) from either the in-memory fan-out buffer or from persistence.
  *
  * @param fetchFromPersistence
  *   Fetch a transaction by offset or id from persistence.
  * @param toApiResponse
  *   Convert a
  *   [[com.digitalasset.canton.platform.store.interfaces.TransactionLogUpdate.TransactionAccepted]]
  *   to a specific API response while also filtering for visibility.
...
class BufferedTransactionPointwiseReader[QUERY_PARAM_TYPE, API_RESPONSE](
  ...
  /** Serves processed and filtered transaction from the buffer by the query parameter, with
    * fallback to a persistence fetch if the transaction is not anymore in the buffer (i.e. it was
    * evicted)
    *
    * @param queryParam
    *   The query parameter.
