...
/** A handle of a running program
  ...
  *   - if KillSwitch shut down, this completes successfully After signalling completion, the
  *     program finished it's execution and has released all resources it acquired.
  * @param killSwitch
  *   to signal abortion and shutdown
  */
final case class Handle(completed: Future[Unit], killSwitch: KillSwitch)

/** This functionality initializes a worker Connection, and clears it for further usage. This only
  * needs to be done once at the beginning of the Connection life-cycle Initialization errors are
...
/** To add High Availability related features to a program, which intends to use
  * database-connections to do it's work. Features include:
  *   - Safety: mutual exclusion of these programs ensured by DB locking mechanisms
  *   - Availability: release of the exclusion is detected by idle programs, which start competing
  *     for the lock to do their work.
  */
trait HaCoordinator {

  /** Execute in High Availability mode. Wraps around the Handle of the execution.
    *
    * @param initializeExecution
    *   HaCoordinator provides a ConnectionInitializer that must to be used for all database
    *   connections during execution Future[Handle] embodies asynchronous initialization of the
    *   execution (e.g. not the actual work. That asynchronous execution completes with the
    *   completed Future of the Handle)
    * @return
    *   the new Handle, which is available immediately to observe and interact with the complete
    *   program here
    */
...
object HaCoordinator {
  ...
  /** This implementation of the HaCoordinator
    ...
    *   - provides a ConnectionInitializer function which is mandatory to execute on all worker
    *     connections during execution
    *   - will spawn a polling-daemon to observe continuous presence of the main lock
    *
    * @param mainConnectionFactory
    *   to spawn the main connection which keeps the Indexer Main Lock
    * @param storageBackend
    *   is the database-independent abstraction of session/connection level database locking
    * @param executionContext
    *   which is use to execute initialisation, will do blocking/IO work, so dedicated execution
