...
class PackageUpgradeValidator(
          ...
          if (uploadedPackageId == upgradingPackageId) {
            ...
            Future.failed(
              Validation.UpgradeVersion
                .Error(uploadedPackageId, upgradingPackageId, upgradingPackageAst.metadata.map(_.version))
                .asGrpcError
            )
          }
  ...
  private def existingVersionedPackageId(
    ...
    for {
      pkgName <- pkg.metadata.map(_.name)
      pkgVersion <- pkg.metadata.map(_.version)
      result <- packageMap.collectFirst { case (pkgId, (`pkgName`, `pkgVersion`)) => pkgId }
    } yield result
  ...
  private def minimalVersionedDar(
    ...
    ).traverse { case (pId, _) => lookupDar(pId) }.map(_.flatten)
