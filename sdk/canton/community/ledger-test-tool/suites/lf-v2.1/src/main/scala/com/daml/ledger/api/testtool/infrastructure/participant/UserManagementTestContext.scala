...
trait UserManagementTestContext {
  ...
  /** Creates a new user.
    *
    * Additionally keeps track of the created users so that they can be cleaned up automatically
    * when the test case ends.
    */
  def createUser(createUserRequest: CreateUserRequest): Future[CreateUserResponse] =
    for {
      ...
      user = response.user.getOrElse(sys.error("Missing user"))
    ...
    val deletions = createdUsersById
      ...
      .map(userId =>
        services.userManagement
          ...
          .map(_ => ())
          .recover {
            case e if ErrorDetails.matches(e, AdminServiceErrors.UserManagement.UserNotFound) =>
              ()
          }
      )
    Future.sequence(deletions).map(_ => ())
  ...
  def createIdentityProviderConfig(
    ...
    for {
      ...
      idp = response.identityProviderConfig.getOrElse(sys.error("Missing identity provider"))
