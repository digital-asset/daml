...
final class FutureAssertions[T](future: Future[T]) {
  ...
  /** Checks that the future failed, and returns the throwable. We use this instead of
    * `Future#failed` because the error message that delivers is unhelpful. It doesn't tell us what
  ...
  /** Checks that the future failed satisfying the predicate and returns the throwable. We use this
    * instead of `Future#failed` because the error message that delivers is unhelpful. It doesn't
    * tell us what the value actually was.
    */
...
object FutureAssertions {
  ...
  /** Run the test every `retryDelay` up to `maxRetryDuration`. The test case will run up to
    * `ceil(maxRetryDuration / retryDelay)` times. The assertion will succeed as soon as any of the
    * test case runs are successful. The assertion will fail if no test case runs are successful and
    * the `maxRetryDuration` is exceeded.
    */
