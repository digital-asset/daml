...
class TimeoutParticipantTestContext(timeoutScaleFactor: Double, delegate: ParticipantTestContext)
  ...
  override def legacyActiveContractsRequest(
      parties: Option[Seq[Party]],
      activeAtOffset: Long,
      templateIds: Seq[Identifier],
      interfaceFilters: Seq[(Identifier, IncludeInterfaceView)] = Seq.empty,
  ): GetActiveContractsRequest =
    delegate.legacyActiveContractsRequest(
      parties,
      activeAtOffset,
      templateIds,
      interfaceFilters,
    )
  override def legacyActiveContracts(
      parties: Option[Seq[Party]],
      activeAtOffsetO: Option[Long],
  ): Future[Vector[CreatedEvent]] =
    withTimeout(
      s"Active contracts for parties $parties",
      delegate.legacyActiveContracts(parties, activeAtOffsetO),
    )
  override def legacyActiveContractsByTemplateId(
      templateIds: Seq[Identifier],
      parties: Option[Seq[Party]],
      activeAtOffsetO: Option[Long],
  ): Future[Vector[CreatedEvent]] = withTimeout(
    s"Active contracts by template ids $templateIds for parties $parties",
    delegate.legacyActiveContractsByTemplateId(templateIds, parties, activeAtOffsetO),
  )
  ...
  override def getTransactionsRequestLegacy(
      transactionFilter: TransactionFilter,
      begin: Long,
  ): Future[GetUpdatesRequest] = delegate.getTransactionsRequestLegacy(
    transactionFilter = transactionFilter,
    begin = begin,
  )

  override def getTransactionsRequestWithEndLegacy(
      transactionFilter: TransactionFilter,
      begin: Long = referenceOffset,
      end: Option[Long],
  ): GetUpdatesRequest = delegate.getTransactionsRequestWithEndLegacy(transactionFilter, begin, end)
  ...
  override def flatTransactionsByTemplateIdLegacy(
      templateId: Identifier,
      parties: Option[Seq[Party]],
  ): Future[Vector[Transaction]] = withTimeout(
    s"Flat transaction by template id $templateId for parties $parties",
    delegate.flatTransactionsByTemplateIdLegacy(templateId, parties),
  )
  ...
  override def flatTransactionsLegacy(parties: Party*): Future[Vector[Transaction]] =
    withTimeout(
      s"Flat transactions for parties $parties",
      delegate.flatTransactionsLegacy(parties*),
    )
  ...
  override def flatTransactionsLegacy(take: Int, parties: Party*): Future[Vector[Transaction]] =
    withTimeout(
      s"$take flat transactions for parties $parties",
      delegate.flatTransactionsLegacy(take, parties*),
    )
