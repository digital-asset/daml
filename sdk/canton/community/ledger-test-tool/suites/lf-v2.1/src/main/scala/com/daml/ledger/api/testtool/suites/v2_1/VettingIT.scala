...
class VettingIT extends LedgerTestSuite {
  ...
  private def vetPkgIdsRequest(
    ...
    changeOpsRequest(
      Seq(refsToVetOp(pkgIds.map((pkgId: String) => VettedPackagesRef(Some(pkgId), None, None)))),
      dryRun,
    )
          ...
          Seq(
            op(
              mainPackageIds.map((pkgId: Ref.PackageId) =>
                VettedPackagesRef(Some(pkgId.toString), None, None)
              )
            )
          ),
          false,
  ...
  updateTest(
    "PVUpdateVetByNameSucceeds",
    "Successfully vet multiple packages by name",
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          // TODO(#27753) Expected to fail once 27753 is complete
          _ <- vetAllInDar(ledger, VettingDepDar.path)
          _ <- vetPkgsMatchingRef(ledger, VettedPackagesRef(None, Some(vettingMainName), None))
          vetBothByNameResponse <- ledger.listVettedPackages(listAllRequest)
        } yield assertResponseHasAllVersions(vetBothByNameResponse),
  )
    ...
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          // TODO(#27753) Expected to fail once 27753 is complete
          _ <- vetAllInDar(ledger, VettingDepDar.path)
          _ <- vetPkgsMatchingRef(ledger, VettedPackagesRef(None, Some("nonexistent-name"), None))

          vetNoneByNonexistentNameResponse <- ledger.listVettedPackages(listAllRequest)
        } yield assertResponseHasDep(vetNoneByNonexistentNameResponse),
  )
      ...
      (ledger: ParticipantTestContext) =>
        for {
          ...
          _ <- vetPkgsMatchingRef(
            ledger,
            VettedPackagesRef(None, Some(vettingMainName), Some("2.0.0")),
          )
    ...
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          ...
          _ <- vetPkgsMatchingRef(
            ledger,
            VettedPackagesRef(None, Some(vettingMainName), Some("3.0.0")),
          )
          vetNoneByNonexistentVersionResponse <- ledger.listVettedPackages(listAllRequest)
        } yield assertResponseHasDep(vetNoneByNonexistentVersionResponse),
  )
      ...
      (ledger: ParticipantTestContext) =>
        for {
          ...
          _ <- vetPkgsMatchingRef(
            ledger,
            VettedPackagesRef(
              Some(vettingMainPkgIdV2),
              Some(vettingMainName),
              Some("2.0.0"),
            ),
          )
    ...
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          ...
          vetNoneByPkgIdWithWrongName <- ledger.listVettedPackages(listAllRequest)
        } yield assertResponseHasDep(vetNoneByPkgIdWithWrongName),
  )
    ...
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          ...
          vetNoneByPkgIdWithWrongVersion <- ledger.listVettedPackages(listAllRequest)
        } yield assertResponseHasDep(vetNoneByPkgIdWithWrongVersion),
  )
  ...
  test(
    ...
    for {
      ...
      _ <- vetPkgsMatchingRef(
        ledger,
        VettedPackagesRef(Some(vettingMainPkgIdV2), None, None),
      ...
      _ <- ledger
        .updateVettedPackages(
          changeOpsRequest(
            Seq(
              refsToVetOp(
                Seq(VettedPackagesRef(Some(vettingMainPkgIdV2), None, None))
              ),
              refsToUnvetOp(
                Seq(VettedPackagesRef(Some(vettingDepPkgId), None, None))
              ),
            )
          )
        )
