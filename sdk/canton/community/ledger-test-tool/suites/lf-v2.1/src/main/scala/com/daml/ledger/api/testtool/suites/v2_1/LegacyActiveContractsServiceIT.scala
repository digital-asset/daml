...
class LegacyActiveContractsServiceIT extends LedgerTestSuite {
  ...
  test(
    ...
    for {
      activeContracts <- ledger.legacyActiveContracts(Some(Seq(party)))
    } yield {
      assert(
      ...
      activeContractsPartyWildcardAndSpecificParty <- ledger
        .activeContracts(
          new GetActiveContractsRequest(
            filter = Some(
              TransactionFilter(
                filtersByParty = ledger.transactionFilter(Some(Seq(party))).filtersByParty,
                filtersForAnyParty = ledger.transactionFilter(None).filtersForAnyParty,
              )
            ),
            verbose = true,
            activeAtOffset = end,
  ...
  test(
    ...
    for {
      offsetAtTestStart <- ledger.currentEnd()
      (dummy, dummyWithParam, dummyFactory) <- createDummyContracts(party, ledger)
      activeContracts <- ledger.legacyActiveContractsByTemplateId(
        Seq(Dummy.TEMPLATE_ID),
        Some(Seq(party)),
      )
      activeContractsPartyWildcard <- ledger
        .legacyActiveContractsByTemplateId(Seq(Dummy.TEMPLATE_ID), None)
        .map(_.filter(_.offset > offsetAtTestStart))
      _ <- archive(ledger, party)(dummy, dummyWithParam, dummyFactory)
    } yield {
      assert(
  ...
  test(
    ...
    for {
      (dummy, dummyWithParam, dummyFactory) <- createDummyContracts(party, ledger)
      contractsBeforeExercise <- ledger.legacyActiveContracts(Some(Seq(party)))
      _ <- ledger.exercise(party, dummy.exerciseDummyChoice1())
      contractsAfterExercise <- ledger.legacyActiveContracts(Some(Seq(party)))
  ...
  test(
    ...
    for {
      ...
      onlyDummy <- ledger.activeContracts(
        ledger.legacyActiveContractsRequest(Some(Seq(party)), offset)
      )
  ...
  test(
    ...
    for {
      ...
      verboseRequest = ledger
        .legacyActiveContractsRequest(Some(Seq(party)), end)
      ...
      verboseEventsPartyWildcard <- ledger.activeContracts(
        ledger.legacyActiveContractsRequest(None, end).update(_.verbose := true)
      )
      nonVerboseEventsPartyWildcard <- ledger.activeContracts(
        ledger.legacyActiveContractsRequest(None, end).update(_.verbose := false)
      )
      _ <- ledger.exercise(party, dummy.exerciseArchive())
    } yield {
  ...
  test(
    ...
    for {
      ...
      allContractsForAlice <- ledger.legacyActiveContracts(Some(Seq(alice)))
      allContractsForBob <- ledger.legacyActiveContracts(Some(Seq(bob)))
      allContractsForAliceAndBob <- ledger.legacyActiveContracts(Some(Seq(alice, bob)))
      allContractsPartyWildcard <- ledger
        .legacyActiveContracts(None)
        .map(_.filter(_.offset > offsetAtTestStart))
      dummyContractsForAlice <- ledger.legacyActiveContractsByTemplateId(
        Seq(Dummy.TEMPLATE_ID),
        Some(Seq(alice)),
      )
      dummyContractsForAliceAndBob <- ledger.legacyActiveContractsByTemplateId(
        Seq(Dummy.TEMPLATE_ID),
        Some(Seq(alice, bob)),
      )
      dummyContractsPartyWildcard <- ledger
        .legacyActiveContractsByTemplateId(
  ...
  test(
    ...
    for {
      dummy <- ledger.create(party, new Dummy(party))
      Vector(dummyEvent) <- ledger.legacyActiveContracts(Some(Seq(party)))
      offset <- ledger.currentEnd()
      flatTransaction <- ledger.transactionByOffset(dummyEvent.offset, Seq(party), AcsDelta)
      transactionTree <- ledger.transactionTreeByOffset(dummyEvent.offset, party)
      _ <- ledger.exercise(party, dummy.exerciseArchive())
    } yield {
      assert(
        flatTransaction.updateId == transactionTree.updateId,
        s"Offset ${dummyEvent.offset} did not resolve to the same flat transaction (${flatTransaction.updateId}) and transaction tree (${transactionTree.updateId}).",
      )
      assert(
        dummyEvent.offset == flatTransaction.offset,
        s"Active contract's created event ${dummyEvent.offset} was not equal to transaction's offset $offset.",
      )
      assert(
        flatTransaction.events.sizeIs == 1,
        "Expected only a single event for flat transaction",
      )
      flatTransaction.events
        .flatMap(_.event.created)
        .foreach(createdEvent =>
          assert(
            flatTransaction.offset == createdEvent.offset,
            s"Flat transaction's created event ${dummyEvent.offset} was not equal to transaction's offset ${flatTransaction.offset}.",
          )
        )
      assert(
        transactionTree.events.sizeIs == 1,
        "Expected only a single event for transaction tree",
      )
      transactionTree.events
        .flatMap(_.event.created)
        .foreach(createdEvent =>
          assert(
            transactionTree.offset == createdEvent.offset,
            s"Transaction tree's created event ${dummyEvent.offset} was not equal to transaction tree's offset ${transactionTree.offset}.",
          )
        )
      ...
      transactionTree.events
        ...
        .foreach { case (treeEvent, flatEvent) =>
          assert(
            treeEvent.nodeId == flatEvent.nodeId,
            s"Transaction tree's event (at offset ${treeEvent.offset}) node id ${treeEvent.nodeId} was not " +
              s"equal to flat transaction's event (at offset ${flatEvent.offset}) node id ${flatEvent.nodeId}.",
          )
        }
    }
  })
  ...
  test(
    ...
    for {
      ...
      bobContracts <- ledger.legacyActiveContracts(Some(Seq(bob)))
      aliceContracts <- ledger.legacyActiveContracts(Some(Seq(alice)))
  ...
  test(
    ...
    for {
      ...
      contracts <- ledger.legacyActiveContracts(Some(Seq(alice)))
  ...
  test(
    ...
    for {
      ...
      bobContracts <- ledger.legacyActiveContracts(Some(Seq(bob)))
      aliceBobContracts <- ledger.legacyActiveContracts(Some(Seq(alice, bob)))
      bobCharlieContracts <- ledger.legacyActiveContracts(Some(Seq(bob, charlie)))
      partyWildcardContracts <- ledger
        .legacyActiveContracts(None)
        .map(_.filter(_.offset > offsetAtTestStart))
      _ <- ledger.exercise(alice, contract.exerciseArchive())
