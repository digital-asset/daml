...
trait ParticipantTestContext extends UserManagementTestContext {
  ...
  // TODO(i23504) Remove
  def legacyActiveContractsRequest(
      parties: Option[Seq[Party]],
      activeAtOffset: Long,
      templateIds: Seq[Identifier] = Seq.empty,
      interfaceFilters: Seq[(Identifier, IncludeInterfaceView)] = Seq.empty,
  ...
  // TODO(i23504) Remove
  def legacyActiveContracts(
      parties: Option[Seq[Party]],
      activeAtOffsetO: Option[Long] = None,
  ): Future[Vector[CreatedEvent]]
  // TODO(i23504) Remove
  def legacyActiveContractsByTemplateId(
      templateIds: Seq[Identifier],
      parties: Option[Seq[Party]],
      activeAtOffsetO: Option[Long] = None,
  ): Future[Vector[CreatedEvent]]
  ...
  /** Create a TransactionFilter with a set of Party objects.
    ...
    * [[transactions(request:com\.daml\.ledger\.api\.v2\.update_service\.GetUpdatesRequest):*]] or
  ...
  // TODO(#23504) remove
  def flatTransactionsByTemplateIdLegacy(
      templateId: Identifier,
      parties: Option[Seq[Party]],
  ): Future[Vector[Transaction]]
  ...
  /** Managed version of
    * [[flatTransactionsLegacy(parties:com\.daml\.ledger\.api\.testtool\.infrastructure\.Party*):*]],
    * use this unless you need to tweak the request (i.e. to test low-level details)
    */
