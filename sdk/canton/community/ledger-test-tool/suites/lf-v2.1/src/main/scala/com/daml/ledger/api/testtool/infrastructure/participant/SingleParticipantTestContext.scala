...
final class SingleParticipantTestContext private[participant] (
  ...
  override def legacyActiveContractsRequest(
    ...
    new GetActiveContractsRequest(
      filter = Some(
        transactionFilter(parties, templateIds, interfaceFilters)
      ),
      verbose = true,
      activeAtOffset = activeAtOffset,
      eventFormat = None,
  ...
  override def legacyActiveContracts(
    ...
    legacyActiveContractsByTemplateId(Seq.empty, parties, activeAtOffsetO)
  ...
  def getTransactionsRequestLegacy(
  ...
  def getTransactionsRequestWithEndLegacy(
      transactionFilter: TransactionFilter,
      begin: Long = referenceOffset,
      end: Option[Long],
  ): GetUpdatesRequest =
  ...
  override def flatTransactionsByTemplateIdLegacy(
    ...
    getTransactionsRequestLegacy(transactionFilter(parties, Seq(templateId)))
      .flatMap(transactions)
  ...
  override def flatTransactionsLegacy(parties: Party*): Future[Vector[Transaction]] =
    getTransactionsRequestLegacy(transactionFilter(Some(parties))).flatMap(transactions)
  ...
  override def flatTransactionsLegacy(take: Int, parties: Party*): Future[Vector[Transaction]] =
    getTransactionsRequestLegacy(transactionFilter(Some(parties))).flatMap { txReq =>
      transactions(take, txReq)
    }
