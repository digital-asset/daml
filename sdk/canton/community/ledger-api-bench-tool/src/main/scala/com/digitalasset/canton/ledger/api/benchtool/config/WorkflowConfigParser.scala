...
object WorkflowConfigParser {
  ...
  object Decoders {
    ...
    implicit val transactionStreamDecoder: Decoder[StreamConfig.TransactionsStreamConfig] =
      (c: HCursor) => {
        for {
          ...
          beginOffset <- c.downField("begin_offset").as[Option[Long]]
          endOffset <- c.downField("end_offset").as[Option[Long]]
    ...
    implicit val transactionLedgerEffectsStreamDecoder
        : Decoder[StreamConfig.TransactionLedgerEffectsStreamConfig] =
      (c: HCursor) => {
        for {
          name <- c.downField("name").as[String]
          filters <- c.downField("filters").as[Option[List[PartyFilter]]]
          beginOffset <- c.downField("begin_offset").as[Long]
          endOffset <- c.downField("end_offset").as[Option[Long]]
          partyNamePrefixFilters <- c
            .downField("party_prefix_filters")
            .as[Option[List[PartyNamePrefixFilter]]]
          ...
          timeout <- c
            .downField("timeout")
            .as[Option[FiniteDuration]]
        } yield StreamConfig.TransactionLedgerEffectsStreamConfig(
          name = name,
          filters = filters.getOrElse(List.empty),
          partyNamePrefixFilters = partyNamePrefixFilters.getOrElse(List.empty),
    ...
    implicit val streamConfigDecoder: Decoder[StreamConfig] =
      Decoder
        ...
        .flatMap[StreamConfig] {
          ...
          case "transactions-ledger-effects" => transactionLedgerEffectsStreamDecoder.widen
    ...
    implicit val userIdConfigDecoder: Decoder[FooSubmissionConfig.UserId] =
      Decoder.forProduct2(
        "id",
        "weight",
      )(FooSubmissionConfig.UserId.apply)
    ...
    implicit val fooSubmissionConfigDecoder: Decoder[FooSubmissionConfig] =
      (c: HCursor) => {
        for {
          ...
          userIds <- c
            .downField("user_ids")
            .as[Option[List[FooSubmissionConfig.UserId]]]
          ...
          userIds = userIds.getOrElse(List.empty),
