...
class Names {
  ...
  val benchtoolUserId = "benchtool"
  val workflowId = s"$benchtoolUserId-$identifierSuffix"
  ...
  def mainPackageId(index: Int) = s"submission-dars-$index-$identifierSuffix"
...
object Names {
  ...
  /** @return
    *   main prefix of a party which is the prefix up to the first '-' character
    */
