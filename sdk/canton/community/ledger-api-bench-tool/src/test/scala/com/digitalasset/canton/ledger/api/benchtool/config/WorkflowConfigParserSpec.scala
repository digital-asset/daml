...
import com.digitalasset.canton.ledger.api.benchtool.config.WorkflowConfig.FooSubmissionConfig.PartySet
import com.digitalasset.canton.ledger.api.benchtool.config.WorkflowConfig.StreamConfig.PartyNamePrefixFilter
...
class WorkflowConfigParserSpec extends AnyWordSpec with Matchers {
  ...
  "WorkflowConfigParser" should {
        ...
        WorkflowConfig(
          ...
          streams = List(
            WorkflowConfig.StreamConfig.TransactionsStreamConfig(
              ...
              beginOffsetExclusive = 456L,
              endOffsetInclusive = Some(789L),
              objectives = Some(
    ...
    "parse transactions-ledger-effects stream configuration" in {
      val yaml =
        """streams:
          |  - type: transactions-ledger-effects
          |    name: stream-1
          |    filters:
          |      - party: Obs-2
          |        templates:
      ...
      parseYaml(yaml) shouldBe Right(
        WorkflowConfig(
          submission = None,
          streams = List(
            WorkflowConfig.StreamConfig.TransactionLedgerEffectsStreamConfig(
              name = "stream-1",
              filters = List(
                WorkflowConfig.StreamConfig.PartyFilter(
                  party = "Obs-2",
    ...
    "parse begin_offset and end_offset markers absence" in {
      val yaml =
        """streams:
          |  - type: transactions
          |    name: stream-1
          ...
          |         - Foo1
          |         - Foo3
          |    subscription_delay: 7min""".stripMargin
      parseYaml(yaml) shouldBe Right(
        WorkflowConfig(
          submission = None,
          streams = List(
