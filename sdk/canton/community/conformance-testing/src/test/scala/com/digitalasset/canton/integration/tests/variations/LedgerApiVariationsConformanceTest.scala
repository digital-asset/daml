...
class LegacyUniversalStreamConformanceTestPostgres extends SingleVersionLedgerApiConformanceBase {
  registerPlugin(new UsePostgres(loggerFactory))
  registerPlugin(new UseReferenceBlockSequencer[DbConfig.Postgres](loggerFactory))
  ...
  "Ledger Api Test Tool" can {
    "pass legacy universal stream integration tests" in { implicit env =>
      ledgerApiTestToolPlugin.runSuites(
        suites =
          "LegacyActiveContractsServiceIT,LegacyTransactionServiceStreamsIT,LegacyTransactionServiceFiltersIT",
        exclude = Seq(
