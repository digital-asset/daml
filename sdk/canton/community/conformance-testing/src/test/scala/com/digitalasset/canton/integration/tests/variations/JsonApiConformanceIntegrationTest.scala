...
sealed trait JsonApiConformanceBase
    ...
    if (useJson)
      env.participants.all.map {
        ...
        case localParticipantConfig: LocalParticipantReference =>
          val jsonApiServer = localParticipantConfig.config.httpLedgerApi.value.server
          jsonApiServer.address -> jsonApiServer.port.value
        case _other =>
  ...
  def environmentDefinition: EnvironmentDefinition =
    EnvironmentDefinition.P1_S1M1
      .addConfigTransforms(
        updateAllParticipantConfigs_(config =>
...
// TODO(#23504) Remove
// Conformance tests testing the legacy feature during the universal stream rollout
final class JsonLegacyUniversalStreamConformanceTestPostgres
    extends JsonApiConformanceBase
    with NonSharded {
  ...
  override def exclusions: Set[String] = Set(
    "LegacyActiveContractsServiceIT:AcsAtPruningOffsetIsAllowed",
    "LegacyActiveContractsServiceIT:AcsBeforePruningOffsetIsDisallowed",
  )

  override protected def testCaseName: String = "pass legacy universal stream integration tests"
}
