...
abstract class ConfirmationResponseProcessorTestBase
  ...
  val activeMediator1: MediatorId = MediatorId(UniqueIdentifier.tryCreate("mediator", "one"))
  val activeMediator2: MediatorId = MediatorId(UniqueIdentifier.tryCreate("mediator", "two"))
  ...
  lazy val extra: LfPartyId = ExampleTransactionFactory.extra

  // Create a topology with several participants so that we can have several root hash messages or Malformed messages
  val participant1: ParticipantId = participant
  val participant2: ParticipantId = ExampleTransactionFactory.signatoryParticipant
  val participant3: ParticipantId = ParticipantId("participant3")
  ...
  lazy val requestIdTs: CantonTimestamp = CantonTimestamp.Epoch
  lazy val requestId: RequestId = RequestId(requestIdTs)
  lazy val decisionTime: CantonTimestamp = requestIdTs.plusSeconds(120)
  ...
  "ConfirmationResponseProcessor" should {
    def shouldBeViewThresholdBelowMinimumAlarm(
      ...
      _.shouldBeCantonError(
        ...
        _ shouldBe s"Received a mediator request with id $requestId for transaction view at $viewPosition, where no quorum of the list satisfies the minimum threshold. Rejecting request...",
      )
    ...
    "request timestamp is propagated to mediator result when response aggregation is performed" should {
      ...
      val informeeMessage = new InformeeMessage(fullInformeeTree)(testedProtocolVersion) {
        val faultyViewPosition: ViewPosition =
          super.informeesAndConfirmationParamsByViewPosition.collectFirst {
            case (key, ViewConfirmationParameters(informees, _)) if informees != Set(submitter) =>
              key
          }.value
          ...
          super.informeesAndConfirmationParamsByViewPosition map {
            case (key, ViewConfirmationParameters(informee, quorums)) =>
              (
                ...
                ViewConfirmationParameters.tryCreate(
                  informee,
                  quorums.map(_.copy(threshold = {
                    if (key == faultyViewPosition) NonNegativeInt.zero else NonNegativeInt.one
                  })),
                ),
              )
          }
        ...
        override val informeesAndConfirmationParamsByViewPosition
          ...
          Map(
            ViewPosition.root -> ViewConfirmationParameters.tryCreate(
              Map(
                submitter -> TrustLevel.Ordinary,
                signatory -> TrustLevel.Ordinary,
                observer -> TrustLevel.Ordinary,
              ),
              Seq(
                Quorum(
                  Map(
    ...
    "send rejections when receiving wrong root hash messages" in {
      ...
      val batchWithDifferentPayloads = example(
        ...
        correctRootHashMessage.copy(
          payload = SerializedRootHashMessagePayload(ByteString.copyFromUtf8("other payloads"))
        ) -> Recipients
      ...
      : Seq[(((MediatorRequest, List[OpenEnvelope[RootHashMessage[SerializedRootHashMessagePayload]]]), String),
        ...
        (batchWithDifferentPayloads -> show"Different payloads in root hash messages. Sizes: 0, 14.") ->
          List(Set[Member](participant, otherParticipant) -> correctViewType),
      ...
      sequentialTraverse_(testCases.zipWithIndex) {
        case ((((req, rootHashMessages), msg), _), sc) =>
    ...
    "receiving Malformed responses" in {
      ...
      for {
        ...
        finalState <- sut.mediatorState.fetch(requestId).value
        _ = inside(finalState) {
          case Some(FinalizedResponse(_, _, _, Verdict.ParticipantReject(reasons))) =>
    ...
    "timeout request that is not pending should not fail" in {
      ...
      sut.processor.handleTimeout(requestId, timeoutTs, decisionTime).map(_ => succeed)
    }
...
class ConfirmationResponseProcessorTest extends ConfirmationResponseProcessorTestBase {
