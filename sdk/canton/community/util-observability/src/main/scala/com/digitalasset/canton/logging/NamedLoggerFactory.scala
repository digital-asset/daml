...
trait NamedLoggerFactory {
  ...
  /** Regular append method to augment log properties and append both key and value to logger
    * factory name
    *
    * @param key
    *   log property key
  ...
  /** Special purpose append method to augment log properties to append only the value to logger
    * factory name for reduced verbosity
    *
    * @param key
...
object NamedLoggerFactory {
  ...
  /** Adds to kv-map (to appear in upstream logs) and to name without "key/" prefix for brevity
    * (#2768, #2793), e.g. used for test where the test name makes it clear that value refers to a
    * test
    */
