      ...
      "refuse messages for future epochs after reaching queue limits" in {
        ...
        assertLogs(
          ...
          (logEntry: LogEntry) => {
            ...
            logEntry.message should include(
              s"from future epoch 10 as we're still in epoch -1 and the quota for node self for queueing future nodes has been reached"
            )
          },
        )
        ...
        assertLogs(
          ...
          (logEntry: LogEntry) => {
            ...
            logEntry.message should include("Dropped PBFT message")
            logEntry.message should include(
              s"from future epoch 10 as we're still in epoch -1 and total capacity for queueing future nodes has been reached"
            )
          },
        )
