  ...
  private def periodicCheck(
    ...
    if (isClosing) {
      ...
      go(None).foreach { case (effectiveTime, retriesSoFar) =>
        broadcastToAdvanceTime(effectiveTime, retriesSoFar)
      }
    ...
    val sendUS = for {
      ...
      _ <- maybeSequencerGroup match {
        case Some(sequencerGroup) if sequencerGroup.active.contains(thisSequencerId) =>
          ...
          logger.debug(
            s"Sending a time-advancing message to hopefully reach $desiredTimestamp with max sequencing time $maxSequencingTime (message ID $messageId)"
          )
