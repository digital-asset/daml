...
class P2PGrpcConnectionState(override val loggerFactory: NamedLoggerFactory)
  ...
  private val p2pEndpointIdToNetworkRef =
    mutable.Map.empty[
      P2PEndpoint.Id,
      (P2PEndpoint, Boolean, P2PNetworkRef[BftOrderingMessage]),
    ]
  ...
  // Used by simulation's node restart
  def clear(): Unit =
    mutex(this) {
      clientPeerSenders.clear()
      serverPeerSenders.clear()
      p2pEndpointIdToNetworkRef.clear()
      p2pEndpointIdToNodeId.clear()
      nodeIdToP2PEndpointId.clear()
    }
  ...
  override def isOutgoing(p2pEndpointId: P2PEndpoint.Id): Boolean =
    p2pEndpointIdToNetworkRef.get(p2pEndpointId).exists(_._2)
  ...
  override def actOnBftNodeId(
    ...
    nodeIdToP2PEndpointId
      .get(bftNodeId)
      .fold(ifEmpty)(p2pEndpointIdToNetworkRef.get(_).map(_._3).fold(ifEmpty)(action))

  override def addNetworkRef(
      p2pEndpoint: P2PEndpoint,
      ref: P2PNetworkRef[BftOrderingMessage],
  ): Unit =
    p2pEndpointIdToNetworkRef.addOne(p2pEndpoint.id -> (p2pEndpoint, true, ref))
  ...
  override def delete(p2pEndpointId: P2PEndpoint.Id): Unit = {
    ...
    p2pEndpointIdToNetworkRef.remove(p2pEndpointId).foreach(_._3.close())
  }
}
