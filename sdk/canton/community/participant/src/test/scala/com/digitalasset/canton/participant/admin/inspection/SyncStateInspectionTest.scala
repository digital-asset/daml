...
sealed trait SyncStateInspectionTest
  ...
  "fetch empty sets if no domains exists" in {
    ...
    crossDomainReceived.toSet shouldBe Set.empty
    crossDomainComputed.toSet shouldBe Set.empty
  }
  ...
  "fetch a received commitment if it has been stored" in {
    ...
    } yield crossDomainReceived.toSet shouldBe Set(received)
  }
  ...
  "fetch a computed commitment if it has been computed" in {
    ...
    } yield crossDomainSent.toSet shouldBe Set(sent)
  }
  ...
  "fetch matched received and computed commitments with hashes" in {
    ...
    for {
      ...
      crossDomainSent.toSet shouldBe Set(sent)
      crossDomainReceived.toSet shouldBe Set(received)
    }
  }
  ...
  "fetch requested counter participant from multiple domains" in {
    ...
    for {
      ...
      crossDomainSent.toSet shouldBe Set(sent, sent2)
      crossDomainReceived.toSet shouldBe Set(received, received2)
    }
  }
  ...
  "only fetch requested states" in {
    ...
    for {
      ...
      crossDomainSentMatched.toSet shouldBe Set(sentMatched)
      crossDomainSentMismatched.toSet shouldBe Set(sentMismatched)
      crossDomainSentOutstanding.toSet shouldBe Set(sentOutstanding)
      crossDomainAll.toSet shouldBe Set(sentMatched, sentMismatched, sentOutstanding)
      crossDomainReceivedMatched.toSet shouldBe Set(receivedMatched)
      crossDomainReceivedMismatched.toSet shouldBe Set(receivedMismatched)
      crossDomainReceivedOutstanding.toSet shouldBe Set.empty
      crossDomainReceivedAll.toSet shouldBe Set(
        receivedMatched,
        receivedMismatched,
      )
    }
  ...
  "should fetch latest iteration if called with lastComputedAndSent" in {
    ...
    } yield crossDomainReceived.toSet shouldBe Set(sentMatched)
  }
  ...
  "not include duplicates with overlapping time periods" in {
    ...
    } yield crossDomainReceived.size shouldBe 1 // we cant use toSet here since it removes duplicates
  }
}
