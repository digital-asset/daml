...
class LedgerApiJdbcUrlTest extends AnyWordSpec with BaseTest {
  "fromConfig for real examples should" should {
    "support in-mem h2" in {
      ...
      result.value shouldBe "jdbc:h2:mem:db1;MODE=PostgreSQL;DB_CLOSE_DELAY=-1;user=participant1;password=pwd"
    }
    ...
    "support file backed h2" in {
      ...
      result.value shouldBe "jdbc:h2:file:./participant2;MODE=PostgreSQL;DB_CLOSE_DELAY=-1;user=participant2;password=morethansafe"
    }
    ...
    "support postgres datasource configuration" in {
      ...
      result.value shouldBe "jdbc:postgresql://localhost:5432/participant2?user=participant2&password=supersafe"
    }
    ...
    "global domain config" in {
      ...
      result.value shouldBe "jdbc:postgresql://db.canton.global:1234/?user=long-user-name&password=some-password"
    }
    ...
    "generated azure connection string" in {
      ...
      result.value shouldBe "jdbc:postgresql://my-db.postgres.database.azure.com:5432/canton?user=canton@canton-db&password=abcABC123&sslmode=require"
    }
  }
  ...
  "special characters" should {
    "be properly encoded" in {
      ...
      result.value shouldBe "jdbc:postgresql://db.canton.global:1234/?user=you%40can%25%26really%26do%21stupid%40stuff3&password=AAAA%26%23%40%21%2F%3A%3D%C3%BC%C3%B6%C3%A9%C3%A8+%2BAAAA"
    }
  }
  ...
  "schema specification" should {
    "not replace the schema if specified for h2" in {
      val result = forH2("""
      ...
      result.value shouldBe "jdbc:h2:mem:db1;DB_CLOSE_DELAY=-1;user=participant1;password=pwd;SCHEMA=another-schema;MODE=PostgreSQL"
    }

    "not replace the schema if specified for postgres" in {
      val result = forPostgres("""
          |url = "jdbc:postgresql://host/canton?user=user&currentSchema=my-schema&password=pwd&sslmode=require"
          |""".stripMargin)

      result.value shouldBe "jdbc:postgresql://host/canton?user=user&currentSchema=my-schema&password=pwd&sslmode=require"
    }
  }
