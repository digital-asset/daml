...
trait ActiveContractStoreTest extends PrunableByTimeTest {
  ...
  protected def activeContractStore(
    ...
    "creating a contract in an empty ACS" in {
      ...
      for {
        created <- acs
          .markContractCreated(coid00, TimeOfChange(rc, ts))
          .value
    ...
    "inserting is idempotent" in {
      ...
      for {
        created1 <- acs.markContractCreated(coid00, toc).value
        created2 <- acs.markContractCreated(coid00, toc).value
        fetch <- acs.fetchState(coid00)
      } yield {
    ...
    "archival must not be timestamped before creation" in {
      ...
      for {
        created <- acs.markContractCreated(coid00, toc2).value
        archived <- acs.archiveContract(coid00, toc).value
        fetch <- acs.fetchState(coid00)
        snapshot <- acs.snapshot(ts2)
      } yield {
    ...
    "several contracts can be inserted" in {
      ...
      val toc2 = TimeOfChange(rc2, ts2)
      for {
        created2 <- acs.markContractCreated(coid01, toc3).value
        created1 <- acs.markContractCreated(coid00, toc).value
        created3 <- acs.markContractCreated(coid10, toc2).value
    ...
    "double creation fails if the timestamp or request counter differs" in {
      ...
      for {
        created1 <- acs.markContractCreated(coid00, toc).value
        created2 <- acs.markContractCreated(coid00, tocTs).value
        fetch2 <- acs.fetchState(coid00)
        snapshot <- acs.snapshot(ts.plusMillis(2))
        created3 <- acs.markContractCreated(coid00, tocRc).value
    ...
    "bulk create with empty set" in {
      ...
      for {
        created <- acs
          .markContractsCreated(Seq.empty[LfContractId], TimeOfChange(rc, ts))
          .value
    ...
    "add" should {
      "be idempotent" in {
        ...
        for {
          add1 <- acs.markContractAdded(coid00, toc0).value
          add2 <- acs.markContractAdded(coid00, toc0).value
        } yield {
      ...
      "be rejected on active contract" in {
        ...
        for {
          added <- acs.markContractsAdded(Seq(coid00), toc0).value
          created <- acs.markContractCreated(coid01, toc0).value
          transferredIn <- acs.transferInContract(coid02, toc0, sourceDomain1).value

          addAdd <- acs.markContractAdded(coid00, toc0).value
          createAdd <- acs.markContractAdded(coid01, toc1).value
          tfInAdd <- acs.markContractAdded(coid02, toc1).value
        } yield {
    ...
    "purge" should {
      "be idempotent" in {
        ...
        val toc1 = TimeOfChange(rc + 1, ts.plusSeconds(1))
        for {
          create <- acs.markContractCreated(coid00, toc0).value
    ...
    "purge/add" should {
      "add should be allowed after a purge" in {
        ...
        for {
          creates <- acs.markContractsCreated(Seq(coid00, coid01, coid02), toc0).value

          archive <- acs.archiveContracts(Seq(coid00), toc1).value
          purge <- acs.purgeContracts(Seq(coid01, coid02), toc1).value

          addAfterArchive <- acs.markContractAdded(coid00, toc2).value
          addAfterPurge <- acs.markContractAdded(coid01, toc2).value
          createAfterPurge <- acs.markContractCreated(coid02, toc2).value
        } yield {
      ...
      "add and purge can be used repeatedly" in {
        ...
        for {
          creates <- acs.markContractCreated(coid00, tocCreate).value
          ...
          purgeAddResults <- MonadUtil.sequentialTraverse(0L until cyclesCount) { i =>
            ...
            for {
              purge <- acs
                ...
                .value
              add <- acs
                .markContractAdded(coid00, TimeOfChange(rc + shift + 1, ts.plusSeconds(shift + 1)))
                .value
    ...
    "contracts can be transferred multiple times" in {
      ...
      for {
        create <- acs.markContractCreated(coid00, toc1).value
        ...
        assert(
          in2.isResult && in2.nonaborts == Chain(
            SimultaneousActivation(
              coid00,
              toc1,
              TransferIn(domain1Idx),
              TransferIn(domain2Idx),
            )
          ),
        ...
        assert(
          out2.isResult && out2.nonaborts == Chain(
            SimultaneousDeactivation(
              coid00,
              toc1,
              TransferOut(domain1Idx),
              TransferOut(domain2Idx),
            )
          ),
    ...
    "complain about simultaneous creations and transfer-ins" in {
      ...
      val toc = TimeOfChange(rc, ts)
      for {
        create <- acs.markContractCreated(coid00, toc).value
        in <- acs.transferInContract(coid00, toc, sourceDomain1).value
        fetch <- acs.fetchState(coid00)
      } yield {
    ...
    "complain about changes before creation" in {
      ...
      val toc4 = TimeOfChange(rc + 2, ts.plusSeconds(3))
      for {
        create <- acs.markContractCreated(coid00, toc3).value
        in1 <- acs.transferInContract(coid00, toc1, sourceDomain1).value
        fetch3 <- acs.fetchState(coid00)
    ...
    "return a snapshot sorted in the contract ID order" in {
      ...
      for {
        _ <- acs.markContractCreated(coid00, toc1).value
        _ <- acs
          .markContractsCreated(
            Seq(coid10, coid11),
            toc,
          ...
          .value
        _ <- acs.markContractCreated(coid01, toc2).value
        snapshot <- acs.snapshot(ts2)
      } yield {
    ...
    "deleting from request counter" in {
      ...
      for {
        _ <- valueOrFail(acs.markContractCreated(coid00, toc1))(
          s"create $coid00"
        )
    ...
    "contract snapshot" in {
      ...
      for {
        ...
        _ <- valueOrFail(acs.markContractsCreated(Seq(coid10), toc2))(
          ...
          (
            ...
            ActiveContractIdsChange(
              activations = Map(
                ...
                coid11 -> StateChangeType(ContractChange.TransferIn),
              ),
          ...
          (
            ...
            ActiveContractIdsChange(
              ...
              deactivations = Map(
                ...
                coid11 -> StateChangeType(ContractChange.TransferOut),
              ),
            ),
          ),
          (
            toc4,
            ActiveContractIdsChange(
              activations = Map(coid11 -> StateChangeType(ContractChange.TransferIn)),
    ...
    "look up active contracts for a package" when {
      ...
      def activateMaybeDeactivate(
          activate: ActiveContractStore => CheckedT[Future, AcsError, AcsWarning, Unit] = { acs =>
            acs.markContractCreated(coid00, toc1)
          },
