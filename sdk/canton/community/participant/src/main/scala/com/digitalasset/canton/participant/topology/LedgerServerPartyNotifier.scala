...
import com.digitalasset.canton.util.{FutureUtil, MonadUtil, SimpleExecutionQueue}
...
class LedgerServerPartyNotifier(
    ...
    store.fetchNotNotified().map { unnotified =>
      ...
      val partiesMetadataO =
        NonEmpty
          .from(
            unnotified
              .collect { case withParticipant @ PartyMetadata(_, _, Some(_)) => withParticipant }
  ...
  ): Seq[(PartyId, Option[ParticipantId], String255, Option[DisplayName])] =
      ...
      .execute(
        {
          ...
          updateAndNotify(
            Seq((partyId, None, LengthLimitedString.getUuid.asString255, Some(displayName))),
  ...
  private def updateAndNotify(
      parties: Seq[(PartyId, Option[ParticipantId], String255, Option[DisplayName])],
      sequencerTimestamp: SequencedTime,
      effectiveTimestamp: EffectiveTime,
  )(implicit traceContext: TraceContext): Future[Unit] =
    for {
      storedParties <- store.metadataForParties(parties.map { case (partyId, _, _, _) => partyId })
      ...
      partiesByEffectiveTimeO = NonEmpty
        .from(
          partiesAndStored
            .flatMap {
              case ((partyId, targetParticipantIdO, submissionId, displayNameO), None) =>
                Some(
                  PartyMetadata(partyId, displayNameO, targetParticipantIdO)(
                    effectiveTimestamp.value,
                    submissionId,
                  )
                ): Option[PartyMetadata]
              case (
                    (partyId, targetParticipantIdO, submissionId, displayNameO),
                    Some(stored),
                  ) =>
                ...
                val update =
                  PartyMetadata(
                    partyId = partyId,
                    displayName = displayNameO.orElse(stored.displayName),
  ...
  private def sendNotifications(
    ...
    val updates = partyMetadataSeq.forgetNE.flatMap {
      case metadata @ PartyMetadata(_, _, Some(hostingParticipant)) =>
      ...
      case metadata @ PartyMetadata(_, _, None) =>
        logger.debug(
