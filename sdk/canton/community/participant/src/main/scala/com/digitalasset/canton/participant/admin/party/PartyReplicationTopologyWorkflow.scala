...
class PartyReplicationTopologyWorkflow(
  ...
  private[party] def authorizeOnboardingTopology(
    ...
    require(
      synchronizerId == topologyManager.synchronizerId.logical,
      s"party replication synchronizer id $synchronizerId does not match topology manager synchronizer id ${topologyManager.synchronizerId.logical}",
    )
    require(
      synchronizerId == topologyStore.storeId.synchronizerId.logical,
      s"party replication synchronizer id $synchronizerId does not match topology store synchronizer id ${topologyStore.storeId.synchronizerId.logical}",
    )
    for {
      _ <- EitherT(
