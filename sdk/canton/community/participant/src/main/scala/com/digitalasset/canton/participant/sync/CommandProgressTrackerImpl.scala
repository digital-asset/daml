...
import com.daml.ledger.api.v1.admin.command_inspection_service.GetCommandStatusResponse.CommandStatus.{
  CommandUpdates,
  RequestStatistics,
}
...
class CommandProgressTrackerImpl(
  ...
  private case class MyCommandResultHandle(key: CommandKey, initial: CommandStatus)
    ...
    private def processSyncErr(err: com.google.rpc.status.Status): Unit = {
      ...
      lock.synchronized(pending.remove(key)).foreach { cur =>
        if (config.maxFailed.value > 0) {
          updateWithStatus(err, CommandState.STATE_FAILED)
    ...
    def failedAsync(
      ...
      updateWithStatus(
        ...
        CommandState.STATE_FAILED,
      )

    def succeeded(): Unit =
      updateWithStatus(CompletionFromTransaction.OkStatus, CommandState.STATE_SUCCEEDED)
  ...
  override def findCommandStatus(
    ...
    val pool = state match {
      case CommandState.STATE_UNSPECIFIED | CommandState.Unrecognized(_) =>
      ...
      case CommandState.STATE_FAILED => findCommands(commandIdPrefix, limit, failed)
      case CommandState.STATE_PENDING =>
        findCommands(commandIdPrefix, limit, pending.values.map(_.ref.get()))
      case CommandState.STATE_SUCCEEDED => findCommands(commandIdPrefix, limit, succeeded)
    }
    pool.take(limit)
  }
  ...
  override def registerCommand(
    ...
    val status = CommandStatus(
      started = clock.now,
      completed = None,
      completion = ApiConversions.toV1(
        CompletionFromTransaction.toApiCompletion(
          commandId = commandId,
          transactionId = "",
      ...
      state = CommandState.STATE_PENDING,
      commands = commands,
      requestStatistics = RequestStatistics(),
      updates = CommandUpdates(),
    )
