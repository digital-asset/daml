...
// TODO(i12338): Make this an sbt app
...
object Main {
  ...
  final case class CodeDetail(code: String, detail: String) extends ExitCode

  val OK: ExitCode = CodeDetail("OK", "The digest is as expected")

  final case class ErrUsage(app: String) extends Code("ERR_USAGE") {
    def detail = s"Usage: $app <directory> <report>"
  }

  final case class NotDirectory(dir: String) extends Code("ERR_NOT_DIRECTORY") {
    def detail = s"The passed directory is not a valid directory: $dir"
  }

  final case class NotKeyFile(path: Path, t: Throwable) extends Code("ERR_NOT_KEY_FILE") {
    def detail = s"Unable to parse metering report key from key file: $path [${t.getMessage}]"
  }

  final case class NoKeys(dir: String) extends Code("ERR_NO_KEYS") {
    def detail = s"No keys found in the key directory: $dir"
  }

  final case class NotFile(report: String) extends Code("ERR_NO_REPORT") {
    def detail = s"The passed report is not a file: $report"
  }

  final case class FailedToReadReport(report: String, t: Throwable)
      extends Code("ERR_READING_REPORT") {
    def detail = s"Failed to read the participant report: $report [${t.getMessage}]"
  }

  final case class FailedVerification(status: VerificationStatus)
