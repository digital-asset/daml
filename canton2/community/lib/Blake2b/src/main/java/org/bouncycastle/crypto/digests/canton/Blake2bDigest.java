...
/**
 * This is a modified version of BouncyCastle's implementation of the cryptographic hash function
 * Blakbe2b. It's changed to support additional parameters that are required to implement Blake2xb
 * on top of Blake2x. Conversely, it disables other features (keying mechanism, personalization) not
 * needed for Blake2xb
 */
public class Blake2bDigest implements ExtendedDigest {
  ...
  // Tree hashing parameters. While the tree mode is not supported, these are still used to
  // implement the
  ...
  public static byte[] digest(
    ...
    Blake2bDigest d =
        new Blake2bDigest(
            digestBytes, fanout, depth, leafLength, nodeOffset, nodeDepth, innerHashLength);
  ...
  private void G(long m1, long m2, int posA, int posB, int posC, int posD) {
    ...
    internalState[posB] =
        Longs.rotateRight(internalState[posB] ^ internalState[posC], 24); // replaces 25 of BLAKE
    internalState[posA] = internalState[posA] + internalState[posB] + m2;
    internalState[posD] = Longs.rotateRight(internalState[posD] ^ internalState[posA], 16);
    internalState[posC] = internalState[posC] + internalState[posD];
    internalState[posB] =
        Longs.rotateRight(internalState[posB] ^ internalState[posC], 63); // replaces 11 of BLAKE
  }
