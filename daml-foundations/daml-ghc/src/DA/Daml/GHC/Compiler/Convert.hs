...
convertRational num denom
 =
    ...
    if | 10 ^ maxPrecision `mod` denom == 0 &&
             lowerBound128Bit < (r * 10 ^ (10 :: Integer)) &&
             r * 10 ^ maxPrecision <= upperBound128Bit ->
           pure $ EBuiltin $ BEDecimal $ fromRational r
       | otherwise ->
           unsupported
               ("Rational precision is out of bounds for " ++
                show ((fromInteger num / fromInteger denom) :: Double) ++
                ".  Maximal supported precision is e^-10, maximal numbers of digits and sign is is 38")
               (num, denom)
  where
    r = num % denom
    upperBound128Bit = 10 ^ (38 :: Integer)
    lowerBound128Bit = -upperBound128Bit + 1
