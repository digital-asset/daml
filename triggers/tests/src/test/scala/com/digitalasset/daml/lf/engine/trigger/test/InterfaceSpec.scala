    ...
    def transactionIdFor(templateId: Identifier): String = {
      events.collect {
        case (
              ...
              Seq(
                Event(
                  Created(CreatedEvent(_, _, Some(`templateId`), _, _, _, _, _, _, _, _, _, _, _))
                )
              ),
            ) =>
          transactionId
    ...
    def shouldHaveNoCreateArgumentsFor(templateId: Identifier): Assertion =
      inside(events) {
        case Seq(
              Event(
                Created(
                  CreatedEvent(_, _, Some(`templateId`), _, None, _, _, Seq(_), _, _, _, _, _, _)
                )
              )
            ) =>
          succeed
    ...
    def shouldHaveCreateArgumentsFor(templateId: Identifier): Assertion =
      inside(events) {
        case Seq(
              Event(
                Created(
                  CreatedEvent(_, _, Some(`templateId`), _, Some(_), _, _, Seq(_), _, _, _, _, _, _)
                )
              )
            ) =>
          succeed
    ...
    def shouldHaveViewValues(n: Int, label: String): Assertion =
      inside(events) {
        case Seq(
              Event(
                Created(
                  CreatedEvent(_, _, _, _, _, _, _, Seq(view), _, _, _, _, _, _)
                )
              )
            ) =>
