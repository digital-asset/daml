...
type instance RuleResult GetModificationTime = IdeReturn UTCTime

-- | Get the contents of a file, either dirty (if the buffer is modified) or from disk.
type instance RuleResult GetFileContents = IdeReturn (UTCTime, StringBuffer)

-- | Does the file exist.
type instance RuleResult GetFileExists = IdeReturn Bool
