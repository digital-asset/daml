...
module DA.Daml.LF.Proto3.DecodeVDev
    ( decodePackage
    , Error(..)
    ) where
...
import           DA.Daml.LF.Proto3.VersionVDev
import           DA.Prelude
import           Data.List (foldl)
import           DA.Daml.LF.Mangling
import qualified Da.DamlLfDev as LF1
...
decodeVersion (TL.toStrict -> minor)
  | minor == hashDamlLfDevProto = pure (VDev minor)
  | otherwise = Left (UnsupportedMinorVersion minor)
