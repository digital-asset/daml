...
toCompileOpts options@Options{..} reportProgress =
  ...
  where
    ...
    isGenTemplMonoLocal msg
        | Just (_, rest) <- stripInfix ["The", "constraint", "`DA.Internal.Template.Template"] (T.words msg)
        , Just (_, rest) <- stripInfix ["matches"] rest
        , Just (_, _) <- stripInfix ["use", "MonoLocalBinds,", "or", "simplify", "it", "using", "the", "instance"] rest
        = True
        | otherwise
        = False
