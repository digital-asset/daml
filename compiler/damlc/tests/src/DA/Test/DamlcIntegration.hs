...
testCase version getService outdir registerTODO (name, file, anns) = singleTest name . TestCase $ \log -> do
  ...
  if any (`notElem` supportedOutputVersions) [v | UntilLF v <- anns] then
    ...
    then pure $ Result
      ...
      resQueries <- runJqQuery log outdir file [(q, isStream) | QueryLF q isStream <- anns]
...
runJqQuery :: (String -> IO ()) -> FilePath -> FilePath -> [(String, Bool)] -> IO [Maybe String]
runJqQuery log outdir file qs = do
  ...
  forM qs $ \(q, isStream) -> do
    ...
    out <- readProcess jq (streamFlags <> ["-L", queryLfDir, fullQuery, outdir </> proj <.> "json"]) ""
    case trim out of
      "true" -> pure Nothing
