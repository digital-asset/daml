...
lfRunScripts log file = timed log "LF scripts execution" $ do
  ...
  where
    lvl =
      -- This level of detail ensures we don't print out package ids, since
      -- those change very easily, requiring an updated golden file each time
      PrettyLevel (-1)
    format world
      = renderPlain
      . ($$ string "") -- ensure there's an empty line at the end to appease git
      . \case
          Right res ->
