...
abstract class HttpServiceWithPostgresIntTest
  ...
  "query persists all active contracts" in withHttpService { fixture =>
    fixture.getUniquePartyAndAuthHeaders("Alice").flatMap { case (party, headers) =>
      ...
      searchExpectOk(
        ...
        fixture,
        headers,
      ).flatMap { searchResult: List[domain.ActiveContract.ResolvedCtTyId[JsValue]] =>
  ...
  private def getField(k: String)(a: domain.ActiveContract.ResolvedCtTyId[JsValue]): JsValue =
