...
object FutureUtil {
  ...
  final class LiftET[E](private val ignore: Int) extends AnyVal {
    def apply[F[_]: Functor, A](fa: F[A]): EitherT[E, F, A] = EitherT.rightT(fa)
  }

  def eitherT[A, B](fa: Future[A \/ B]): EitherT[A, Future, B] =
    EitherT.eitherT(fa)

  def either[A, B](d: A \/ B)(implicit ev: Applicative[Future]): EitherT[A, Future, B] =
    EitherT.either(d)

  def rightT[A, B](fa: Future[B])(implicit ev: Functor[Future]): EitherT[A, Future, B] =
    EitherT.rightT(fa)

  def leftT[A, B](fa: Future[A])(implicit ev: Functor[Future]): EitherT[A, Future, B] =
    EitherT.leftT(fa)
