  ...
  "fromJsObject" should {
    ...
    val sqlWheres = {
      ...
      Table(
        ...
        ("""{"%lte": 42}""", VA.int64, sql"create_arguments->${"foo": String} <= ${JsNumber(42): JsValue}::jsonb AND create_arguments @> ${JsObject(): JsValue}::jsonb"),
      )
    }

    "compile to SQL" in forEvery(sqlWheres) { (query, va, sql: doobie.Fragment) =>
      val defs = typeInObject(va.t)
      val vp = ValuePredicate.fromJsObject(Map((dummyFieldName: String) -> query.parseJson), dummyTypeCon, defs)
      val frag = vp.toSqlWhereClause
      frag.toString should ===(sql.toString)
      import language.reflectiveCalls
      flattenFragmentExistential(frag.asInstanceOf[{def a: AnyRef}].a) should ===(flattenFragmentExistential(sql.asInstanceOf[{def a: AnyRef}].a))
    }
  }
}
