...
private final class PostgresQueries(tablePrefix: String, tpIdCacheMaxEntries: Long)(implicit
  ...
  private[http] def partiesConnectedByContract(
    ...
    import ipol.{gas, pas}, com.daml.nonempty.catsinstances._, cats.instances.vector._
    val tpidsFilter = Fragments.in(fr"tpid", tpids.toVector.toNEF)
    reachableParties(parties intersect allStoredForTpids) { (searchFrom, alreadySeen) =>
      ...
      sql"""SELECT signatories || observers
            FROM $contractTableName AS c
            WHERE (signatories && $partyVector::text[] OR observers && $partyVector::text[])
                  AND (NOT (${seen.toVector}::text[] @> (signatories || observers)))
                  AND $tpidsFilter
         """
        .query[Vector[String]]
      ...
      case NonEmpty(p) => connection pure p // TODO SC #13529 move to reachableParties
