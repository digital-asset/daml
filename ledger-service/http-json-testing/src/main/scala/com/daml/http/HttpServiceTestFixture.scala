...
object HttpServiceTestFixture extends LazyLogging with Assertions with Inside {
  ...
  def withLedger[A](
    ...
    val clientF = portsF.map(ports =>
      config.ledgerClientWithoutId(ports.head._1, None, ApplicationId("http-service-test"))
    )

    val fa: Future[A] = for {
      ports <- portsF
      client <- clientF
      a <- testFn(ports.head._1, client, LedgerId(config.ledgerIds.headOption.value))
    } yield a
