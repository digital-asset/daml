...
daml ledger upload-dar Test-stable.dar --host localhost --port 6862
daml ledger upload-dar Test-dev.dar --host localhost --port 6863

cd $DAML_REPO_LOCATION
for i in {1..2}
do
	echo "Running Iteration $i of Semantic Tests"
	bazel run -- //ledger/ledger-api-test-tool localhost:6861 localhost:6862 localhost:6863 localhost:6864 --command-submission-ttl-scale-factor 3.5 --timeout-scale-factor 3.5 -v
done
