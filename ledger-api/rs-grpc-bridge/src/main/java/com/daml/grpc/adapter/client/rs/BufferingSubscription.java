...
/**
 * Used in the ClientPublisher to handle downstream signals by either buffering them or forwarding them to an appropriate
...
class BufferingSubscription<Resp> implements Subscription {
    ...
    /**
     * gRPC upstream will send an element that the subscriber did not ask for.
     * We keep track of that, and any new demand that was signaled to the transport layer in this variable.
     * It gets decremented with each incoming element, and once it reaches zero, part of the buffered downstream demand
     * (capped at Integer.MAX_VALUE) will be flushed upstream, and added to this counter.
     */
    @Nonnegative
     ...
     *                                Implementation should flush any buffered elements and completion
     *                                signals to the subscriber.
     * @param requestObserverSupplier Should return the ClientCallStreamObserver if the request was already started,
     *                                or null otherwise.
     * @param es                      Will be used to sequence all callbacks that are passed and all operations
 *                                that would change mutable state.
     * @param subscriber              The subscriber this subscription was created for.
     * @param logPrefix
     */
    BufferingSubscription(@Nonnull Consumer<Subscriber<? super Resp>> onFirstDemand,
        ...
        downstreamEventBuffer = new DownstreamEventBuffer(this::propagateCancellation, this::propagateIntegerDemand, logPrefix);
    }
    ...
    public void request(long demand) {
        ...
        if (demand < 1) {
            terminateStream(String.format("Subscriber signaled non-positive subscription request (%d)", demand));
        } else {
            es.sequence(() -> {
                if (!cancelled) {
                    ...
                    logger.trace("{}Swallowing demand from gRPC upstream due to previous cancellation.", logPrefix);
                }
            });
        }
    }
    ...
    private void propagateDemandIfNoElemsPending() {
        ...
        if (clientCallStreamObserver != null && callStarted && unsatisfiedDemand == 0) {
            this.unsatisfiedDemand = downstreamEventBuffer.propagateCancellationOrDemand(clientCallStreamObserver);
        }
    }
    ...
    /**
     * Marks the call as started, and if there's no unsatisfied demand signaled to the upstream,
     * it sends the buffered amount (or the cancellation if this is the first call to this method.)
     *
