...
class BufferingResponseObserver<Req, Resp> implements ClientResponseObserver<Req, Resp> {
    ...
    @Nonnull
    private final UpstreamEventBuffer<Resp> buffer = new UpstreamEventBuffer<>(logPrefix);
    ...
    BufferingResponseObserver(@Nonnull Subscriber<? super Resp> subscriber, @Nonnull ExecutionSequencer es) {
        this.es = es;
        this.subscription = new BufferingSubscription<Resp>(buffer::flushOnFirstRequest, () -> requestObserver, this.es, subscriber, logPrefix);
    ...
    public void beforeStart(ClientCallStreamObserver<Req> ccso) {
        // IMPORTANT: methods of the ClientCallStreamObserver are not safe to call here or after this method returns.
        // We need to wait for the first upstream signal (onNext/onComplete/onError) to know for sure that the call
        // has already been started. Failing to do so may result in IllegalStateExceptions thrown by the CCSO.
