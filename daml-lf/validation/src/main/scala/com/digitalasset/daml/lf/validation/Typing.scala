...
private[validation] object Typing {
  ...
  private def typeOfPrimLit(lit: PrimLit): Type = lit match {
    ...
    case PLNumeric(x) => TNumeric(TNat(x.scale))
    case PLText(_) => TText
  ...
  protected[validation] lazy val typeOfBuiltinFunction = {
    ...
    val tNumericBinop: Type = TForall(alpha.name -> KNat, tBinop(TNumeric(alpha)))
    val tNumericComparison: Type =
      TForall(alpha.name -> KNat, TNumeric(alpha) ->: TNumeric(alpha) ->: TBool)

    Map[BuiltinFunction, Type](
      BTrace -> TForall(alpha.name -> KStar, TText ->: alpha ->: alpha),
      // Numeric arithmetic
      BAddNumeric -> tNumericBinop,
      BSubNumeric -> tNumericBinop,
      BMulNumeric -> tNumericBinop,
      BDivNumeric -> tNumericBinop,
      ...
      BLessNumeric -> tNumericComparison,
      ...
      BLessEqNumeric -> tNumericComparison,
      ...
      BGreaterNumeric -> tNumericComparison,
      ...
      BGreaterEqNumeric -> tNumericComparison,
      ...
      BEqualNumeric -> tNumericComparison,
      ...
      case TApp(tFun, tArg) =>
        ...
        k match {
          case KArrow(argKind, resKind) =>
            ...
            resKind
          case KStar | KNat => throw EExpectedHigherKind(ctx, k)
        }
      case TBuiltin(bType) =>
        kindOfBuiltin(bType)
      case TForall((v, k), b) =>
