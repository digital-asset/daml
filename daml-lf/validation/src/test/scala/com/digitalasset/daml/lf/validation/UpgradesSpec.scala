...
class UpgradesSpec extends AsyncWordSpec with Matchers with Inside with CantonFixture {
  ...
  "Upload-time Upgradeability Checks" should {
    "uploading the same package multiple times succeeds" ignore {
      testPackagePair(
    ...
    "uploads against the same package name must be version unique" ignore {
      testPackagePair(
    ...
    "Succeeds when v1 upgrades to v2 and then v3" ignore {
      testPackagePair(
    ...
    "Succeeds when v1 upgrades to v3 and then v2" ignore {
      testPackagePair(
    ...
    "Fails when v1 upgrades to v2, but v3 does not upgrade v2" ignore {
      testPackagePair(
    ...
    "Fails when v1 upgrades to v3, but v3 does not upgrade v2" ignore {
      testPackagePair(
