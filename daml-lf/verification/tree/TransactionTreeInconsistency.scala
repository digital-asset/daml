...
object TransactionTreeInconsistency {
    ...
    if(i == 2 * tr.size - 1){
      ...
      p._2 match {
        ...
        case r: Node.Rollback =>
          ...
          unfold(sameStack(beginRollback(scanTransaction(tr, init)(j)._1), traverseTransaction(sub, beginRollback(scanTransaction(tr, init)(j)._1))))
          ...
          unfold(sameStack(scanTransaction(tr, init)(j)._1.get.beginRollback(), traverseTransaction(sub, beginRollback(scanTransaction(tr, init)(j)._1))))
  ...
  @opaque
  @pure
  def activeKeysGetTraverseTransaction(tr: Tree[(NodeId, Node)], s: Either[KeyInputError, State], k: GlobalKey): Unit = {
    ...
    unfold(propagatesError(traverseTransaction(tr, beginRollback(s)), endRollback(traverseTransaction(tr, beginRollback(s)))))

    sameGlobalKeysTransitivity(s, beginRollback(s), traverseTransaction(tr, beginRollback(s)))
    sameGlobalKeysTransitivity(s, traverseTransaction(tr, beginRollback(s)), endRollback(traverseTransaction(tr, beginRollback(s))))
   ...
   */
  @pure @opaque
  def doesNotAppearBeforeSameActiveKeysGet(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], k: GlobalKey,
                                           i: BigInt, j: BigInt): Unit = {
    decreases(j)
    require(0 <= i)
    require(i <= j)
    require(j < 2 * tr.size)
    ...
    require(stateInvariant(init)(traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1))
      ...
      if(dir == TraversalDirection.Down){
        ...
        n._2 match {
          case a: Node.Action =>
            ...
            handleNodeActiveKeysGet(sj.get, n._1, a, k, traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
          case r: Node.Rollback =>
   ...
   */
  @pure @opaque
  def firstAppearsSameActiveKeysGet(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], k: GlobalKey,
                                           i: BigInt, j: BigInt): Unit = {
    decreases(j)
    require(0 <= i)
    require(i <= j)
   ...
   */
  @pure @opaque
  def firstAppearsHandleNodeUndefined(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], n: Node.Action,
                                    i: BigInt): Unit = {
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    unfold(inconsistencyCheck(scanTransaction(tr, init)(i)._1.get, n.gkeyOpt, nodeActionKeyMapping(n)))
   ...
   */
  @pure @opaque
  def appearsBeforeSameActiveKeysGet(tr: Tree[(NodeId, Node)], init1: Either[KeyInputError, State], init2: Either[KeyInputError, State],
                                           k: GlobalKey, i: BigInt): Unit = {
    decreases(i)
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    require(stateInvariant(init1)(traverseUnbound(tr)._1, traverseLC(tr, init1.get.locallyCreated, init1.get.consumed, true)._1))
    require(traverseLC(tr, init2.get.locallyCreated, init2.get.consumed, true)._3)
    require(stateInvariant(init2)(traverseUnbound(tr)._1, traverseLC(tr, init2.get.locallyCreated, init2.get.consumed, true)._1))
    ...
    if (i == 0) {
      ...
      scanIndexingNode(tr, init1, init2, traverseInFun, traverseOutFun, traverseInFun, traverseOutFun, i - 1)
      ...
      if (dir == TraversalDirection.Down) {
        ...
        n._2 match {
          case a: Node.Action =>
            ...
            //required
            assert(appearsAtIndex(tr, init1, traverseInFun, traverseOutFun, k, i - 1) == (a.gkeyOpt == Some(k)))
            ...
            if(a.gkeyOpt == Some(k)){
              handleNodeDifferentStatesActiveKeysGet(s1.get, s2.get, n._1, a,
                traverseUnbound(tr)._1, traverseLC(tr, init1.get.locallyCreated, init1.get.consumed, true)._1,
                traverseUnbound(tr)._1, traverseLC(tr, init2.get.locallyCreated, init2.get.consumed, true)._1)
            }
            else {
              handleNodeActiveKeysGet(s1.get, n._1, a, k, traverseUnbound(tr)._1,
                traverseLC(tr, init1.get.locallyCreated, init1.get.consumed, true)._1)
              handleNodeActiveKeysGet(s2.get, n._1, a, k, traverseUnbound(tr)._1,
          ...
          case r: Node.Rollback =>
            ...
            if(!doesNotAppearBefore(tr, init1, traverseInFun, traverseOutFun, k, i2)){
              ...
              def appearsBeforeSameActiveKeysGetSameFirstAppears: Unit = {
                ...
                scanIndexingNode(tr, init1, init2, traverseInFun, traverseOutFun, traverseInFun, traverseOutFun, i1)

                scanTransaction(tr, init1)(i1)._2._2 match {
                  case a: Node.Action =>
                    //required
                    assert(scanTransaction(tr, init1)(i1)._2._2 == scanTransaction(tr, init2)(i1)._2._2)
                    ...
                    def appearsBeforeSameActiveKeysGetSameFirstAppearsContainsKey: Unit = {
                      ...
                      unfold(traverseInFun(scanTransaction(tr, init1)(i1)._1, scanTransaction(tr, init1)(i1)._2))
                      unfold(traverseInFun(scanTransaction(tr, init2)(i1)._1, scanTransaction(tr, init2)(i1)._2))
                      unfold(handleNode(scanTransaction(tr, init1)(i1)._1, scanTransaction(tr, init1)(i1)._2._1, a))
                      unfold(handleNode(scanTransaction(tr, init2)(i1)._1, scanTransaction(tr, init2)(i1)._2._1, a))
                      handleSameNodeActiveKeys(scanTransaction(tr, init1)(i1)._1.get, scanTransaction(tr, init2)(i1)._1.get,
                        scanTransaction(tr, init1)(i1)._2._1, a)
              ...
              firstAppearsSame(tr, init1, init2, traverseInFun, traverseOutFun, traverseInFun, traverseOutFun,
                k, i1)
   ...
   */
  @pure @opaque
  def appearsBeforeSameUndefined(tr: Tree[(NodeId, Node)], init1: Either[KeyInputError, State], init2: Either[KeyInputError, State],
                                 n: Node.Action, i: BigInt): Unit = {
    decreases(i)
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    require(n.gkeyOpt.forall(k => !doesNotAppearBefore(tr, init1, traverseInFun, traverseOutFun, k, i)))
    ...
    scanIndexingNode(tr, init1, init2, traverseInFun, traverseOutFun, traverseInFun, traverseOutFun, i)
    ...
    scanTransaction(tr, init1)(i)._1.get.handleNode(scanTransaction(tr, init1)(i)._2._1, n).isLeft ==
  ...
  @pure
  @opaque
  def firstAppearsHandleNodeUndefinedEmpty(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], n: Node.Action,
                                      i: BigInt): Unit = {
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    scanIndexingNode(tr, init, Map.empty[GlobalKey, KeyMapping], traverseInFun, traverseOutFun, collectFun, (z, t) => z, i)
    firstAppearsSame(tr, init, Map.empty[GlobalKey, KeyMapping],
      traverseInFun, traverseOutFun, collectFun, (z, t) => z, n.gkeyOpt.get, i)
    collectGet(tr, i, n)
  ...
  @pure
  @opaque
  def appearsBeforeHandleNodeUndefinedEmpty(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], n: Node.Action,
                                           i: BigInt): Unit = {
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    scanIndexingNode(tr, init, rempty, traverseInFun, traverseOutFun, traverseInFun, traverseOutFun, i)
    ...
    if(i == 2 * tr.size - 1){
      ...
      traverseLCSubsetOf(tr, Set.empty[ContractId], Set.empty[ContractId], true,
        init.get.locallyCreated, init.get.consumed, true)
      ...
      unfold(propagatesError(scanTransaction(tr, rempty)(i + 1)._1, traverseTransaction(tr, rempty)))
...
//      assert(scanTransaction(tr, init)(i + 1)._1.isRight)
  ...
  @pure
  @opaque
  def traverseTransactionEmptyDefined(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    val p: GlobalKey => Boolean = k => init.get.activeKeys.get(k) == emptyState(tr).globalKeys.get(k)

    if(emptyState(tr).globalKeys.keySet.forall(p) &&
      !traverseTransaction(tr, init).isRight){
    ...
    else if(!emptyState(tr).globalKeys.keySet.forall(p) &&
      traverseTransaction(tr, init).isRight){
      ...
      val (si, n, dir) = scanTransaction(tr, init)(i)
      firstAppearsSame(tr, Map.empty[GlobalKey, KeyMapping], init, collectFun, (z, t) => z, traverseInFun, traverseOutFun, k, i)
      unfold(firstAppears(tr, init, traverseInFun, traverseOutFun, k, i))
      unfold(appearsAtIndex(tr, init, traverseInFun, traverseOutFun, k, i))
    ...
    emptyState(tr).globalKeys.keySet.forall(k => init.get.activeKeys.get(k) == emptyState(tr).globalKeys.get(k))
      ==
    traverseTransaction(tr, init).isRight
  )
