...
object TransactionTreeDef {
   ...
   */
  @pure @opaque
  def traverseInFun(s: Either[KeyInputError, State], p: (NodeId, Node)): Either[KeyInputError, State] = {
    p._2 match {
   ...
   */
  @pure @opaque
  def traverseOutFun(s: Either[KeyInputError, State], p: (NodeId, Node)): Either[KeyInputError, State] = {
   ...
   */
  @pure
  def scanTransaction(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): List[(Either[KeyInputError, State], (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def traverseTransaction(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Either[KeyInputError, State] = {
...
object TransactionTree {
  ...
  @pure
  @opaque
  def scanTransactionProp(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt, j: BigInt): Unit = {
    decreases(j)
    require(0 <= i)
    require(i <= j)
    require(j < 2 * tr.size)
  ...
  @pure
  @opaque
  def scanTransactionProp(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(0 <= i)
    require(i < 2 * tr.size)
  ...
  @pure
  @opaque
  def traverseTransactionProp(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    decreases(tr)
  ...
  @pure
  @opaque
  def traverseTransactionDefined(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(0 <= i)
    require(i < 2 * tr.size)
    ...
    propagatesErrorTransitivity(scanTransaction(tr, init)(i)._1, scanTransaction(tr, init)(2 * tr.size - 1)._1, traverseTransaction(tr, init))
  }.ensuring(
  ...
  @pure
  @opaque
  def findBeginRollback(tr: Tree[(NodeId, Node)], init1: Either[KeyInputError, State], init2: Either[KeyInputError, State], i: BigInt): (BigInt, Tree[(NodeId, Node)]) = {
    decreases(tr)
    require(tr.isUnique)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    tr match {
      ...
      case ContentNode(c, str) =>
        ...
        if (c == scanTransaction(tr, init1)(i)._2) {
          ...
          val (j, sub) = findBeginRollback(str, traverseInFun(init1, c), traverseInFun(init2, c), i - 1)
      ...
      case ArticulationNode(l, r) =>
        ...
        if (i < 2 * l.size) {
          ...
          assert(scanTransaction(tr, init1)(i)._1 == traverseTransaction(sub, beginRollback(scanTransaction(tr, init1)(j)._1)))
          assert(scanTransaction(tr, init2)(i)._1 == traverseTransaction(sub, beginRollback(scanTransaction(tr, init2)(j)._1)))

          (j, sub)
        }
        else {
          val (j, sub) = findBeginRollback(r, traverseTransaction(l, init1), traverseTransaction(l, init2), i - 2 * l.size)
    ...
    (j, sub) =>
      0 <= j && j < i && sub.size < tr.size &&
        ...
        (scanTransaction(tr, init1)(i)._1 == traverseTransaction(sub, beginRollback(scanTransaction(tr, init1)(j)._1))) &&
        (scanTransaction(tr, init2)(i)._1 == traverseTransaction(sub, beginRollback(scanTransaction(tr, init2)(j)._1)))
  )
  ...
  @pure
  @opaque
  def findBeginRollback(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): (BigInt, Tree[(NodeId, Node)]) = {
    require(tr.isUnique)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    (j, sub) =>
      0 <= j && j < i && sub.size < tr.size &&
        ...
        (scanTransaction(tr, init)(i)._1 == traverseTransaction(sub, beginRollback(scanTransaction(tr, init)(j)._1)))
  )
}