...
object TransactionTreeKeysDef {
  ...
  @pure
  @opaque
  def appearsAtIndex[Z](tr: Tree[(NodeId, Node)], init: Z, f1: (Z, (NodeId, Node)) => Z,
                        f2: (Z, (NodeId, Node)) => Z, k: GlobalKey, i: BigInt): Boolean = {
    require(i >= 0)
    require(i < 2 * tr.size)

    tr.scan(init, f1, f2)(i)._2._2 match {
      case a: Node.Action if (tr.scan(init, f1, f2)(i)._3 == TraversalDirection.Down) && (a.gkeyOpt == Some(k)) => true
      case _ => false
    }
  }
  ...
  @pure
  @opaque
  def doesNotAppearBefore[Z](tr: Tree[(NodeId, Node)], init: Z, f1: (Z, (NodeId, Node)) => Z,
                             f2: (Z, (NodeId, Node)) => Z, k: GlobalKey, i: BigInt): Boolean = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
  ...
  @pure
  @opaque
  def firstAppears[Z](tr: Tree[(NodeId, Node)], init: Z, f1: (Z, (NodeId, Node)) => Z,
                      f2: (Z, (NodeId, Node)) => Z, k: GlobalKey, i: BigInt): Boolean = {
    require(i >= 0)
    require(i < 2 * tr.size)
  ...
  @pure
  @opaque
  def containsAllKeysFun(init: Either[KeyInputError, State])(b: Boolean, n: (NodeId, Node)): Boolean = {
   ...
   */
  @pure
  def containsAllKeysBefore(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Boolean = {
    require(i >= 0)
    require(i < 2 * tr.size)
   ...
   */
  @pure
  def scanContainsAllKeys(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): List[(Boolean, (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def traverseContainsAllKeys(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Boolean = {
  ...
  def collectFun(m: Map[GlobalKey, KeyMapping], n: (NodeId, Node)): Map[GlobalKey, KeyMapping] = {
    n._2 match {
      case a: Node.Action if a.gkeyOpt.isDefined && !m.contains(a.gkeyOpt.get) => m.updated(a.gkeyOpt.get, nodeActionKeyMapping(a))
      case _ => m
    }
  }
   ...
   */
  @pure
  def collectTrace(tr: Tree[(NodeId, Node)]): List[(Map[GlobalKey, KeyMapping], (NodeId, Node), TraversalDirection)] = {
...
object TransactionTreeKeys {
  ...
  @pure
  @opaque
  def appearsAtIndexSame[Z1, Z2](tr: Tree[(NodeId, Node)], init1: Z1, init2: Z2, f11: (Z1, (NodeId, Node)) => Z1,
                                      f12: (Z1, (NodeId, Node)) => Z1, f21: (Z2, (NodeId, Node)) => Z2,
                                      f22: (Z2, (NodeId, Node)) => Z2, k: GlobalKey, i: BigInt): Unit = {
    require(i >= 0)
    require(i < 2 * tr.size)
   ...
   */
  @pure @opaque
  def doesNotAppearBeforeSame[Z1, Z2](tr: Tree[(NodeId, Node)], init1: Z1, init2: Z2, f11: (Z1, (NodeId, Node)) => Z1,
                          f12: (Z1, (NodeId, Node)) => Z1, f21: (Z2, (NodeId, Node)) => Z2,
                          f22: (Z2, (NodeId, Node)) => Z2, k: GlobalKey, i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
  ...
  @pure
  @opaque
  def firstAppearsSame[Z1, Z2](tr: Tree[(NodeId, Node)], init1: Z1, init2: Z2, f11: (Z1, (NodeId, Node)) => Z1,
                                      f12: (Z1, (NodeId, Node)) => Z1, f21: (Z2, (NodeId, Node)) => Z2,
                                      f22: (Z2, (NodeId, Node)) => Z2, k: GlobalKey, i: BigInt): Unit = {
    require(i >= 0)
    require(i < 2 * tr.size)
  ...
  @pure
  @opaque
  def doesNotAppearBeforeDiffIndex[Z](tr: Tree[(NodeId, Node)], init: Z, f1: (Z, (NodeId, Node)) => Z,
                                      f2: (Z, (NodeId, Node)) => Z, k: GlobalKey, i: BigInt, j: BigInt): Unit = {
    decreases(j - i)
    require(0 <= i)
    require(i <= j)
    require(j < 2 * tr.size)
  ...
  @pure
  @opaque
  def findFirstAppears[Z](tr: Tree[(NodeId, Node)], init: Z, f1: (Z, (NodeId, Node)) => Z,
                               f2: (Z, (NodeId, Node)) => Z, k: GlobalKey, i1: BigInt, i2: BigInt): BigInt = {
    decreases(i2 - i1)
    require(0 <= i1)
    require(i1 < i2)
  ...
  @pure
  @opaque
  def containsAllKeysBeforeImplies(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt, j: BigInt): Unit = {
    decreases(i - j)
    require(0 <= j)
    require(j <= i)
    require(i < 2 * tr.size)
    ...
    if(j == i){
      ...
      unfold(containsAllKeysFun(init)(containsAllKeysBefore(tr, init, i - 1), tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i - 1)._2))
  ...
  @pure
  @opaque
  def containsAllKeysImplies(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(containsAllKeys(tr, init))
    require(0 <= i)
    require(i < 2 * tr.size)

    scanIndexingState(tr, true, containsAllKeysFun(init), containsAllKeysFun(init), 0)
    unfold(containsAllKeysFun(init)(containsAllKeysBefore(tr, init, 2 * tr.size - 1), tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(2 * tr.size - 1)._2))
  ...
  @pure
  @opaque
  def containsAllKeysImpliesDown(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(containsAllKeys(tr, init))
    require(0 <= i)
    require(i < 2 * tr.size)

    scanIndexingNode(tr, init, true, traverseInFun, traverseOutFun, containsAllKeysFun(init), containsAllKeysFun(init), i)
    unfold(containsAllKeysFun(init)(containsAllKeysBefore(tr, init, i), scanTransaction(tr, init)(i)._2))
    scanTransactionProp(tr, init, i)
    containsKeySameGlobalKeys(init, scanTransaction(tr, init)(i)._1, scanTransaction(tr, init)(i)._2._2)
  ...
  @pure
  @opaque
  def containsAllKeysBeforeAltImplies(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt, j: BigInt): Unit = {
    decreases(i - j)
    require(0 <= j)
    require(j <= i)
    require(i < 2 * tr.size)
    ...
    if (j == i) {
      ...
      unfold(containsAllKeysFun(init)(tr.scan(true, containsAllKeysFun(init), (z, t) => z)(i - 1)._1, tr.scan(true, containsAllKeysFun(init), (z, t) => z)(i - 1)._2))
  ...
  @pure
  @opaque
  def containsAllKeysAltImplies(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
   ...
   */
  @pure @opaque
  def containsAllKeysBeforeAlt(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    scanIndexingNode(tr, true, true, containsAllKeysFun(init), containsAllKeysFun(init), containsAllKeysFun(init), (z, t) => z, i)
    if(i == 0){
      scanIndexingState(tr, true, containsAllKeysFun(init), (z, t) => z, 0)
      scanIndexingState(tr, true, containsAllKeysFun(init), containsAllKeysFun(init), 0)
    }
    else{
      scanIndexingNode(tr, true, true, containsAllKeysFun(init), containsAllKeysFun(init), containsAllKeysFun(init), (z, t) => z, i - 1)
      containsAllKeysBeforeAlt(tr, init, i - 1)
      scanIndexingState(tr, true, containsAllKeysFun(init), (z, t) => z, i)
      scanIndexingState(tr, true, containsAllKeysFun(init), containsAllKeysFun(init), i)
      unfold(containsAllKeysFun(init)(tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i - 1)._1,
        tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i - 1)._2))
      if(tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i - 1)._3 == TraversalDirection.Up){

        val j = findDown(tr, true, containsAllKeysFun(init), (z, t) => z, i - 1)
        scanIndexingState(tr, true, containsAllKeysFun(init), (z, t) => z, j + 1)
        unfold(containsAllKeysFun(init)(tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(j)._1,
  ...
  }.ensuring(tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i) == scanContainsAllKeys(tr, init)(i))
  ...
  def containsAllKeysAlt(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    if(tr.size > 0) {
      ...
      scanIndexingNode(tr, true, true, containsAllKeysFun(init), containsAllKeysFun(init), containsAllKeysFun(init), (z, t) => z, 2 * tr.size - 1)
      containsAllKeysBeforeAlt(tr, init, 2 * tr.size - 1)
      unfold(containsAllKeysFun(init)(tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(2 * tr.size - 1)._1,
        tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(2 * tr.size - 1)._2))
      if (tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(2 * tr.size - 1)._3 == TraversalDirection.Up) {
      ...
      case a: Node.Action =>
        ...
        if(a.gkeyOpt.isDefined){
          ...
          if(!m.contains(a.gkeyOpt.get)){
            ...
            MapAxioms.extensionality(Map[GlobalKey, KeyMapping](k -> nodeActionKeyMapping(a)) ++ m, m.updated(k, nodeActionKeyMapping(a)))
          }
          else{
            ...
            SetProperties.equalsSubsetOfTransitivity(Set(k), Map[GlobalKey, KeyMapping](k -> nodeActionKeyMapping(a)).keySet, m.keySet)
            MapProperties.concatSubsetOfEquals(Map[GlobalKey, KeyMapping](k -> nodeActionKeyMapping(a)), m)
            MapAxioms.extensionality(Map[GlobalKey, KeyMapping](k -> nodeActionKeyMapping(a)) ++ m, m)
          }
        }
        else {
  ...
  def collectTraceProp(tr: Tree[(NodeId, Node)], i: BigInt): Unit = {
    ...
    if(!collectTrace(tr)(i)._1.submapOf(collect(tr))) {
      val j = scanNotPropRev(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, x => x.submapOf(collect(tr)), i)
  ...
  def collectTraceDoesNotAppear(tr: Tree[(NodeId, Node)], k: GlobalKey, i: BigInt): Unit = {
    ...
    if(i == 0){
      ...
      unfold(appearsAtIndex(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, k, i - 1))
  ...
  def collectDoesNotAppear(tr: Tree[(NodeId, Node)], k: GlobalKey): Unit = {
    ...
    doesNotAppearBefore(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, k, 2 * tr.size - 1) ==
  ...
  def collectGet(tr: Tree[(NodeId, Node)], i: BigInt, n: Node.Action): Unit = {
    ...
    require(firstAppears(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, n.gkeyOpt.get, i))

    unfold(firstAppears(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, n.gkeyOpt.get, i))
    collectTraceDoesNotAppear(tr, n.gkeyOpt.get, i)

    unfold(appearsAtIndex(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, n.gkeyOpt.get, i))
    if(i == 2 * tr.size - 1){
      Unreachable()
    }
    else{
  ...
  def collectContains(tr: Tree[(NodeId, Node)], k: GlobalKey): BigInt = {
    ...
    if(tr.size == 0){
      ...
      unfold(doesNotAppearBefore(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, k, 0))
      findFirstAppears(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, k, 0, 2 * tr.size - 1)
    }
  }.ensuring(
    i =>
      (tr.size > 0)
  ...
  def emptyContainsAllKeys(tr: Tree[(NodeId, Node)]): Unit = {
    ...
    if(!containsAllKeys(tr, rempty)){
      ...
      if(j == 2 * tr.size - 1){
        ...
        scanIndexingNode(tr, true, Map.empty[GlobalKey, KeyMapping], containsAllKeysFun(rempty), (z, t) => z, collectFun, (z, t) => z, j)
        unfold(containsAllKeysFun(rempty)(tr.scan(true, containsAllKeysFun(rempty), (z, t) => z)(j)._1, tr.scan(true, containsAllKeysFun(rempty), (z, t) => z)(j)._2))
        unfold(containsKey(rempty)(tr.scan(true, containsAllKeysFun(rempty), (z, t) => z)(j)._2._2))
        unfold(containsNodeKey(emptyState(tr))(tr.scan(true, containsAllKeysFun(rempty), (z, t) => z)(j)._2._2))
          ...
          case a: Node.Action =>
            ...
            a.gkeyOpt match {
              ...
              case Some(k) =>
                ...
                scanIndexingState(tr, Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z, j + 1)
                unfold(collectFun(tr.scan(Map.empty[GlobalKey, KeyMapping], collectFun, (z, t) => z)(j)._1,
