...
object TransactionTreeFullDef {
  ...
  @pure
  @opaque
  def traverseInFunFull(s: Either[KeyInputError, State], p: (NodeId, Node)): Either[KeyInputError, State] = {
   ...
   */
  @pure
  def scanAddKey(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): List[(Either[KeyInputError, State], (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def traverseAddKey(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Either[KeyInputError, State] = {
   ...
   */
  @pure
  def scanTransactionFull(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): List[(Either[KeyInputError, State], (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def traverseTransactionFull(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Either[KeyInputError, State] = {
...
object TransactionTreeFull {
   ...
   */
  @pure @opaque
  def scanAddKeyIsConcatLeftGlobalKey(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    if(i == 0){
      ...
      if(init.isRight){
        ...
        MapAxioms.extensionality(init.get.globalKeys, Map.empty[GlobalKey, KeyMapping] ++ init.get.globalKeys)
      }
    }
    else{
      scanAddKeyIsConcatLeftGlobalKey(tr, init, i - 1)
      scanIndexingNode(tr, Map.empty[GlobalKey, KeyMapping], init, collectFun, (z, t) => z, addKeyBeforeNode, (z, t) => z, i - 1)
  ...
  @pure
  @opaque
  def traverseAddKeyIsConcatLeftGlobalKey(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    if(tr.size > 0) {
      ...
      scanIndexingNode(tr, Map.empty[GlobalKey, KeyMapping], init, collectFun, (z, t) => z, addKeyBeforeNode, (z, t) => z, 2 * tr.size - 1)
  ...
  @pure
  @opaque
  def scanTransactionConcatLeftGlobalKeys(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt,
                                          glK: Map[GlobalKey, KeyMapping]): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    if(i == 0){
      ...
      scanIndexingNode(tr, init, concatLeftGlobalKeys(init, glK), traverseInFun, traverseOutFun,
        traverseInFun, traverseOutFun, i - 1)
      ...
      if(dir == TraversalDirection.Down){
        ...
        n._2 match {
          case a: Node.Action =>
            scanIndexingNode(tr, init, true, traverseInFun, traverseOutFun,
              containsAllKeysFun(init), containsAllKeysFun(init), i - 1)
  ...
  @pure
  @opaque
  def scanTransactionAddKeyBeforeNode(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt,
                                      n: Node): Unit = {
    require(i >= 0)
    require(i < 2 * tr.size)
    require(containsAllKeysBefore(tr, init, i))
  ...
  @pure
  @opaque
  def scanTransactionFullCommute(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    require(init.isRight)

    scanIndexingNode(tr, scanAddKey(tr, init)(i)._1, init, traverseInFun, traverseOutFun, traverseInFunFull, traverseOutFun, i)
    ...
    if(i == 0) {
      ...
      scanIndexingNode(tr, scanAddKey(tr, init)(i)._1, init, traverseInFun, traverseOutFun, traverseInFunFull, traverseOutFun, i - 1)
      scanIndexingNode(tr, init, init, traverseInFunFull, traverseOutFun, addKeyBeforeNode, (z, t) => z, i - 1)
  ...
  @pure
  @opaque
  def traverseTransactionFullCommute(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    require(init.isRight)
    ...
    if(tr.size > 0) {
      ...
      scanIndexingNode(tr, traverseAddKey(tr, init), init, traverseInFun, traverseOutFun, traverseInFunFull, traverseOutFun, 2 * tr.size - 1)
      scanIndexingNode(tr, init, init, traverseInFunFull, traverseOutFun, addKeyBeforeNode, (z, t) => z, 2 * tr.size - 1)
      ...
      if (dir == TraversalDirection.Down) {
        ...
        scanTransactionAddKeyBeforeNode(tr, scanAddKey(tr, init)(2 * tr.size - 1)._1, 2 * tr.size - 1, n._2)
      }
    }
  ...
  @pure
  @opaque
  def scanAddKeyContains(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt, j: BigInt): Unit = {
    decreases(j)
    require(i >= 0)
    require(i < j)
    require(j < 2 * tr.size)
    ...
    if(j == i + 1){
      ...
      containsKeyAddKeyBeforeNode(scanAddKey(tr, init)(j - 1)._1, scanAddKey(tr, init)(j - 1)._2._2, scanAddKey(tr, init)(i)._2._2)
    }
  ...
  @pure
  @opaque
  def containsAllKeysBeforeAddKeyBeforeNode(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], n: Node, i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    require(containsAllKeysBefore(tr, init, i))
    ...
    scanIndexingState(tr, true, containsAllKeysFun(init), containsAllKeysFun(init), i)
    scanIndexingState(tr, true, containsAllKeysFun(addKeyBeforeNode(init, n)), containsAllKeysFun(addKeyBeforeNode(init, n)), i)
    if (i == 0) {
      Trivial()
    }
    else {
      scanIndexingNode(tr, true, true, containsAllKeysFun(init), containsAllKeysFun(init), containsAllKeysFun(addKeyBeforeNode(init, n)), containsAllKeysFun(addKeyBeforeNode(init, n)), i - 1)
      val (s1, ni, dir) = tr.scan(true, containsAllKeysFun(init), containsAllKeysFun(init))(i - 1)
      val s2 = tr.scan(true, containsAllKeysFun(addKeyBeforeNode(init, n)), containsAllKeysFun(addKeyBeforeNode(init, n)))(i - 1)._1
  ...
  @pure
  @opaque
  def scanAddKeyContainsAllKeysBefore(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    if (i == 0) {
      ...
      scanIndexingNode(tr, init, true, addKeyBeforeNode, (z, t) => z, containsAllKeysFun(scanAddKey(tr, init)(i)._1), (z, t) => z, i - 1)
      scanAddKeyContainsAllKeysBefore(tr, init , i - 1)

      val si = tr.scan(init, addKeyBeforeNode, (z, t) => z)(i - 1)._1
      val (ci, ni, dir) = tr.scan(true, containsAllKeysFun(scanAddKey(tr, init)(i)._1), (z, t) => z)(i - 1)
  ...
  @pure
  @opaque
  def traverseAddKeyContainsAllKeys(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    if (tr.size == 0) {
      ...
      scanIndexingNode(tr, init, true, addKeyBeforeNode, (z, t) => z, containsAllKeysFun(traverseAddKey(tr, init)), (z, t) => z, 2 * tr.size - 1)
      scanAddKeyContainsAllKeysBefore(tr, init, 2 * tr.size - 1)

      val si = tr.scan(init, addKeyBeforeNode, (z, t) => z)(2 * tr.size - 1)._1
      val (ci, ni, dir) = tr.scan(true, containsAllKeysFun(traverseAddKey(tr, init)), (z, t) => z)(2 * tr.size - 1)
...
//        scanAddKeyContains(tr, init, i - 1, i)
  ...
  def traverseTransactionFullEmptyKeys(tr: Tree[(NodeId, Node)]): Unit = {
    ...
    traverseTransactionFull(tr, Right[KeyInputError, State](State.empty)).get.globalKeys == collect(tr)
  )
  ...
  @opaque
  @dropVCs
  def traverseTransactionAdvanceDefined(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    require(stateInvariant(traverseAddKey(tr, init))(traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1))
    require(stateInvariant(Right[KeyInputError, State](emptyState(tr)))(traverseUnbound(tr)._1,
      traverseLC(tr, Set.empty[ContractId], Set.empty[ContractId], true)._1))
    require(!traverseTransactionFull(tr, Right[KeyInputError, State](State.empty)).get.withinRollbackScope)
    ...
    def traverseTransactionAdvanceCondition: Unit = {
      ...
      val p2: GlobalKey => Boolean = k => init.get.activeKeys.get(k).forall(m => Some(m) == trFullEmpty.get.globalKeys.get(k))
      ...
      emptyState(tr).globalKeys.keySet.forall(k => trKey.get.activeKeys.get(k) == emptyState(tr).globalKeys.get(k)) ==
      trFullEmpty.get.globalKeys.keySet.forall(k => init.get.activeKeys.get(k).forall(m => Some(m) == trFullEmpty.get.globalKeys.get(k)))
    )
    ...
    (init.get.advance(traverseTransactionFull(tr, Right[KeyInputError, State](State.empty)).get).isRight ==
      traverseTransactionFull(tr, init).isRight)
  )
  ...
  @pure
  @opaque
  def traverseTransactionAdvance(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    if(traverseTransactionFull(tr, init).isRight) {
      ...
      traverseLCExtractInitLC(tr, init.get.locallyCreated, init.get.consumed, Set.empty[ContractId], true)
      SetProperties.unionCommutativity(traverseTransaction(tr, rempty).get.locallyCreated, init.get.locallyCreated)
      ...
      traverseLCExtractInitConsumed(tr, init.get.locallyCreated, Set.empty[ContractId], init.get.consumed, true)
      SetProperties.unionCommutativity(traverseTransaction(tr, rempty).get.consumed, init.get.consumed)
      SetProperties.equalsTransitivity(
    ...
    (traverseTransactionFull(tr, init).isRight ==>
      (init.get.advance(traverseTransactionFull(tr, Right[KeyInputError, State](State.empty)).get).get ==
