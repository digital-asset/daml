...
object TransactionTreeAdvanceDef {
  ...
  @pure
  @opaque
  def activeStateInFun(s: (ActiveLedgerState, List[ActiveLedgerState]), p: (NodeId, Node)): (ActiveLedgerState, List[ActiveLedgerState]) = {
    p._2 match {
  ...
  @pure
  @opaque
  def activeStateOutFun(s: (ActiveLedgerState, List[ActiveLedgerState]), p: (NodeId, Node)): (ActiveLedgerState, List[ActiveLedgerState]) = {
    p._2 match {
   ...
   */
  @pure
  def scanActiveState(tr: Tree[(NodeId, Node)], init: (ActiveLedgerState, List[ActiveLedgerState])): List[((ActiveLedgerState, List[ActiveLedgerState]), (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def scanActiveState(tr: Tree[(NodeId, Node)]): List[((ActiveLedgerState, List[ActiveLedgerState]), (NodeId, Node), TraversalDirection)] = {
   ...
   */
  @pure
  def traverseActiveState(tr: Tree[(NodeId, Node)], init: (ActiveLedgerState, List[ActiveLedgerState])): (ActiveLedgerState, List[ActiveLedgerState]) = {
   ...
   */
  @pure
  def traverseActiveState(tr: Tree[(NodeId, Node)]): (ActiveLedgerState, List[ActiveLedgerState]) = {
...
object TransactionTreeAdvance {
  ...
  @pure
  @opaque
  def traverseActiveStateSameStack(tr: Tree[(NodeId, Node)], init: (ActiveLedgerState, List[ActiveLedgerState])): Unit = {
    decreases(tr)
  ...
  @pure
  @opaque
  def findBeginRollbackActiveState(tr: Tree[(NodeId, Node)], init: (ActiveLedgerState, List[ActiveLedgerState]), i: BigInt, j: BigInt): Unit = {
    decreases(tr)
    ...
    tr match {
      ...
      case ArticulationNode(l, r) =>
        ...
        if (j < 2 * l.size) {
          ...
          findBeginRollbackActiveState(r, l.traverse(init, activeStateInFun, activeStateOutFun), i - 2 * l.size, j - 2 * l.size)
        }
        else {
          scanContains(l, init, activeStateInFun, activeStateOutFun, i)
          scanContains(r, l.traverse(init, activeStateInFun, activeStateOutFun), activeStateInFun, activeStateOutFun, j - 2 * l.size)
          SetProperties.disjointContains(l.content, r.content, tr.scan(init, activeStateInFun, activeStateOutFun)(i)._2)
          SetProperties.disjointContains(l.content, r.content, tr.scan(init, activeStateInFun, activeStateOutFun)(j)._2)
          Unreachable()
        }
    }
  }.ensuring(scanActiveState(tr, init)(i)._1._1 :: scanActiveState(tr, init)(i)._1._2 == scanActiveState(tr, init)(j)._1._2)
  ...
  @pure
  @opaque
  def scanActiveStateAdvance(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    decreases(i)
    ...
    scanIndexingState(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), activeStateInFun, activeStateOutFun, i)
    ...
    if (i == 0) {
      ...
      scanIndexingNode(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), init, activeStateInFun, activeStateOutFun, traverseInFun, traverseOutFun, i - 1)
      ...
      if (dir == TraversalDirection.Down) {
        ...
        n._2 match {
          ...
          case r: Node.Rollback =>
            ...
            scanIndexingNode(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), init, activeStateInFun, activeStateOutFun, traverseInFun, traverseOutFun, j)

            findBeginRollbackActiveState(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), j, i - 1)
  ...
  @pure
  @opaque
  def traverseActiveStateAdvance(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    if (tr.size == 0) {
      ...
      scanIndexingState(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), activeStateInFun, activeStateOutFun, 0)

      traverseTransactionDefined(tr, init, 2 * tr.size - 1)
      unfold(propagatesError(scanTransaction(tr, init)(2 * tr.size - 1)._1, traverseTransaction(tr, init)))
      scanActiveStateAdvance(tr, init, 2 * tr.size - 1)
      scanIndexingNode(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), init, activeStateInFun, activeStateOutFun, traverseInFun, traverseOutFun, 2 * tr.size - 1)
      ...
      if (dir == TraversalDirection.Down) {
        ...
        n._2 match {
          ...
          case r: Node.Rollback =>
            ...
            findBeginRollbackActiveState(tr, (ActiveLedgerState.empty, Nil[ActiveLedgerState]()), j, 2 * tr.size - 1)
            scanActiveStateAdvance(tr, init, j)
        }
      }
    }
