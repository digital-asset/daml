...
object TransactionTreeInvariant {
   ...
   */
  @pure @opaque
  def scanStateNodeCompatibility(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(i < 2 * tr.size)
    require(0 <= i)
    ...
    unfold(stateNodeCompatibility(si.get, n._2, traverseUnbound(tr)._1, traverseLC(tr, ilc, icons, true)._1, dir))

    dir match {
      case TraversalDirection.Up => Trivial()
      case TraversalDirection.Down =>
        scanIndexingNode(tr, init, (ilc, icons, true), traverseInFun, traverseOutFun, buildLC, (z, t) => z, i)
        scanIndexingNode(tr, init, (Set.empty[ContractId], Set.empty[ContractId], true), traverseInFun, traverseOutFun, unboundFun, (z, t) => z, i)
    ...
    stateNodeCompatibility(scanTransaction(tr, init)(i)._1.get, scanTransaction(tr, init)(i)._2._2, traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1, scanTransaction(tr, init)(i)._3)
  )
   ...
   */
  @pure @opaque
  def scanInvariant(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State], i: BigInt): Unit = {
    require(i >= 0)
    require(i < 2 * tr.size)
    require(init.isRight)
    require(traverseUnbound(tr)._3)
    ...
    val p: Either[KeyInputError, State] => Boolean = x => stateInvariant(x)(traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
          ...
          n._2 match {
            case a: Node.Action =>
              if(dir == TraversalDirection.Down) {
                ...
                handleNodeInvariant(state, n._1, a, traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
              }
            case r: Node.Rollback =>
              if(dir == TraversalDirection.Down) {
                unfold(beginRollback(s))
                stateInvariantBeginRollback(state, traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
              }
              else{
                unfold(endRollback(s))
                stateInvariantEndRollback(state, traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
              }
          }
      }
    }
    ...
    stateInvariant(scanTransaction(tr, init)(i)._1)(traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
  )
  ...
  def traverseInvariant(tr: Tree[(NodeId, Node)], init: Either[KeyInputError, State]): Unit = {
    ...
    stateInvariant(traverseTransaction(tr, init))(traverseUnbound(tr)._1, traverseLC(tr, init.get.locallyCreated, init.get.consumed, true)._1)
  )
