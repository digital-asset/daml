...
object CSMConversion {
  ...
  def globalKeysGlobalKeyInputs(gki: Map[GlobalKey, KeyMapping]): Unit = {
    ...
    MapProperties.equalsTransitivity(gki, gki.mapValues(toKeyInput andThen toKeyMapping), globalKeys(globalKeyInputs(gki)))
  ...
  def toOriginal(s: State): StateOriginal[NodeId] = {
    StateOriginal[NodeId](s.locallyCreated, globalKeyInputs(s.globalKeys), toOriginal(s.activeState), s.rollbackStack.map(toOriginal), ContractKeyUniquenessMode.Strict)
  }
  @pure
  def toAlt(consumed: Set[ContractId])(s: StateOriginal[NodeId]): State = {
    State(s.locallyCreated, consumed, globalKeys(s.globalKeyInputs), toAlt(s.activeState), s.rollbackStack.map(toAlt))
  }
  ...
  @pure
  @opaque
  def lookupActiveGlobalKeyInputToAlt(s: StateOriginal[NodeId], key: GlobalKey, consumed: Set[ContractId]) = {
    ...
    MapProperties.mapValuesGet(toAlt(consumed)(s).globalKeys, keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy), key)

  }.ensuring(s.lookupActiveGlobalKeyInput(key) == (toAlt(consumed)(s)).globalKeys.mapValues(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)).get(key))

  @pure
  @opaque
  def getLocalActiveKeyToAlt(s: ActiveLedgerStateOriginal[NodeId], key: GlobalKey, consumed: Set[ContractId]): Unit = {
    unfold(toAlt(s))
    unfold(keyMappingToActiveMapping(toAlt(s).consumedBy))
    MapProperties.mapValuesGet(s.localKeys, (v: ContractId) => if (s.consumedBy.contains(v)) KeyInactive else KeyActive(v), key)
    MapProperties.mapValuesGet(toAlt(s).localKeys.mapValues(KeyActive), keyMappingToActiveMapping(toAlt(s).consumedBy), key)
    MapProperties.mapValuesGet(toAlt(s).localKeys, KeyActive, key)
  }.ensuring(s.getLocalActiveKey(key) == toAlt(s).localKeys.mapValues(KeyActive).mapValues(keyMappingToActiveMapping(toAlt(s).consumedBy)).get(key))

  @pure
  @opaque
  def lookupActiveKeyToAlt(s: StateOriginal[NodeId], key: GlobalKey, consumed: Set[ContractId]): Unit = {
    unfold(toAlt(consumed)(s).activeKeys)
    unfold(toAlt(consumed)(s).keys)
    MapProperties.mapValuesConcat(toAlt(consumed)(s).globalKeys, toAlt(consumed)(s).activeState.localKeys.mapValues(KeyActive), keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy))
    MapProperties.equalsGet(toAlt(consumed)(s).activeKeys,
      toAlt(consumed)(s).globalKeys.mapValues(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)) ++
        toAlt(consumed)(s).activeState.localKeys.mapValues(KeyActive).mapValues(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)), key)
    MapAxioms.concatGet(toAlt(consumed)(s).globalKeys.mapValues(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)), toAlt(consumed)(s).activeState.localKeys.mapValues(KeyActive).mapValues(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)), key)
  ...
  @pure
  @opaque
  def consumeToAlt(s: StateOriginal[NodeId], cid: ContractId, nid: NodeId, consumed: Set[ContractId]): Unit = {
    unfold(toAlt(consumed)(s).consume(cid, nid))
  }.ensuring(toAlt(consumed + cid)(s.copy(activeState = s.activeState.consume(cid, nid)))== toAlt(consumed)(s).consume(cid, nid))

//
  @pure @opaque
  def globalKeyInputsContainsToAlt(s: StateOriginal[NodeId], gk: GlobalKey, consumed: Set[ContractId]): Unit = {
  ...
  @pure
  @opaque
  def globalKeyInputsUpdatedToAlt(s: StateOriginal[NodeId], gk: GlobalKey, keyInput: KeyInput, consumed: Set[ContractId]): Unit = {
    unfold(globalKeys(s.globalKeyInputs))
    ...
    MapAxioms.extensionality(s.globalKeyInputs.updated(gk, keyInput).mapValues(toKeyMapping), s.globalKeyInputs.mapValues(toKeyMapping).updated(gk, keyInput.toKeyMapping))
  }.ensuring(globalKeys(s.globalKeyInputs.updated(gk, keyInput)) == globalKeys(s.globalKeyInputs).updated(gk, keyInput.toKeyMapping))


  @pure
  @opaque
  def visitCreateToAlt(s: StateOriginal[NodeId], contractId: ContractId, mbKey: Option[GlobalKey], consumed: Set[ContractId]): Unit = {
  ...
  }.ensuring(addKey(toAlt(consumed)(s), mbKey, KeyInactive).visitCreate(contractId, mbKey) == toAlt(consumed)(s.visitCreate(contractId, mbKey)))


  @pure
  def extractPair(e: Either[Option[ContractId] => (KeyMapping, StateOriginal[NodeId]), (KeyMapping, StateOriginal[NodeId])], result: Option[ContractId]): (KeyMapping, StateOriginal[NodeId]) = {
    e match {
  ...
  @pure
  def extractState(e: Either[Option[ContractId] => (KeyMapping, StateOriginal[NodeId]), (KeyMapping, StateOriginal[NodeId])], result: Option[ContractId]): StateOriginal[NodeId] = {
    extractPair(e, result)._2
  }

    @pure
    @opaque
    def resolveKeyToAlt(s: StateOriginal[NodeId], result: Option[ContractId], gkey: GlobalKey, consumed: Set[ContractId],
                        unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
...
//      globalKeyInputsContainsToAlt(s, gkey, consumed)
      ...
      unfold(addKey(toAlt(consumed)(s), gkey, result))
      globalKeyInputsUpdatedToAlt(s, gkey, result match {
        case None() => NegativeKeyLookup
        case Some(cid) => Transaction.KeyActive(cid)
      }, consumed)
      unfold(toAlt(consumed)(s).activeKeys.getOrElse(gkey, keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy)(result)))
      unfold(keyMappingToActiveMapping(toAlt(consumed)(s).activeState.consumedBy))


    }.ensuring(
      (addKey(toAlt(consumed)(s), gkey, result).activeKeys(gkey), addKey(toAlt(consumed)(s), gkey, result)) ==
        (extractPair(s.resolveKey(gkey), result)._1, toAlt(consumed)(extractState(s.resolveKey(gkey), result)))
    )

  @pure
  @opaque
  def visitLookupToAlt(s: StateOriginal[NodeId], gk: GlobalKey, keyInput: Option[ContractId], keyResolution: Option[ContractId],
                       consumed: Set[ContractId], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(toAlt(consumed)(s))(unbound, lc))
  ...
  @pure
  @opaque
  def assertKeyMappingToAlt(s: StateOriginal[NodeId], cid: ContractId, gkey: Option[GlobalKey], consumed: Set[ContractId],
                            unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(toAlt(consumed)(s))(unbound, lc))
  ...
  }.ensuring(addKey(toAlt(consumed)(s), gkey, Some(cid)).assertKeyMapping(cid, gkey) == toAlt(consumed)(s.assertKeyMapping(cid, gkey)))

  @pure
  @opaque
  def visitExerciseToAlt(s: StateOriginal[NodeId], nodeId: NodeId, targetId: ContractId, gk: Option[GlobalKey], byKey: Boolean, consuming: Boolean,
                         consumed: Set[ContractId], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(toAlt(consumed)(s))(unbound, lc))
    require(s.mode == ContractKeyUniquenessMode.Strict)
    unfold(addKey(toAlt(consumed)(s), gk, Some(targetId)).visitExercise(nodeId, targetId, gk, byKey, consuming))
    ...
    addKey(toAlt(consumed)(s), gk, Some(targetId)).visitExercise(nodeId, targetId, gk, byKey, consuming) ==
    toAlt(if (consuming) consumed + targetId else consumed)(s.visitExercise(nodeId, targetId, gk, byKey, consuming))
  )
  ...
  @pure
  @opaque
  def handleNodeOriginal(e: Either[KeyInputError, StateOriginal[NodeId]], nodeId: NodeId, node: Node.Action, keyInput: Option[ContractId]): Either[KeyInputError, StateOriginal[NodeId]] = {
    e match {
  ...
  @pure @opaque
  def handleNodeToAlt(s: StateOriginal[NodeId], nodeId: NodeId, node: Node.Action, keyInput: Option[ContractId], consumed: Set[ContractId], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(toAlt(consumed)(s))(unbound, lc))
    require(s.mode == ContractKeyUniquenessMode.Strict)
    ...
    node match {
      ...
      case fetch: Node.Fetch => assertKeyMappingToAlt(s, fetch.coid, fetch.gkeyOpt, consumed, unbound, lc)
      ...
      case exe: Node.Exercise => visitExerciseToAlt(s, nodeId, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming, consumed, unbound, lc)
    }
  }.ensuring(
  ...
  @pure
  @opaque
  def handleNodeToAlt(e: Either[KeyInputError, StateOriginal[NodeId]], nodeId: NodeId, node: Node.Action, keyInput: Option[ContractId], consumed: Set[ContractId], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
  ...
  @pure @opaque
  def endRollbackOriginal(s: StateOriginal[NodeId]): Either[KeyInputError, StateOriginal[NodeId]] = {
    if(s.withinRollbackScope){
      Right[KeyInputError, StateOriginal[NodeId]](s.endRollback())
    }
    else{
      Left[KeyInputError, StateOriginal[NodeId]](Left[InconsistentContractKey, DuplicateContractKey](InconsistentContractKey(GlobalKey(BigInt(0)))))
    }
  }
  ...
  @pure
  @opaque
  def advanceToAlt(s: ActiveLedgerStateOriginal[NodeId], substate: ActiveLedgerStateOriginal[NodeId]): Unit = {
   ...
   */
  @pure @opaque
  def advanceToAlt(s: StateOriginal[NodeId], substate: StateOriginal[NodeId], resolver: KeyResolver, consumed1: Set[ContractId], consumed2: Set[ContractId]): Unit = {
    require(s.mode == ContractKeyUniquenessMode.Strict)
    ...
    MapAxioms.extensionality(globalKeys(substate.globalKeyInputs ++ s.globalKeyInputs), globalKeys(substate.globalKeyInputs) ++ globalKeys(s.globalKeyInputs))
