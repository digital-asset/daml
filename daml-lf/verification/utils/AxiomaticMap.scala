...
case class Map[K, V] (@pure @extern toScala: ScalaMap[K, V]) {
  ...
  def updated(k: K, v: V): Map[K, V] = {
    ...
    def updatedProperties: Unit = {
      ...
      SetProperties.subsetOfEqualsTransitivity(res.values, values ++ Map(k -> v).values, values ++ Set(v))
...
object MapAxioms {
   ...
   */
  @pure @extern
  def concatGet[K, V](m1: Map[K, V], m2: Map[K, V], ks: K): Unit = {
  }.ensuring((m1 ++ m2).get(ks) == m2.get(ks).orElse(m1.get(ks)))
  ...
  @pure
  @extern
  def emptyGet[K, V](ks: K): Unit = {
  }.ensuring(Map.empty[K, V].get(ks) == None[V]())
   ...
   */
  @pure @extern
  def singletonGet[K, V](k: K, v: V, ks: K): Unit = {
  }.ensuring(Map(k -> v).get(ks) == (if (k == ks) Some(v) else None[V]()))
   ...
   */
  @pure @extern
  def mapValuesGet[K, V, V2](m: Map[K, V], f: K => V => V2, k: K): Unit = {
  }.ensuring(m.mapValues(f).get(k) == m.get(k).map(f(k)))
  ...
  @pure
  @extern
  def preimageGet[K, V](m: Map[K, V], s: Set[V], k: K): Unit = {
  }.ensuring(m.preimage(s).contains(k) == (m.get(k).exists(s.contains)))
