...
sealed trait Tree[T] {
  ...
  def scan[Z](init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z): List[(Z, T, TraversalDirection)] = {
    ...
    this match {
      ...
      case ContentNode(n, sub) =>
        concatIndex(sub.scan(f1(init, n), f1, f2), List((sub.traverse(f1(init, n), f1, f2), n, TraversalDirection.Up)), 2 * size - 2)
        (init, n, TraversalDirection.Down) :: (sub.scan(f1(init, n), f1, f2) ++ List((sub.traverse(f1(init, n), f1, f2), n, TraversalDirection.Up)))
    }
  }.ensuring(
    res =>
      (res.size == 2 * size) &&
...
case class ArticulationNode[T](left: ContentNode[T], right: StructuralNode[T]) extends StructuralNode[T]
...
object TreeProperties {
   ...
   */
  @pure @opaque
  def scanIndexing[T, Z](n: T, sub: StructuralNode[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, i: BigInt): Unit = {
    require(i >= 0)
    require(i < 2 * ContentNode(n, sub).size)
    unfold(ContentNode(n, sub).size)
    unfold(ContentNode(n, sub).scan(init, f1, f2))
    if(i != 0) {
      concatIndex(sub.scan(f1(init, n), f1, f2), List((sub.traverse(f1(init, n), f1, f2), n, TraversalDirection.Up)), i - 1)
    }
  }.ensuring(ContentNode(n, sub).scan(init, f1, f2)(i) ==
    (
      if (i == BigInt(0)) (init, n, TraversalDirection.Down)
      else if (i == 2 * (ContentNode(n, sub).size) - 1) (sub.traverse(f1(init, n), f1, f2), n, TraversalDirection.Up)
  ...
  @pure
  @opaque
  def scanIndexing[T, Z](l: ContentNode[T], r: StructuralNode[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, i: BigInt): Unit = {
    require(i >= 0)
  ...
  }.ensuring(ArticulationNode(l, r).scan(init, f1, f2)(i) ==
    (
   ...
   */
  @pure @opaque
  def scanIndexingState[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, i: BigInt): Unit = {
    decreases(tr)
    require(i >= 0)
    require(i < 2 * tr.size)
    require(tr.size > 0)
    ...
    (tr.scan(init, f1, f2)(i)._1 == (
      ...
      (tr.traverse(init, f1, f2) == f2(tr.scan(init, f1, f2)(2 * tr.size - 1)._1, tr.scan(init, f1, f2)(2 * tr.size - 1)._2))
  )
  ...
  @pure
  @opaque
  def scanIndexingNode[T, Z1, Z2](tr: Tree[T], init1: Z1, init2: Z2, f11: (Z1, T) => Z1, f12: (Z1, T) => Z1, f21: (Z2, T) => Z2, f22: (Z2, T) => Z2, i: BigInt): Unit = {
    decreases(tr)
    require(i >= 0)
    require(i < 2 * tr.size)
    ...
    tr match {
      ...
      case ArticulationNode(l, r) =>
        ...
        if(i < 2 * l.size) {
          ...
          scanIndexingNode(r, l.traverse(init1, f11, f12), l.traverse(init2, f21, f22), f11, f12, f21, f22, i - 2 * l.size)
        }
  ...
  @pure
  @opaque
  def scanNotProp[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, p: Z => Boolean, i: BigInt): BigInt = {
    decreases(i)
    require(i >= 0)
    require(i < 2 * tr.size)
    require(p(init))
  ...
  @pure
  @opaque
  def scanNotPropRev[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, p: Z => Boolean, i: BigInt): BigInt = {
    ...
    j =>
      (j >= i && j < 2 * tr.size - 1 && !p(tr.scan(init, f1, f2)(j)._1) && p(tr.scan(init, f1, f2)(j + 1)._1)) ||
  ...
  @pure
  @opaque
  def traverseNotProp[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, p: Z => Boolean): BigInt = {
    require(p(init))
    require(!p(tr.traverse(init, f1, f2)))
    ...
    i =>
      i >= 0 && ((i < 2 * tr.size - 1 && p(tr.scan(init, f1, f2)(i)._1) && !p(tr.scan(init, f1, f2)(i + 1)._1))
   ...
   */
  @pure @opaque
  def scanContains[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, i: BigInt): Unit = {
    decreases(tr)
    require(0 <= i)
    require(i < 2 * tr.size)
    unfold(tr.size)
   ...
   */
  @pure @opaque
  def isUniqueIndexing[T, Z](tr: Tree[T], init: Z, f1: (Z, T) => Z, f2: (Z, T) => Z, i: BigInt, j: BigInt): Unit = {
    decreases(tr)
    require(tr.isUnique)
    require(0 <= i)
    require(i < 2  * tr.size)
