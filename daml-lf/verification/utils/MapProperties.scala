...
object MapProperties {
  ...
  def mapValuesConcat[K, V, V2](m1: Map[K, V], m2: Map[K, V], f: K => V => V2): Unit = {
    if ((m1 ++ m2).mapValues[V2](f) =/= (m1.mapValues[V2](f) ++ m2.mapValues[V2](f))) {
      val k = notEqualsWitness((m1 ++ m2).mapValues[V2](f), m1.mapValues[V2](f) ++ m2.mapValues[V2](f))
  ...
  def mapValuesUpdated[K, V, V2](m: Map[K, V], k: K, v: V, f: K => V => V2): Unit = {
    ...
    equalsTransitivity(m.updated(k, v).mapValues[V2](f), m.mapValues[V2](f) ++ Map(k -> v).mapValues[V2](f), m.mapValues[V2](f) ++ Map(k -> f(k)(v)))
  ...
  @pure
  @extern
  def preimageGet[K, V](m: Map[K, V], v: V, k: K): Unit = {
  }.ensuring(m.preimage(v).contains(k) == (m.get(k) == Some[V](v)))
  ...
  def preimageSingleton[K, V](k: K, v: V, s: Set[V]): Unit = {
    ...
    if (m.preimage(s) =/= (if (s.contains(v)) Set(k) else Set.empty[K])) {
      val ks = SetProperties.notEqualsWitness[K](m.preimage(s), if (s.contains(v)) Set(k) else Set.empty[K])
      MapAxioms.preimageGet(m, s, ks)
      SetProperties.emptyContains(ks)
      singletonGet(k, v, ks)
      SetProperties.singletonContains(k, ks)
    }
  }.ensuring(((m: Map[K, V]) => m.preimage(s) === (if (s.contains(v)) Set(k) else Set.empty[K]))(Map[K, V](k -> v)))
  ...
  def concatPreimage[K, V](m1: Map[K, V], m2: Map[K, V], s: Set[V]): Unit = {
    if (!(m1 ++ m2).preimage(s).subsetOf(m1.preimage(s) ++ m2.preimage(s))) {
      val k: K = SetProperties.notSubsetOfWitness((m1 ++ m2).preimage(s), m1.preimage(s) ++ m2.preimage(s))
  ...
  def inclPreimage[K, V](m: Map[K, V], k: K, v: V, s: Set[V]): Unit = {
    ...
    if (s.contains(v)) {
      ...
      SetProperties.subsetOfEqualsTransitivity(m.updated(k, v).preimage(s), m.preimage(s) ++ singl.preimage(s), m.preimage(s) + k)
    } else{
      SetProperties.unionEqualsRight(m.preimage(s), singl.preimage(s), Set.empty)
      SetProperties.unionEmpty(m.preimage(s))
      SetProperties.equalsTransitivity(m.preimage(s) ++ singl.preimage(s), m.preimage(s) ++ Set.empty, m.preimage(s))
      SetProperties.subsetOfEqualsTransitivity(m.updated(k, v).preimage(s), m.preimage(s) ++ singl.preimage(s), m.preimage(s))
      SetProperties.subsetOfTransitivity(m.updated(k, v).preimage(s), m.preimage(s), m.preimage(s) + k)
    }
  }.ensuring(
    if (s.contains(v))
      m.updated(k, v).preimage(s).subsetOf(m.preimage(s) + k)
    else
      m.updated(k, v).preimage(s).subsetOf(m.preimage(s)) && m.updated(k, v).preimage(s).subsetOf(m.preimage(s) + k)
  )
