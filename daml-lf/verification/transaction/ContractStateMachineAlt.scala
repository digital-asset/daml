...
case class State(
  ...
  def keys: Map[GlobalKey, KeyMapping] = {
    ...
    SetProperties.unionEqualsRight(globalKeys.keySet, activeState.localKeys.keySet, activeState.localKeys.mapValues(KeyActive).keySet)
    MapProperties.concatKeySet(globalKeys, activeState.localKeys.mapValues(KeyActive))
    SetProperties.equalsTransitivity((globalKeys ++ activeState.localKeys.mapValues(KeyActive)).keySet, globalKeys.keySet ++ activeState.localKeys.mapValues(KeyActive).keySet, globalKeys.keySet ++ activeState.localKeys.keySet)

    globalKeys ++ activeState.localKeys.mapValues(KeyActive)
  }.ensuring(
    res =>
      res.keySet === globalKeys.keySet ++ activeState.localKeys.keySet
  )
  ...
  @pure @opaque
  def visitCreate(contractId: ContractId, mbKey: Option[GlobalKey]): Either[DuplicateContractKey, State] = {
  ...
  @pure @opaque
  def visitLookup(gk: GlobalKey, keyResolution: Option[ContractId]): Either[InconsistentContractKey, State] = {
    val res = Either.cond(activeKeys.getOrElse(gk, KeyInactive) == keyResolution, this, InconsistentContractKey(gk))
  ...
  @pure @opaque
  def assertKeyMapping(cid: ContractId, mbKey: Option[GlobalKey]): Either[InconsistentContractKey, State] = {
    val res = mbKey match {
  ...
  def handleNode(id: NodeId, node: Node.Action): Either[KeyInputError, State] = {
    ...
    case exe: Node.Exercise => toKeyInputError(visitExercise(id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming))
  }
    ...
    def sameHandleNode: Unit = {
      node match {
        ...
        case exe: Node.Exercise =>
          sameGlobalKeysTransitivity(this, visitExercise(id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming), res)
          sameStackTransitivity(this, visitExercise(id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming), res)
      }
  ...
  def endRollback(): Either[KeyInputError, State] = {
    val res = rollbackStack match {
      case Nil() => Left[KeyInputError, State](Left[InconsistentContractKey, DuplicateContractKey](InconsistentContractKey(GlobalKey(BigInt(0)))))
      case Cons(headState, tailStack) => Right[KeyInputError, State](this.copy(activeState = headState, rollbackStack = tailStack))
    }
    unfold(sameGlobalKeys(this, res))
    unfold(sameLocallyCreated(this, res))
    unfold(sameConsumed(this, res))
  ...
  def advance(substate: State): Either[Unit, State] = {
    ...
    if(substate.globalKeys.keySet.forall(k => activeKeys.get(k).forall(m => Some(m) == substate.globalKeys.get(k)))){
      Right[Unit, State](
        this.copy(
...
object State {
  def empty: State = new State(Set.empty, Set.empty, Map.empty, ContractStateMachine.ActiveLedgerState.empty, List.empty)
}
  ...
  type KeyResolver = Map[GlobalKey, KeyMapping]
    ...
    final case class ActiveLedgerState(
      ...
      def advance(substate: ActiveLedgerState): ActiveLedgerState =
        ActiveLedgerState(
          locallyCreatedThisTimeline = locallyCreatedThisTimeline ++ substate.locallyCreatedThisTimeline,
    ...
    object ActiveLedgerState {
      def empty: ActiveLedgerState = ActiveLedgerState(Set.empty[ContractId], Map.empty[ContractId, NodeId], Map.empty[GlobalKey, ContractId])
    }
}
