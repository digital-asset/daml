...
object CSMInconsistency {
  ...
  @pure
  @opaque
  def visitExerciseUndefined(s: State, nodeId: NodeId, targetId: ContractId, gk: Option[GlobalKey], byKey: Boolean, consuming: Boolean): Unit = {
  ...
  }.ensuring(s.visitExercise(nodeId, targetId, gk, byKey, consuming).isLeft == inconsistencyCheck(s, gk, KeyActive(targetId)))
  ...
  def handleNodeUndefined(s: State, id: NodeId, node: Node.Action): Unit = {
    ...
    node match {
      ...
      case exe: Node.Exercise => visitExerciseUndefined(s, id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming)
    }
  }.ensuring(s.handleNode(id, node).isLeft == inconsistencyCheck(s, node.gkeyOpt, nodeActionKeyMapping(node)))
  ...
  @opaque
  @pure
  def visitCreateActiveKeysGet(s: State, contractId: ContractId, mbKey: Option[GlobalKey], k2: GlobalKey): Unit = {
  ...
  @opaque
  @pure
  def visitLookupActiveKeysGet(s: State, gk: GlobalKey, keyResolution: Option[ContractId], k2: GlobalKey): Unit = {
  ...
  @opaque
  @pure
  def assertKeyMappingActiveKeysGet(s: State, cid: ContractId, mbKey: Option[GlobalKey], k2: GlobalKey): Unit = {
  ...
  @pure
  @opaque
  def visitExerciseActiveKeysGet(s: State, nodeId: NodeId, targetId: ContractId, gk: Option[GlobalKey], byKey: Boolean, consuming: Boolean, k2: GlobalKey, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
      ...
      case Right(state) =>
        ...
        gk match {
          ...
          case None() =>
            ...
            SetProperties.disjointContains(unbound.map(KeyActive), s.globalKeys.values, KeyActive(targetId))
        ...
        (s.activeState.localKeys.get(k2).map(KeyActive), s.globalKeys.get(k2)) match {
          case (Some(Some(c1)), _) => MapProperties.updatedContains(state.activeState.consumedBy, targetId, nodeId, c1)
          case (_, Some(Some(c2))) => MapProperties.updatedContains(state.activeState.consumedBy, targetId, nodeId, c2)
          case _ => Trivial()
        }
    }

  }.ensuring(s.visitExercise(nodeId, targetId, gk, byKey, consuming).get.activeKeys.get(k2) == s.activeKeys.get(k2))
   ...
   */
    @pure
    @opaque
    def handleNodeActiveKeysGet(s: State, id: NodeId, node: Node.Action, k2: GlobalKey, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
      ...
      node match {
        ...
        case exe: Node.Exercise =>
          ...
          visitExerciseActiveKeysGet(s, id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming, k2, unbound, lc)
      }
  ...
  @opaque
  @pure
  def visitCreateDifferentStatesActiveKeysGet(s1: State, s2: State, contractId: ContractId, mbKey: Option[GlobalKey]): Unit = {
    ...
    val me1 =
      s1.copy(
        ...
        activeState = s1.activeState
          .copy(locallyCreatedThisTimeline =
            s1.activeState.locallyCreatedThisTimeline + contractId
          ))
    ...
    val me2 =
      s2.copy(
        ...
        activeState = s2.activeState
          .copy(locallyCreatedThisTimeline =
            s2.activeState.locallyCreatedThisTimeline + contractId
          ))
  ...
  }.ensuring(mbKey.forall(k => s1.visitCreate(contractId, mbKey).get.activeKeys.get(k) == s2.visitCreate(contractId, mbKey).get.activeKeys.get(k)))
  ...
  @opaque
  @pure
  def visitLookupDifferentStatesActiveKeysGet(s1: State, s2: State, gk: GlobalKey, keyResolution: Option[ContractId],
                                             unbound1: Set[ContractId], lc1: Set[ContractId], unbound2: Set[ContractId], lc2: Set[ContractId]): Unit = {
    require(s1.visitLookup(gk, keyResolution).isRight)
    require(s2.visitLookup(gk, keyResolution).isRight)
    require(keyResolution.isDefined || (containsKey(s1)(gk) && containsKey(s2)(gk) && stateInvariant(s1)(unbound1, lc1) && stateInvariant(s2)(unbound2, lc2)))
    ...
    (s1.visitLookup(gk, keyResolution).get.activeKeys.get(gk) == s2.visitLookup(gk, keyResolution).get.activeKeys.get(gk)) &&
  ...
  @opaque
  @pure
  def assertKeyMappingDifferentStatesActiveKeysGet(s1: State, s2: State, cid: ContractId, mbKey: Option[GlobalKey]): Unit = {
    ...
    mbKey match {
      case None() => Trivial()
      case Some(k) => visitLookupDifferentStatesActiveKeysGet(s1, s2, k, Some(cid), Set.empty[ContractId],
        Set.empty[ContractId], Set.empty[ContractId], Set.empty[ContractId])
    }

  }.ensuring(
    mbKey.forall(gk => (s1.assertKeyMapping(cid, mbKey).get.activeKeys.get(gk) == s2.assertKeyMapping(cid, mbKey).get.activeKeys.get(gk)) &&
  ...
  @opaque
  @pure
  def visitExerciseDifferentStatesActiveKeysGet(s1: State, s2: State, nodeId: NodeId, targetId: ContractId, mbKey: Option[GlobalKey],
                                               byKey: Boolean, consuming: Boolean): Unit = {
  ...
  }.ensuring(mbKey.forall(k => s1.visitExercise(nodeId, targetId, mbKey, byKey, consuming).get.activeKeys.get(k) ==
   ...
   */
    @opaque
    @pure
    def handleNodeDifferentStatesActiveKeysGet(s1: State, s2: State, nid: NodeId, node: Node.Action,
                                                    unbound1: Set[ContractId], lc1: Set[ContractId], unbound2: Set[ContractId],
                                                    lc2: Set[ContractId]): Unit = {
      ...
      node match {
        ...
        case lookup: Node.LookupByKey =>
          ...
          visitLookupDifferentStatesActiveKeysGet(s1, s2, lookup.gkey, lookup.result, unbound1, lc1, unbound2, lc2)
        case exe: Node.Exercise => visitExerciseDifferentStatesActiveKeysGet(s1, s2, nid, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming)
      }

    }.ensuring(
      node.gkeyOpt.forall(k => s1.handleNode(nid, node).get.activeKeys.get(k) ==
