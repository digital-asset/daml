...
object CSMInvariantDef {
  ...
  @pure
  def invariantWrtActiveState(s: State)(unbound: Set[ContractId], lc: Set[ContractId]): ActiveLedgerState => Boolean =
    a =>
      //point b.1.
  ...
  @pure
  def invariantWrtList(s: State)(unbound: Set[ContractId], lc: Set[ContractId])(l: List[ActiveLedgerState]): Boolean = l.forall(invariantWrtActiveState(s)(unbound, lc))
  ...
  def stateInvariant(s: State)(unbound: Set[ContractId], lc: Set[ContractId]): Boolean = {
    ...
    val invariant: Boolean = {
      ...
      //point f.
      (s.globalKeys.values.filter(_.isDefined)).forall(v => s.globalKeys.preimage(v).size <= BigInt(1)) &&
    ...
    def invariantProperties: Unit = {
      ...
      SetProperties.equalsTransitivityStrong(s.globalKeys.keySet, s.globalKeys.keySet ++ s.activeState.localKeys.keySet, s.activeKeys.keySet)
      ...
      SetProperties.mapSubsetOf[ContractId, KeyMapping](s.activeState.localKeys.values, lc, KeyActive)
      SetProperties.disjointSubsetOf(lc.map[KeyMapping](KeyActive), s.activeState.localKeys.values.map[KeyMapping](KeyActive), s.globalKeys.values)
  ...
  @pure
  @opaque
  def stateNodeCompatibility(s: State, n: Node, unbound: Set[ContractId], lc: Set[ContractId], dir: TraversalDirection): Boolean = {
   ...
   */
  @pure
  def invariantWrtList[T](e: Either[T, State])(unbound: Set[ContractId], lc: Set[ContractId])(l: List[ActiveLedgerState]): Boolean =
    e.forall(s => invariantWrtList(s)(unbound, lc)(l))

  @pure
  def stateInvariant[T](e: Either[T, State])(unbound: Set[ContractId], lc: Set[ContractId]): Boolean = {
...
object CSMInvariant{
  ...
  @pure
  @opaque
  def stateInvariantSameFields[T](s1: State, s2: Either[T, State], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
  ...
  @pure
  @opaque
  def stateInvariantSameFields[U, T](s1: Either[U, State], s2: Either[T, State], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s1)(unbound, lc))
    require(sameLocalKeys(s1, s2))
    require(sameConsumedBy(s1, s2))
    require(sameGlobalKeys(s1, s2))
  ...
  @pure
  @opaque
  def stateInvariantSameState[T](s1: State, s2: Either[T, State], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s1)(unbound, lc))
  ...
  @pure
  @opaque
  def stateInvariantSameState[U, T](s1: Either[U, State], s2: Either[T, State], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s1)(unbound, lc))
    require(sameState(s1, s2))
    require(propagatesError(s1, s2)) //we want to avoid the case s1.isLeft and s2.isRight
  ...
  @pure
  @opaque
  def stateInvariantAssertKeyMapping(s: State, cid: ContractId, mbKey: Option[GlobalKey], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
  ...
  @pure
  @opaque
  def stateInvariantVisitLookup(s: State, gk: GlobalKey, keyResolution: Option[ContractId], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
  ...
  @pure
  @opaque
  def stateInvariantVisitExercise(s: State, nodeId: NodeId, targetId: ContractId, mbKey: Option[GlobalKey], byKey: Boolean, consuming: Boolean, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
    ...
    s.assertKeyMapping(targetId, mbKey) match {
      ...
      case Right(state) =>
        ...
        if(consuming) {
          ...
          SetProperties.equalsSubsetOfTransitivity(s.activeState.consumedBy.updated(targetId, nodeId).keySet,
            s.activeState.consumedBy.keySet + targetId, s.consumed + targetId)

          val res = state.consume(targetId, nodeId)

          if (!invariantWrtList(res)(unbound, lc)(s.rollbackStack)) {
            val a = ListProperties.notForallWitness(s.rollbackStack, invariantWrtActiveState(res)(unbound, lc))
  ...
  }.ensuring(stateInvariant(s.visitExercise(nodeId, targetId, mbKey, byKey, consuming))(unbound, lc))
  ...
  @pure
  @opaque
  def stateInvariantVisitCreate(s: State, contractId: ContractId, mbKey: Option[GlobalKey], unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
    ...
    if (!invariantWrtList(me)(unbound, lc)(me.rollbackStack)) {
      val a = ListProperties.notForallWitness(me.rollbackStack, invariantWrtActiveState(me)(unbound, lc))
      ListProperties.forallContains(s.rollbackStack, invariantWrtActiveState(s)(unbound, lc), a)
    ...
    mbKey match {
      case None() =>
         ...
         */
        SetProperties.subsetOfTransitivity(s.activeState.localKeys.values, s.locallyCreated, me.locallyCreated)
      ...
      case Some(gk) =>
        ...
        SetProperties.subsetOfTransitivity(s.activeState.createKey(gk, contractId).localKeys.values,
          s.activeState.localKeys.values + contractId, s.locallyCreated + contractId)
         ...
         */
        SetProperties.disjointSubsetOf(unbound, s.activeState.localKeys.values + contractId, s.activeState.createKey(gk, contractId).localKeys.values)
         ...
         * contain contractId.
         */
        val newF: ContractId => Boolean = v => ns.activeState.localKeys.preimage(v).size <= BigInt(1)
        ...
        if (s.activeState.localKeys.values.contains(contractId)){
          SetProperties.subsetOfContains(s.activeState.localKeys.values, s.locallyCreated, contractId)
        }
        ...
        //STEP 5.2 newF(contractId)
        {
          ...
          SetProperties.subsetOfSize(s.activeState.localKeys.updated(gk, contractId).preimage(contractId),
        ...
        if(!s.activeState.localKeys.values.forall(newF)){
          ...
          SetProperties.forallContains(s.activeState.localKeys.values, v => s.activeState.localKeys.preimage(v).size <= BigInt(1), w)
          MapProperties.inclPreimage(s.activeState.localKeys, gk, contractId, w)
          SetProperties.subsetOfSize(s.activeState.localKeys.updated(gk, contractId).preimage(w),
        ...
        MapProperties.updatedValues(s.activeState.localKeys, gk, contractId)
        SetProperties.forallSubsetOf(s.activeState.localKeys.updated(gk, contractId).values, s.activeState.localKeys.values + contractId, newF)
        ...
        SetProperties.equalsSubsetOfTransitivity(s.activeState.localKeys.updated(gk, contractId).keySet, s.activeState.localKeys.keySet + gk, s.globalKeys.keySet)
      }
   ...
   */
    @pure
    @opaque
    def handleNodeInvariant(s: State, id: NodeId, node: Node.Action, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
      require(stateInvariant(s)(unbound, lc))
      ...
      node match {
        case create: Node.Create => stateInvariantVisitCreate(s, create.coid, create.gkeyOpt, unbound, lc)
        case fetch: Node.Fetch => stateInvariantAssertKeyMapping(s, fetch.coid, fetch.gkeyOpt, unbound, lc)
        case lookup: Node.LookupByKey =>
          unfold(containsOptionKey(s)(node.gkeyOpt))
          stateInvariantVisitLookup(s, lookup.gkey, lookup.result, unbound, lc)
        case exe: Node.Exercise => stateInvariantVisitExercise(s, id, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming, unbound, lc)
      }
...
object CSMInvariantDerivedProp {
  ...
  @pure
  @opaque
  def invariantContainsKey(s: State, gk: GlobalKey, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
    unfold(containsKey(s)(gk))
  ...
  @pure
  @opaque
  def invariantGetGlobalKeys(s: State, k: GlobalKey, k2: GlobalKey, c: ContractId, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
    ...
    if(s.activeState.localKeys.get(k2) == Some(c)) {
      ...
      SetProperties.mapContains[ContractId, KeyMapping](s.activeState.localKeys.values, KeyActive, s.activeState.localKeys(k2))
      SetProperties.disjointContains(s.activeState.localKeys.values.map[KeyMapping](KeyActive), s.globalKeys.values, KeyActive(c))
      Unreachable()
    }
    ...
    if (s.globalKeys.get(k2) == Some(KeyActive(c))) {
      ...
      SetProperties.forallContains(s.globalKeys.values.filter(_.isDefined), v => s.globalKeys.preimage(v).size <= BigInt(1), KeyActive(c))
      Unreachable()
    }
  }.ensuring((s.globalKeys.get(k2) != Some(KeyActive(c))) && (s.activeState.localKeys.get(k2) != Some(c)))
  ...
  @pure
  @opaque
  def invariantGetLocalKeys(s: State, k: GlobalKey, k2: GlobalKey, c: ContractId, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    require(stateInvariant(s)(unbound, lc))
    ...
    if (s.globalKeys.get(k2) == Some(KeyActive(c))) {
      ...
      SetProperties.mapContains[ContractId, KeyMapping](s.activeState.localKeys.values, KeyActive, c)
      SetProperties.disjointContains(s.activeState.localKeys.values.map[KeyMapping](KeyActive), s.globalKeys.values, s.globalKeys(k2))
      Unreachable()
    }
    ...
    if (s.activeState.localKeys.get(k2) == Some(c)) {
      ...
      SetProperties.forallContains(s.activeState.localKeys.values, v => s.activeState.localKeys.preimage(v).size <= BigInt(1), c)
      Unreachable()
    }
  }.ensuring((s.globalKeys.get(k2) != Some(KeyActive(c))) && (s.activeState.localKeys.get(k2) != Some(c)))
  ...
  def emptyActiveStateInvariant(s: State, unbound: Set[ContractId], lc: Set[ContractId]): Unit = {
    ...
    SetProperties.forallIsEmpty(ActiveLedgerState.empty.localKeys.values, v => ActiveLedgerState.empty.localKeys.preimage(v).size <= BigInt(1))
