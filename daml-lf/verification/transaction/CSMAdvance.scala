...
object CSMAdvanceDef {
  ...
  def actionActiveStateAddition(id: NodeId, n: Node.Action): ActiveLedgerState = {
    n match {
      case create: Node.Create => ActiveLedgerState(
        Set(create.coid),
      ...
      case exe: Node.Exercise => ActiveLedgerState(
...
object CSMAdvance {
  ...
  def emptyAdvance(s: ActiveLedgerState): Unit = {
    ...
    SetAxioms.extensionality(s.locallyCreatedThisTimeline ++ Set.empty[ContractId],
      s.locallyCreatedThisTimeline)
    SetAxioms.extensionality(Set.empty[ContractId] ++ s.locallyCreatedThisTimeline,
      s.locallyCreatedThisTimeline)
    MapProperties.concatEmpty(s.consumedBy)
    MapAxioms.extensionality(s.consumedBy ++ Map.empty[ContractId, NodeId],
      s.consumedBy)
    MapAxioms.extensionality(Map.empty[ContractId, NodeId] ++ s.consumedBy,
      s.consumedBy)
    MapProperties.concatEmpty(s.localKeys)
    MapAxioms.extensionality(s.localKeys ++ Map.empty[GlobalKey, ContractId],
      s.localKeys)
    MapAxioms.extensionality(Map.empty[GlobalKey, ContractId] ++ s.localKeys,
      s.localKeys)
  ...
  @pure
  @opaque
  def advanceAssociativity(s1: ActiveLedgerState, s2: ActiveLedgerState, s3: ActiveLedgerState): Unit = {
    unfold(s1.advance(s2).advance(s3))
    unfold(s1.advance(s2))
    unfold(s1.advance(s2.advance(s3)))
    unfold(s2.advance(s3))

    SetProperties.unionAssociativity(s1.locallyCreatedThisTimeline, s2.locallyCreatedThisTimeline, s3.locallyCreatedThisTimeline)
    SetAxioms.extensionality((s1.locallyCreatedThisTimeline ++ s2.locallyCreatedThisTimeline) ++ s3.locallyCreatedThisTimeline,
    ...
    MapAxioms.extensionality((s1.consumedBy ++ s2.consumedBy) ++ s3.consumedBy,
      s1.consumedBy ++ (s2.consumedBy ++ s3.consumedBy))
    MapProperties.concatAssociativity(s1.localKeys, s2.localKeys, s3.localKeys)
    MapAxioms.extensionality((s1.localKeys ++ s2.localKeys) ++ s3.localKeys,
  ...
  def visitCreateActiveState(s: State, contractId: ContractId, mbKey: Option[GlobalKey]): Unit = {
    ...
    unfold(s.activeState.advance(ActiveLedgerState(Set(contractId), Map.empty[ContractId, NodeId], (mbKey match {
      case None() => Map.empty[GlobalKey, ContractId]
    ...
    MapAxioms.extensionality(s.activeState.consumedBy ++ Map.empty[ContractId, NodeId],
      s.activeState.consumedBy)

    mbKey match {
      case None() =>
        MapProperties.concatEmpty(s.activeState.localKeys)
        MapAxioms.extensionality(s.activeState.localKeys ++ Map.empty[GlobalKey, ContractId],
    ...
    s.visitCreate(contractId, mbKey).forall(
      _.activeState == s.activeState.advance(
        ActiveLedgerState(Set(contractId), Map.empty[ContractId, NodeId],
          (mbKey match {
        case None() => Map.empty[GlobalKey, ContractId]
        case Some(k) => Map[GlobalKey, ContractId](k -> contractId)
      })))
  ...
  @pure
  @opaque
  def visitExerciseActiveState(s: State, nodeId: NodeId, targetId: ContractId,
                               mbKey: Option[GlobalKey], byKey: Boolean, consuming: Boolean): Unit = {

    unfold(s.visitExercise(nodeId, targetId, mbKey, byKey, consuming))
    unfold(s.activeState.advance(ActiveLedgerState(Set.empty[ContractId], (if (consuming) Map(targetId -> nodeId) else Map.empty[ContractId, NodeId]), Map.empty[GlobalKey, ContractId])))
    ...
    s.assertKeyMapping(targetId, mbKey) match {
      case Right(state) =>
        ...
        MapAxioms.extensionality(state.activeState.consumedBy ++ Map.empty[ContractId, NodeId],
          state.activeState.consumedBy)


        SetProperties.unionEmpty(state.activeState.locallyCreatedThisTimeline)
        SetAxioms.extensionality(state.activeState.locallyCreatedThisTimeline ++ Set.empty[ContractId],
          state.activeState.locallyCreatedThisTimeline)
        MapProperties.concatEmpty(state.activeState.localKeys)
        MapAxioms.extensionality(state.activeState.localKeys ++ Map.empty[GlobalKey, ContractId],
    ...
    s.visitExercise(nodeId, targetId, mbKey, byKey, consuming).forall(
      _.activeState == s.activeState.advance(ActiveLedgerState(Set.empty[ContractId], (if (consuming) Map(targetId -> nodeId) else Map.empty[ContractId, NodeId]), Map.empty[GlobalKey, ContractId]))
    )
  )
  ...
  def handleNodeActiveState(s: State, nodeId: NodeId, n: Node.Action): Unit = {
    ...
    n match {
      ...
      case fetch: Node.Fetch => sameActiveStateActiveState(s, s.assertKeyMapping(fetch.coid, fetch.gkeyOpt))
      case lookup: Node.LookupByKey => sameActiveStateActiveState(s, s.visitLookup(lookup.gkey, lookup.result))
      case exe: Node.Exercise => visitExerciseActiveState(s, nodeId, exe.targetCoid, exe.gkeyOpt, exe.byKey, exe.consuming)
    }

  }.ensuring(
    s.handleNode(nodeId, n).forall(sf => sf.activeState == s.activeState.advance(actionActiveStateAddition(nodeId, n)))
  )
