...
private[parser] object ExprParser {
  ...
  private lazy val fieldInit: Parser[(Identifier, Expr)] = id ~ (`=` ~> expr) ^^ {
    case fName ~ value => fName -> value
  }

  private lazy val fieldInits: Parser[ImmArray[(Identifier, Expr)]] =
  ...
  private[parser] lazy val varBinder: Parser[(Identifier, Type)] =
