...
object Ast {
  ...
  final case class PLNumeric(override val value: BigDecimal) extends PrimLit
  ...
  final case object BRoundNumeric extends BuiltinFunction(2) // : ∀s. Int64 → Numeric s → Numeric s
  ...
  final case object BLessEqNumeric extends BuiltinFunction(2) // : ∀s. Numeric s → Numeric s → Bool
  ...
  final case object BEqualNumeric extends BuiltinFunction(2) // : ∀s. Numeric s -> Numeric s -> Bool
