...
object Ast {
  ...
  final case class GenModule[E](
    ...
    implementations: InterfaceImplementations[E],
  ...
  final case class GenPackage[E](
    ...
    // Map of all interface implementations in the package
    // to the module that includes the implementation.
    // There can be at most one implementation for a given (interface, template) tuple
    // in a daml-lf package.
    implementations: Map[InterfaceInstance, ModuleName],
  )
