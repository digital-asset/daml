...
Note that the rules above are designed such that for every expression,
at most one rule applies. For brevity and readability, we did not
list the cases where one of the sub-expressions *errors out*, that is,
evaluates to a result of the form ``Err v``.

An error in any sub-expression will result in an error for the overall
expression, but two or more sub-expressions that error out in this way may
result in either of those errors being propagated upwards. In this way,
``Ok v`` results are deterministic, but ``Err v`` results are non-deterministic.

For example, the omitted sub-expression error rules for application are:
...
Meaning that if both ``e₁`` and ``e₂`` error out, then either of those
errors may be propagated upwards. Which error gets propagated is
implementation-defined.

Note, however, that we do not propagate errors from inside the body
of a lambda before its arguments are evaluated, inside the body of
a let expression before the bound value is evaluated, or inside
a case branch before its pattern is matched. This order-dependency
is captured by ``⇓  r`` rules above, which handle those particular
error cases. This gives DAML-LF an eager evaluation semantics.

This semantics does not require nor preclude the cacheing or
memoization of ``Ok`` results for top-level values, or for any
other value. This is considered an implementation detail.
...
cases where sub-expressions fail.
