...
object Typecheck {
  def extractDelExistNew[K, V](arg: Upgrading[Map[K, V]]): (Map[K, V], Map[K, Upgrading[V]], Map[K, V]) =
    ( arg.past -- arg.present.keySet
    , arg.past.keySet.intersect(arg.present.keySet).map(k => k -> Upgrading(arg.past(k), arg.present(k))).toMap
    , arg.present -- arg.past.keySet
    )

  def checkDeleted[K, V, T <: Throwable](arg: Upgrading[Map[K, V]], handler: V => T): Try[(Map[K, Upgrading[V]], Map[K, V])] = {
  ...
  def checkPackage(present: (Ref.PackageId, Ast.Package), past: (Ref.PackageId, Ast.Package)): Try[Unit] = {
