...
object LossyValueTranslator {
  ...
  def unsafeTranslate(value: Value): (SValue, Type) = {
    val ValueTy(value, ty) = translate(value)
    (value, ty)
  }

  private def translate(value: Value): ValueTy = {

    def w(value: SValue, ty: Type): ValueTy = ValueTy(value, ty)
    def bt(value: SValue, bt: BuiltinType): ValueTy = ValueTy(value, TBuiltin(bt))
    ...
    def go(value0: Value, nesting: Int = 0): ValueTy = {
      if (nesting > Value.MAXIMUM_NESTING) {
        ...
        value0 match {
          case ValueUnit => bt(SValue.SUnit, BTUnit)
          case ValueBool(b) => bt(if (b) SValue.SValue.True else SValue.SValue.False, BTBool)
          case ValueInt64(i) => bt(SValue.SInt64(i), BTInt64)
          case ValueTimestamp(t) => bt(SValue.STimestamp(t), BTTimestamp)
          case ValueDate(t) => bt(SValue.SDate(t), BTDate)
          case ValueText(t) => bt(SValue.SText(t), BTText)
          case ValueParty(p) => bt(SValue.SParty(p), BTParty)
          ...
          case ValueNumeric(d) =>
            Numeric.fromBigDecimal(TNat.Decimal.n, d) match {
              case Right(value) =>
                w(SValue.SNumeric(value), TApp(TBuiltin(BTNumeric), TNat.Decimal))
          ...
          case ValueOptional(mbValue) =>
            mbValue match {
              case None =>
                w(SValue.SValue.None, TBuiltin(BTOptional))
              case Some(x) =>
                val vt = go(x, newNesting)
                w(SValue.SOptional(Some(vt.value)), TApp(TBuiltin(BTOptional), vt.ty))
            }

          case ValueList(ls) =>
            val list = ls.map(go(_, newNesting))
            list.iterator.nextOption() match {
              case None => w(SValue.SValue.EmptyList, TBuiltin(BTList))
              case Some(ref) => w(SValue.SList(list.map(_.value)), TApp(TBuiltin(BTList), ref.ty))
            }

          case ValueTextMap(entries) =>
            entries.iterator.nextOption() match {
              case None =>
                w(SValue.SValue.EmptyTextMap, TBuiltin(BTTextMap))
              case Some((_, refVal)) =>
