  ...
  class ModuleEncoder(selfPkgId: PackageId) {
    ...
    private implicit def encodeKind(k: Kind): PLF.Kind =
      k match {
        ...
        case KStar =>
          star
        case KNat =>
          assertSince(LV.Features.numerics, "nat kind not supported by LF <= 1.6")
          nat
      }

    /** * Encoding of types ***/
    private val builtinTypeMap =
      DecodeV1.builtinTypeInfos.map(info => info.lfName -> info).toSeq.toMap

    private implicit def encodeBuiltinType(bType: BuiltinType): PLF.PrimType = {
      val info = builtinTypeMap(bType)
      assertSince(info.minVersion, bType.toString)
      info.protoName
    }
    ...
    private def encodeTypeBuilder(typ0: Type): PLF.Type.Builder = {
      ...
      typ match {
        ...
        case TNat(n) =>
          assertSince(LV.Features.numerics, "TNat not supported by version <= 1.6")
          builder.setNat(n.toLong)
        case TTyCon(tycon) =>
          builder.setCon(
            PLF.Type.Con.newBuilder().setTycon(tycon).accumulateLeft(args)(_ addArgs _))
        case TBuiltin(BTArrow) if isOlderThan(LV.Features.arrowType) =>
          args match {
        ...
        case TBuiltin(bType) =>
          val typs =
            if (bType == BTNumeric && isOlderThan(LV.Features.numerics))
    ...
    private val builtinFunctionMap =
      DecodeV1.builtinInfos.map(info => info.lfName -> info).toSeq.toMap
      ...
      expr0 match {
        ...
        case ETyApps(expr, typs1) =>
          val typs: ImmArray[Type] =
            if (isLegacyDecimalBuiltin(expr) && isOlderThan(LV.Features.numerics))
