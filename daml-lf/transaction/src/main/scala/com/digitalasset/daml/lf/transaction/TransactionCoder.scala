...
object TransactionCoder {
  ...
  private[lf] def encodeNode[Nid, Cid](
    ...
    node match {
      ...
      case (actionNode: GenActionNode[Nid, Cid]) =>
        val nodeVersion = actionNode.version
        if (enclosingVersion < nodeVersion)
          Left(
            EncodeError(
              s"A transaction of version $enclosingVersion cannot contain nodes of newer version (${actionNode.version})"
            )
          )
        else {

          if (enclosingVersion >= TransactionVersion.minNodeVersion)
            nodeBuilder.setVersion(actionNode.version.protoValue)
            ...
            case ne @ NodeExercises(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _) =>
              ...
              for {
                _ <- Either.cond(
                  ...
                  right = (),
                  left = EncodeError(actionNode.version, isTooOldFor = "non-empty choice-observers"),
                )
                _ <- encodeAndSetValue(
                  encodeCid,
                  nodeVersion,
                ...
                _ <- ne.exerciseResult match {
                  ...
                  case None =>
                    Either.cond(
                      ...
                      left = EncodeError(
                        actionNode.version,
                        isTooOldFor = "NodeExercises without result",
                      ),
                    )
                }
                _ <- encodeAndSetContractKey(
