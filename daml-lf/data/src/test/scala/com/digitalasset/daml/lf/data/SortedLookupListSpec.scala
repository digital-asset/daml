...
class SortedLookupListSpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "SortMap.fromImmArray should fails if the input list contians duplicate keys" in {
    ...
    forAll(negativeTestCases)(l => SortedLookupList.fromImmArray(l) shouldBe Symbol("right"))

    forAll(positiveTestCases)(l => SortedLookupList.fromImmArray(l) shouldBe Symbol("left"))
  ...
  "SortMap.fromSortedImmArray should fails if the input list is not sorted" in {
    ...
    forAll(negativeTestCases)(l => SortedLookupList.fromOrderedImmArray(l) shouldBe Symbol("right"))

    forAll(positiveTestCases)(l => SortedLookupList.fromOrderedImmArray(l) shouldBe Symbol("left"))
