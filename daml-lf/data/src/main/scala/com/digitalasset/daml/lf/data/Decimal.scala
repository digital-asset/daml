...
object Decimal {
  ...
  type Decimal = DecimalModule.T
  import DecimalModule.cast
  ...
  /** Checks that a `Decimal` falls between `min` and `max`, and
    ...
    */
  private def checkWithinBoundsAndRound(x0: BigDecimal): Either[String, Decimal] = {
    if (x0 > max || x0 < min) {
  ...
  /** Like `checkWithinBoundsAndRound`, but _fails_ if the given number contains
    ...
    */
  def fromBigDecimal(x0: BigDecimal): Either[String, Decimal] =
    for {
  ...
  def assertFromBigDecimal(x: BigDecimal): Decimal =
