...
object SExpr {
  ...
  /** Reference to a variable. 'index' is the 1-based de Bruijn index,
    ...
    */
  final case class SEVar private (index: Int) extends SExpr {
    def execute(machine: Machine): Ctrl = {
      CtrlValue(machine.getEnv(index))
    }
  }
  object SEVar extends (Int => SEVar) with Memoizable[Int, SEVar] {
    def apply(index: Int): SEVar = memoize(index, new SEVar(index))
  }
  ...
  final case class SEBuiltin private (b: SBuiltin) extends SExpr {
    def execute(machine: Machine): Ctrl = {
  ...
  object SEBuiltin extends Memoizable[SBuiltin, SEBuiltin] {
    def apply(b: SBuiltin): SEBuiltin =
      memoize(b, new SEBuiltin(b))
  }
  ...
  final case class LfDefRef private (ref: DefinitionRef) extends SDefinitionRef
  object LfDefRef extends (DefinitionRef => LfDefRef) with Memoizable[DefinitionRef, LfDefRef] {
    def apply(ref: DefinitionRef): LfDefRef = memoize(ref, new LfDefRef(ref))
  }

  // references to definitions generated by the Speedy compiler
  final case class ChoiceDefRef private (ref: DefinitionRef, choiceName: ChoiceName)
      extends SDefinitionRef
  object ChoiceDefRef
      extends ((DefinitionRef, ChoiceName) => ChoiceDefRef)
      with Internable[ChoiceDefRef] {
    def apply(ref: DefinitionRef, choiceName: ChoiceName): ChoiceDefRef =
      intern(new ChoiceDefRef(ref, choiceName))
  }
