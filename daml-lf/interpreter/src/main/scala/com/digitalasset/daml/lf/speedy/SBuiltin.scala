...
import com.daml.lf.speedy.Speedy.{CtrlWronglyTypeContractId, Machine, SpeedyHungry}
...
object SBuiltin {
  ...
  sealed abstract class SBBinaryOpInt64(op: (Long, Long) => Long) extends SBuiltin(2) {
    final def execute(args: util.ArrayList[SValue], machine: Machine): Unit =
      machine.ctrl = SInt64(
        op(args.get(0).asInstanceOf[SInt64].value, args.get(1).asInstanceOf[SInt64].value))
  }
  ...
  sealed abstract class SBBinaryOpNumeric2(op: (Scale, Numeric, Numeric) => Numeric)
    ...
    final def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = SNumeric(op(scale, a, b))
    }
  }
  ...
  final case object SBAppendText extends SBuiltin(2) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = (args.get(0), args.get(1)) match {
  ...
  final case object SBToTextNumeric extends SBuiltin(2) {
    override def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = SText(Numeric.toUnscaledString(x))
    }
  }
  ...
  final case object SBToQuotedTextParty extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = SText(s"'${v.value: String}'")
    }
  }
  ...
  final case object SBToTextCodePoints extends SBuiltin(1) {
    override def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = SText(Utf8.pack(codePoints.toImmArray))
    }
  }
  ...
  final case object SBFromTextParty extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = Party.fromString(v.value) match {
        case Left(_) => SValue.SValue.None
  ...
  final case object SBFromTextCodePoints extends SBuiltin(1) {
    override def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ctrl = SList(FrontStack(codePoints.map(SInt64)))
    }
  }
  ...
  final case object SBEqual extends SBuiltin(2) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SValue.SBool(svalue.Equality.areEqual(args.get(0), args.get(1)))
    }
  }

  sealed abstract class SBCompare(pred: Int => Boolean) extends SBuiltin(2) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SValue.SBool(pred(svalue.Ordering.compare(args.get(0), args.get(1))))
    }
  }
  ...
  final case object SBSome extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SOptional(Some(args.get(0)))
    }
  }
  ...
  final case class SBRecCon(id: Identifier, fields: Array[Name])
    ...
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SRecord(id, fields, args)
    }
  }
  ...
  final case class SBStructCon(fields: Array[Name])
      ...
      with SomeArrayEquals {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SStruct(fields, args)
    }
  }
  ...
  final case class SBVariantCon(id: Identifier, variant: Ast.VariantConName, constructorRank: Int)
      extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SVariant(id, variant, constructorRank, args.get(0))
    }
  }
  ...
  @throws[SpeedyHungry]
  def translateValue(machine: Machine, value: V[V.ContractId]): SValue = {
  ...
  final case class SBULookupKey(templateId: TypeConName) extends SBuiltin(2) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      machine.ptx.keys.get(gkey) match {
        case Some(mbCoid) =>
          machine.ctrl = SOptional(mbCoid.map { coid =>
            SContractId(coid)
          })
        case None =>
  ...
  final case object SBGetTime extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      throw SpeedyHungry(
        SResultNeedTime(timestamp => machine.ctrl = STimestamp(timestamp)),
      )
    }
  }
    ...
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      throw SpeedyHungry(
        SResultScenarioPassTime(
          relTime,
          timestamp => machine.ctrl = STimestamp(timestamp),
        ),
      )
    }
  }
  ...
  final case object SBSGetParty extends SBuiltin(2) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      ...
      args.get(0) match {
        case SText(name) =>
          throw SpeedyHungry(
            SResultScenarioGetParty(name, party => machine.ctrl = SParty(party)),
          )
        case v =>
  ...
  final case class SBToAny(ty: Ast.Type) extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      machine.ctrl = SAny(ty, args.get(0))
    }
  }
  ...
  final case object SBTextToUpper extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      args.get(0) match {
        case SText(t) =>
          machine.ctrl = SText(t.toUpperCase(util.Locale.ROOT))
  ...
  final case object SBTextToLower extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit = {
      args.get(0) match {
        case SText(t) =>
          machine.ctrl = SText(t.toLowerCase(util.Locale.ROOT))
        // TODO [FM]: replace with ASCII-specific function, or not
        case x =>
              ...
              args.get(2) match {
                case SText(t) =>
                  ...
                  if (to <= 0 || from >= length || to <= from) {
                    ...
                    machine.ctrl = SText(t.slice(ifrom, ito))
                  }
                case x =>
          ...
          args.get(1) match {
            case SText(t) =>
              ...
              if (n < 0) {
                ...
                machine.ctrl = SOptional(Some(SInt64(rn)))
              }
            case x =>
        ...
        case SText(pattern) =>
          args.get(1) match {
            case SText(t) =>
              ...
              machine.ctrl = SList(FrontStack(seq))
            case x =>
        ...
        case SText(sep) =>
          args.get(1) match {
            case SList(vs) =>
              ...
              machine.ctrl = SText(xs.iterator.mkString(sep))
            case x =>
  ...
  final case class SBIfThenElse(ifCase: SExpr, elseCase: SExpr) extends SBuiltin(1) {
    def execute(args: util.ArrayList[SValue], machine: Machine): Unit =
      machine.ctrl =
        if (args.get(0) == SValue.SValue.True)
