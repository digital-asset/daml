  ...
  final case object SBFoldr extends SBuiltin(3) {
    override private[speedy] final def execute(
      ...
      if (pap.arity - pap.actuals.size >= 2) {
        ...
        machine.pushKont(KFoldr(func, array, array.length, machine.frame, machine.actuals, machine.env.size))
        machine.returnValue = init
      } else {
        ...
        stack.pop match {
          ...
          case Some((head, tail)) =>
            machine.pushKont(KFoldr1Map(func, tail, FrontStack.empty, init, machine.frame, machine.actuals, machine.env.size))
