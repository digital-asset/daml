...
class AnfTest extends WordSpec with Matchers {
  ...
  def app(func: SExpr, arg: SExpr): SExpr = SEAppGeneral(func, Array(arg))
  def app2(func: SExpr, arg1: SExpr, arg2: SExpr): SExpr = SEAppGeneral(func, Array(arg1, arg2))
  def binop(op: SBuiltin, x: SExpr, y: SExpr): SExpr = SEApp(SEBuiltin(op), Array(x, y))

  def ite(i: SExpr, t: SExpr, e: SExpr): SExpr =
    SECase(i, Array(SCaseAlt(patTrue, t), SCaseAlt(patFalse, e)))

  // anf builders
  def let1(rhs: SExpr, body: SExpr): SExpr =
    SELet1General(rhs, body)

  def let1b2(op: SBuiltin, arg1: SExprAtomic, arg2: SExprAtomic, body: SExpr): SExpr =
    SELet1Builtin(op, Array(arg1, arg2), body)

  def appa(func: SExprAtomic, arg: SExprAtomic): SExpr =
    SEAppAtomicGeneral(func, Array(arg))

  def app2a(func: SExprAtomic, arg1: SExprAtomic, arg2: SExprAtomic): SExpr =
    SEAppAtomicGeneral(func, Array(arg1, arg2))

  def binopa(op: SBuiltin, x: SExprAtomic, y: SExprAtomic): SExpr =
    SEAppAtomicSaturatedBuiltin(op, Array(x, y))

  def itea(i: SExprAtomic, t: SExpr, e: SExpr): SExpr =
    SECaseAtomic(i, Array(SCaseAlt(patTrue, t), SCaseAlt(patFalse, e)))

  // true/false case-patterns
  def patTrue: SCasePat =
    SCPVariant(Identifier.assertFromString("P:M:bool"), IdString.Name.assertFromString("True"), 1)

  def patFalse: SCasePat =
  ...
  def num1 = num(1)
  def num2 = num(2)
  def num(n: Long): SExprAtomic = SEValue(SInt64(n))

  // run a test...
  def testTransform(original: SExpr, expected: AExpr, show: Boolean = false): Assertion = {
