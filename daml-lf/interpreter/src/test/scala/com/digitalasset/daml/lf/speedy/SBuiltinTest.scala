...
class SBuiltinTest extends AnyFreeSpec with Matchers with TableDrivenPropertyChecks with Inside {
  ...
  "GenMap operations" - {
    ...
    "GENMAP_INSERT" - {
      ...
      "crash when comparing non comparable keys" in {
        ...
        inside(eval(expr1)) { case Left(SErrorCrash(_, msg)) =>
          msg shouldBe "functions are not comparable"
        }
        inside(eval(expr2)) { case Left(SErrorCrash(_, msg)) =>
          msg shouldBe "functions are not comparable"
        }
      }
    }
  ...
  "AnyExceptionMessage" - {
    "request unknown packageId" in {
      ...
      ) { case Left(SErrorCrash(_, mgs)) => msg => s"need package '-unknown-package-'" }
    }
