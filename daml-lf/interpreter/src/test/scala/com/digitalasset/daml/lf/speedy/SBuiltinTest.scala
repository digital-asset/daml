...
object SBuiltinTest {
  ...
  def buildDisclosedContract(
      ...
      if (withKey) {
        Some(
          crypto.Hash.assertHashContractKey(
            templateId,
            key.toNormalizedValue(TransactionVersion.minExplicitDisclosure),
          )
        )
      } else {
        None
    ...
    (disclosedContract, if (withKey) SOptional(Some(key)) else SOptional(None))
  }
}
