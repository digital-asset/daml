...
object HistogramData {
  final val defaultBucketBoundaries = List(5L, 10L, 25L, 50L, 75L, 100L, 250L, 500L, 750L, 1000L, 2500L, 5000L, 7500L, 10000L)
  ...
  @tailrec private def computeData(values: List[Long], acc: HistogramData): HistogramData = {
    values match {
      case head :: tail =>
        computeData(tail, HistogramData(acc.count + 1, acc.sum + head, updateBucketCounts(acc.bucketCounts, defaultBucketBoundaries, head)))
      case Nil =>
        acc
    }
  }

  private def updateBucketCounts(counts: List[Long], boundaries: List[Long], value: Long): List[Long] = {
    boundaries match {
      case head :: tail =>
        if (value <= head) {
...
abstract class TestMetricsBase {
    ...
    val sdkMeterProvider: SdkMeterProvider = SdkMeterProvider.builder().setClock(testClock).registerMetricReader(metricReader).build();
    ...
    def getHistogramValues(metric: MetricHandle): HistogramData = {
      ...
      HistogramData(point.getCount, point.getSum.toLong, point. getCounts.asScala.map(_.longValue).toList)
    }
