...
import DamlHub, { damlHubLogout, isRunningOnHub, usePublicParty, usePublicToken } from '@daml/hub-react';
  ...
  if (credentials) {
    const PublicPartyLedger: React.FC = ({ children }) => {
      ...
      if (publicToken && publicParty) {
        return <publicContext.DamlLedger token={publicToken.token} party={publicParty}>
          {children}
    ...
    return <Wrap>
      ...
      >
        <MainScreen getPublicParty={credentials.getPublicParty} onLogout={() => {
