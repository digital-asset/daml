      ...
      activeContractsBefore <- activeContractsOf(
        ledgerDao.transactionsReader
          .getActiveContracts(
            ...
            filter = Map.empty,
            wildcardParties = Set(alice, bob, charlie),
      ...
      result <- activeContractsOf(
        ledgerDao.transactionsReader
          .getActiveContracts(
            ...
            filter = Map(otherTemplateId -> Set(party1)),
      ...
      resultUnknownParty <- activeContractsOf(
        ledgerDao.transactionsReader
          .getActiveContracts(
            ...
            wildcardParties = Set(party2, unknownParty),
            eventProjectionProperties = EventProjectionProperties(
              verbose = true,
              witnessTemplateIdFilter = Map(
                party1 -> Set(someTemplateId),
