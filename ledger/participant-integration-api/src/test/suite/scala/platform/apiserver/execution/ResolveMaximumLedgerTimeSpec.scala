...
class ResolveMaximumLedgerTimeSpec
  ...
  it should "resolve maximum ledger time only using disclosed contracts" in new TestScope {
    private val disclosedContracts = ImmArray(
      buildDisclosedContract(cId_1, t1),
      buildDisclosedContract(cId_2, t2),
    )
  ...
  it should "resolve maximum ledger time only with no provided disclosed contracts" in new TestScope {
    private val actualResult: MaximumLedgerTime =
