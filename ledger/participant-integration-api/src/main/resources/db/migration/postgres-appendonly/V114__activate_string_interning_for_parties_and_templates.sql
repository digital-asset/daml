...
CREATE FUNCTION array_size_select_clause(
    column_name TEXT
)
RETURNS TEXT
LANGUAGE plpgsql
...
BEGIN
   return if_null_then_else(
       column_name,
       '1',
       'CASE WHEN ' || column_name || ' IS NULL THEN NULL ELSE cardinality(' || column_name || ') END'
   );
END
$$;
...
BEGIN
   ...
   '      ' || array_size_select_clause(party_array_column_a) || ' AS pa,' ||chr(10)||
   '      ' || array_size_select_clause(party_array_column_b) || ' AS pb,' ||chr(10)||
   '      ' || array_size_select_clause(party_array_column_c) || ' AS pc,' ||chr(10)||
   '      ' || array_size_select_clause(party_array_column_d) || ' AS pd,' ||chr(10)||
   '      ' || array_size_select_clause(party_array_column_e) || ' AS pe,' ||chr(10)||
   '      ' || join_column ||chr(10)||
...
DROP FUNCTION array_size_select_clause(TEXT);
