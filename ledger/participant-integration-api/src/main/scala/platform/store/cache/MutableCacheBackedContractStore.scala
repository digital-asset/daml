...
class MutableCacheBackedContractStore(
    ...
    subscribeToContractStateEvents: (Offset, Long) => Source[ContractStateEvent, NotUsed],
  ...
  private[cache] val cacheOffsetEventSequentialId = new AtomicLong(EventSequentialId.beforeBegin)
  private[cache] val trackedEventOffset = new AtomicReference(Offset.beforeBegin)
  ...
  private val (contractStateUpdateKillSwitch, contractStateUpdateDone) =
    RestartSource
      .withBackoff(
        ...
        subscribeToContractStateEvents(trackedEventOffset.get(), cacheOffsetEventSequentialId.get())
  ...
  contractStateUpdateDone
    .onComplete {
      case Failure(exception) =>
        logger.error("Contract state event consumption stream failed", exception)
        // Take down the entire show if contract state updates stream fails
        close()
      case Success(_) =>
  ...
  private def readThroughContractsCache(contractId: ContractId)(implicit
    ...
    val currentCacheOffset = cacheOffsetEventSequentialId.get()
  ...
  private def readThroughKeyCache(
    ...
    val currentOffset = cacheOffsetEventSequentialId.get()
  ...
  private def updateOffsets(event: ContractStateEvent): Unit = {
    trackedEventOffset.set(event.eventOffset)
    cacheOffsetEventSequentialId.set(event.eventSequentialId)
