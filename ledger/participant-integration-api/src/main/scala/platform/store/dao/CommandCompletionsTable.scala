...
private[platform] object CommandCompletionsTable {
  ...
  def prepareCompletionInsert(
    ...
    SQL"insert into participant_command_completions(completion_offset, record_time, application_id, submitting_party, command_id, transaction_id) values ($offset, $recordTime, ${submitterInfo.applicationId}, ${submitterInfo.singleSubmitterOrThrow()}, ${submitterInfo.commandId}, $transactionId)"
  ...
  def prepareRejectionInsert(
    ...
    SQL"insert into participant_command_completions(completion_offset, record_time, application_id, submitting_party, command_id, status_code, status_message) values ($offset, $recordTime, ${submitterInfo.applicationId}, ${submitterInfo.singleSubmitterOrThrow()}, ${submitterInfo.commandId}, $code, $message)"
  }
