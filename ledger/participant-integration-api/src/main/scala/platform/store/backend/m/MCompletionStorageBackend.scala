...
object MCompletionStorageBackend extends CompletionStorageBackend {
  override def commandCompletions(
      ...
      parties: Set[Party],
  )(connection: Connection): List[CompletionStreamResponse] = MStore(connection) { mStore =>
    import com.daml.platform.store.backend.m.SearchUtil.rangeIterator
    val completions = mStore.commandCompletions
  ...
  override def pruneCompletions(
    ...
    mStore =>
      ...
      mStore.copy(
        commandCompletions = mStore.commandCompletions
