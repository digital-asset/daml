...
object MDataSourceStorageBackend extends DataSourceStorageBackend {
  override def createDataSource(
    ...
    val id =
      if (jdbcUrl.startsWith(mprefix)) jdbcUrl.substring(mprefix.length)
      else if (jdbcUrl.startsWith(h2prefix)) jdbcUrl.substring(h2prefix.length)
      else throw new UnsupportedOperationException("only h2 and m jdbc urls supported")
    MStore.create(id)
    new MDataSource(id)
  }

  override def checkDatabaseAvailable(connection: Connection): Unit = MStore(connection)(_ => ())

  def purge(id: String): Unit = MStore.remove(id)
}
