...
class MPartyStorageBackend(ledgerEndCache: LedgerEndCache) extends PartyStorageBackend {
  override def partyEntries(
      ...
      pageSize: Int,
      queryOffset: Long,
  )(connection: Connection): Vector[(Offset, PartyLedgerEntry)] = MStore(connection) { mStore =>
    val startOffset = startExclusive.toHexString
    val endOffset = endInclusive.toHexString
    mStore.partyEntries.iterator
  ...
  ): Vector[domain.PartyDetails] = MStore(connection) { mStore =>
    val ledgerEndOffset = ledgerEndCache()._1.toHexString
    (parties match {
      case Some(parties) =>
        parties.iterator
          .flatMap(p => mStore.partyIndex.get(p).iterator)
      case None =>
        mStore.partyIndex.iterator
          .map(_._2)
    })
      .flatMap { parties =>
