...
object InitHookDataSourceProxy {
  val logger: ContextualizedLogger = ContextualizedLogger.get(this.getClass)
  implicit val dummyLoggingContext: LoggingContext = LoggingContext.ForTesting // TODO FIXME
}

import InitHookDataSourceProxy._

case class InitHookDataSourceProxy(
    delegate: DataSource,
  ...
  override def getConnection: Connection = {
    ...
    logger.debug(
      s"$connectionId Creating new connection"
    ) // TODO fix logging, move to debug/remove this granular logging
...
class LoggingConnectionProxy(delegate: Connection, connectionId: String) extends Connection {
