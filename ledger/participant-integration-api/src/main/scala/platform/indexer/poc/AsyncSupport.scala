...
object AsyncSupport {
    ...
    ResourceOwner.forCloseable {
      () =>
        ...
        new Executor with AutoCloseable {
          override def execute[FIN, FOUT](f: FIN => FOUT): FIN => Future[FOUT] = in => Future(f(in))(workerEC)
  ...
  def asyncResourcePool[RESOURCE <: AutoCloseable](createResource: () => RESOURCE,
                                                   size: Int): ResourceOwner[PooledResourceExecutor[RESOURCE]] =
    for {
      asyncPool <- asyncPool(size)
      resourcePool <- ResourceOwner.forCloseable(() => PostgresDAO.ResourcePool(createResource, size))
