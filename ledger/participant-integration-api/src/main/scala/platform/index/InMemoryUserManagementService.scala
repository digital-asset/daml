...
class InMemoryUserManagementService extends UserManagementService {
  ...
  override def grantRights(id: UserId, granted: Set[UserRight]): Future[Result[Set[UserRight]]] =
    Future.successful {
      lookup(id) match {
        case Some(userInfo) =>
  ...
  override def revokeRights(id: UserId, revoked: Set[UserRight]): Future[Result[Set[UserRight]]] =
    Future.successful {
      lookup(id) match {
        case Some(userInfo) =>
