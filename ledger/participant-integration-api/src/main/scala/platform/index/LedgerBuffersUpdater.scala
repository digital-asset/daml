...
class LedgerBuffersUpdater(
    ...
    Flow[(Vector[(Offset, Update)], Long)]
      .mapAsync(prepareUpdatesParallelism) { case (batch, lastEventSequentialId) =>
        Future {
          ...
          } -> (batch.last._1 -> lastEventSequentialId)
        }(prepareUpdatesExecutionContext)
      }
      .async
      .mapAsync(1) { case (updates, ledgerEnd) =>
        Timed.future(
          metrics.daml.index.updateLedgerApiTransientState,
          Future {
  ...
  private def updateCaches(updates: Vector[TransactionLogUpdate]): Unit =
  ...
  private def updateLedgerEnd(lastOffset: Offset, lastEventSequentialId: Long)(implicit
      loggingContext: LoggingContext
  ): Unit = {
