...
final class ErrorInterceptor extends ServerInterceptor {
  ...
  override def interceptCall[ReqT, RespT](call: ServerCall[ReqT, RespT],
    ...
    val forwardingCall = new SimpleForwardingServerCall[ReqT, RespT](call) {
       ...
       * TODO error codes: Consider also checking whether metadata is security sanitized.
       * TODO error codes: Consider intercepting the exceptions before they are translated in a Status/Metadata pair.
       * TODO              Would be more robust and would simplify this code.
       * TODO              Seems somewhat difficult to do as we want to affect the code generated by scalapb.
       */
      override def close(status: Status, trailers: Metadata): Unit = {
        if (
          status.getCode == Status.Code.INTERNAL &&
            (status.getDescription == null || !BaseError.isSanitizedSecuritySensitiveMessage(status.getDescription))
        ) {
...
class ErrorListener[ReqT, RespT](delegate: ServerCall.Listener[ReqT],
  ...
  override def onHalfClose(): Unit = {
    try {
      ...
      case t: StatusRuntimeException =>
        call.close(t.getStatus, t.getTrailers)
      case t: Throwable =>
        val e = LedgerApiErrors.InternalError.UnexpectedOrUnknownException(t = t)(
          new DamlContextualizedErrorLogger(logger, emptyLoggingContext, None)
        ).asGrpcError
  ...
  // TODO error codes: Do onCancel, onComplete, onReady or onMessage also need special error handling?
