...
object JcsSigner {
  ...
  def sign(report: ParticipantReport, scheme: Scheme, key: Key): Either[Error, ParticipantReport] = {
    generateCheck(report: ParticipantReport, scheme: Scheme, key: Key).map(check =>
      report.copy(check = Some(check))
    )
  }

  private def generateCheck(report: ParticipantReport, scheme: Scheme, key: Key): Either[Error, Check] = {
    for {
