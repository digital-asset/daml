...
/** Subscribes to ledger configuration updates coming from the index,
  ...
  */
private[apiserver] final class CurrentLedgerConfiguration private (
...
private[apiserver] object CurrentLedgerConfiguration {
  def owner(
      index: IndexConfigManagementService,
      config: LedgerConfiguration,
  )(implicit
    ...
    ResourceOwner.forCloseable(() => new CurrentLedgerConfiguration(index, config, materializer))
}
