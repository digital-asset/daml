        ...
        for {
          ...
          User(
            ...
            metadata = ObjectMeta(
              ...
              resourceVersionO = pObjectMetaResourceVersion.map(_.toLong),
              annotations = pAnnotations,
            ),
          ),
          pFieldMask,
        ...
        for {
          ...
          _ <-
            if (userUpdate.isNoUpdate) {
              ...
              Future.failed(
                LedgerApiErrors.Admin.UserManagement.InvalidUserUpdate
                  .Reject(
                    userId = user.id,
  ...
  private def handleResult[T](operation: String)(result: UserManagementStore.Result[T]): Future[T] =
    result match {
      ...
      case Left(e: UserManagementStore.MaxAnnotationsSizeExceeded) =>
        ...
        Future.failed(
          LedgerApiErrors.Admin.UserManagement.MaxAnnotationsSizeExceeded
            .Reject(userId = e.userId)
            .asGrpcError
        )
...
object ApiUserManagementService {
  ...
  def toProtoObjectMeta(meta: ObjectMeta): proto_admin.object_meta.ObjectMeta =
    proto_admin.object_meta.ObjectMeta(
      // TODO pbatko: Resource version serialization
