...
private class NoOpLedger extends Ledger {
  override def publishTransaction(
    ...
    Future.successful(SubmissionResult.NotSupported)
  ...
  ): Source[(Offset, GetTransactionTreesResponse), NotUsed] = Source.empty
  ...
  override def activeContracts(filter: Map[Party, Set[Ref.Identifier]], verbose: Boolean)(implicit
      loggingContext: LoggingContext
  ): (Source[GetActiveContractsResponse, NotUsed], Offset) = (Source.empty, Offset.beforeBegin)

  override def lookupContract(contractId: Value.ContractId, forParties: Set[Party])(implicit
      loggingContext: LoggingContext
  ): Future[Option[Value.ContractInst[Value.VersionedValue[Value.ContractId]]]] =
    Future.successful(None)

  override def lookupMaximumLedgerTime(contractIds: Set[Value.ContractId])(implicit
      loggingContext: LoggingContext
  ): Future[Option[Instant]] = Future.successful(None)

  override def lookupKey(key: GlobalKey, forParties: Set[Party])(implicit
      loggingContext: LoggingContext
  ): Future[Option[Value.ContractId]] = Future.successful(None)
  ...
  override def getParties(parties: Seq[Party])(implicit
      loggingContext: LoggingContext
  ): Future[List[domain.PartyDetails]] = Future.successful(List.empty)
  ...
  override def partyEntries(startExclusive: Offset)(implicit
      loggingContext: LoggingContext
  ): Source[(Offset, PartyLedgerEntry), NotUsed] = Source.empty
  ...
  override def getLfArchive(packageId: PackageId)(implicit
      loggingContext: LoggingContext
  ): Future[Option[DamlLf.Archive]] = Future.successful(None)

  override def getLfPackage(packageId: PackageId)(implicit
      loggingContext: LoggingContext
  ): Future[Option[Ast.Package]] = Future.successful(None)

  override def packageEntries(startExclusive: Offset)(implicit
      loggingContext: LoggingContext
  ): Source[(Offset, PackageLedgerEntry), NotUsed] = Source.empty

  override def lookupLedgerConfiguration()(implicit
      loggingContext: LoggingContext
  ): Future[Option[(Offset, Configuration)]] = Future.successful(None)

  override def configurationEntries(startExclusive: Offset)(implicit
      loggingContext: LoggingContext
  ): Source[(Offset, ConfigurationEntry), NotUsed] = Source.empty
  ...
  override def deduplicateCommand(
    ...
    Future.successful(CommandDeduplicationDuplicate(Instant.EPOCH))
