...
class CommandStaticTimeIT
  ...
  private def timeSource(ctx: LedgerContext) =
    ClientAdapter.serverStreaming(GetTimeRequest(getLedgerId(ctx)), ctx.timeService.getTime)
        ...
        for {
          ...
          completion <- commandClient
            ...
            .trackSingleCommand(SubmitRequest(
              Some(submitRequest.getCommands.withLedgerId(getLedgerId(ctx)).withCommandId(newCommandId()))))
        } yield {
      ...
      "time is changed, and ledger effective time is within the new acceptance window" should {
        ...
        "commands should be accepted" in allFixtures { ctx =>
          for {
            ...
            _ <- ctx.timeService.setTime(SetTimeRequest(getLedgerId(ctx), Some(oldTime), Some(fromInstant(newTime))))
