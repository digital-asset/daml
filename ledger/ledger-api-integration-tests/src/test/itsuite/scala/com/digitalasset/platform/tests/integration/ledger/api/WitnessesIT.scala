...
import com.digitalasset.ledger.api.testing.utils.{SuiteResourceManagementAroundEach, AkkaBeforeAndAfterAll}
...
class WitnessIT
  ...
  private val filter = TransactionFilter(Map(
  ...
  "disclosure rules are respected" in forAllMatchingFixtures {
    ...
    def exercise(cid: String, choice: String): Future[ExercisedEvent] =
      ctx.testingHelpers.submitAndListenForSingleTreeResultOfCommand(
        ctx.testingHelpers.submitRequestWithId(s"$choice-exercise")
          .update(
            _.commands.commands :=
    ...
    for {
      ...
      createTx <- ctx.testingHelpers.submitAndListenForSingleResultOfCommand(
        ctx.testingHelpers.submitRequestWithId("create")
          .update(
            _.commands.commands :=
      ...
      divulgeCreatedEv <- ctx.testingHelpers.simpleCreate(
        ...
        Record(fields = List(RecordField(value = "alice".asParty), RecordField(value = "charlie".asParty)))
      )
      _ <- ctx.testingHelpers.simpleExercise(
        ...
        "Divulge",
        Value(Value.Sum.Record(Record(fields = List(RecordField(value = createdEv.contractId.asContractId)))))
      )
