...
object ValueValidator {
  ...
  def validateValue(value: api.Value): Either[StatusRuntimeException, domain.Value] =
    value.sum match {
      ...
      case Sum.List(api.List(elems)) =>
        elems
          .foldLeft[Either[StatusRuntimeException, BackStack[domain.Value]]](
            Right(BackStack.empty))((valuesE, v) =>
            for {
              values <- valuesE
              validatedValue <- validateValue(v)
