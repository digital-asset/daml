...
object EventFilter {
  ...
  final case class TemplateAwareFilter(transactionFilter: TransactionFilter) {
    ...
    def filter[E <: AcsUpdateEvent](
        event: E,
        withUpdatedStakeholders: Set[v1.Party] => E): Option[E] = {
      // The events are generated by the engine, then
      //  - we can assert identifier are always in `New Style`
      //  - witnesses are party
      val requestingStakeholders = event.stakeholders.filter(p =>
        subscribersByTemplateId(event.templateId).contains(Party.assertFromString(p)))
      if (requestingStakeholders.nonEmpty)
        Some(withUpdatedStakeholders(requestingStakeholders))
      else
        None
    }
