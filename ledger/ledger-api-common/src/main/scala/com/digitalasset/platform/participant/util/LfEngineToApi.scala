  ...
  def lfValueToApiValue(
    ...
    value0 match {
      ...
      case Lf.ValueDecimal(d) =>
        Right(ApiValue(ApiValue.Sum.Decimal(Decimal.toUtf8String(d).javaString)))
      ...
      case Lf.ValueText(t) => Right(ApiValue(ApiValue.Sum.Text(t.javaString)))
      ...
      case Lf.ValueMap(m) =>
        m.toImmArray.reverse
          .foldLeft[Either[String, List[ApiMap.Entry]]](Right(List.empty[ApiMap.Entry])) {
            case (Right(list), (k, v)) =>
              lfValueToApiValue(verbose, v).map(w => ApiMap.Entry(k.javaString, Some(w)) :: list)
            case (left, _) => left
          }
