      ...
      _ <- withFreshUser(primaryParty = "primaryParty0") { user =>
        ledger.userManagement
          ...
          .mustFailWith(
            ...
            errorCode = LedgerApiErrors.Admin.UserManagement.InvalidUpdateUserRequest,
            exceptionMessageSubstring = Some(
              s"INVALID_ARGUMENT: INVALID_USER_UPDATE_REQUEST(8,0): Update operation for user id '${user.id}' failed due to: MergeUpdateModifierOnPrimitiveFieldWithDefaultValue"
            ),
          )
      }
