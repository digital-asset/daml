...
class TimeoutParticipantTestContext(timeoutScaleFactor: Double, delegate: ParticipantTestContext)
  ...
  def allocateParty(req: AllocatePartyRequest): Future[AllocatePartyResponse] =
    withTimeout("Allocate party full", delegate.allocateParty(req))
  ...
  override def listKnownParties_full(): Future[ListKnownPartiesResponse] = withTimeout(
    "List known parties",
    delegate.listKnownParties_full(),
  )
