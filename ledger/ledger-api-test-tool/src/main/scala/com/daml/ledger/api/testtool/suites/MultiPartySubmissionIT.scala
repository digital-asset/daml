  ...
  test(
    ...
    for {
      ...
      assertGrpcErrorRegex(
        ...
        Some(
          Pattern.compile(
            "Interpretation error: Error: (User abort: Assertion failed.|Unhandled (Daml )?exception: [0-9a-zA-Z\\.:]*@[0-9a-f]*\\{ message = \"LookupOtherByKey value matches\" \\}\\.) [Dd]etails(: |=)Last location: \\[[^\\]]*\\], partial transaction: root node"
          )
        ),
        checkDefiniteAnswerMetadata = true,
      )
