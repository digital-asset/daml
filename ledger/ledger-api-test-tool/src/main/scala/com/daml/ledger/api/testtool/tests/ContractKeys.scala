      ...
      for {
        ...
        bobLooksUpTextKeyFailure <- ledger
          .exercise(
            bob,
            bobTKO.contractId
              .exerciseTKOLookup(_, Tuple2(alice, key1), Some(tk1.contractId)))
          .failed

        // trying to lookup an unauthorized non-existing key should fail
        bobLooksUpBogusTextKeyFailure <- ledger
          .exercise(bob, bobTKO.contractId.exerciseTKOLookup(_, Tuple2(alice, unknownKey), None))
          .failed
        ...
        _ <- ledger.exercise(
          alice,
          aliceTKO.contractId.exerciseTKOFetch(_, Tuple2(alice, key1), tk1.contractId))

        // successful, authorized lookup of non-existing key
        _ <- ledger.exercise(
          alice,
          aliceTKO.contractId.exerciseTKOLookup(_, Tuple2(alice, unknownKey), None))
        ...
        aliceFailedFetch <- ledger
          .exercise(
            alice,
            aliceTKO.contractId
              .exerciseTKOFetch(_, Tuple2(alice, unknownKey), tk1.contractId))
          .failed
        ...
        _ <- ledger.exercise(
          alice,
          aliceTKO.contractId.exerciseTKOLookup(_, Tuple2(alice, key1), None))

        // lookup the key, consume it, then verify we cannot look it up anymore
        _ <- ledger.exercise(
          alice,
          aliceTKO.contractId
            .exerciseTKOConsumeAndLookup(_, tk2.contractId, Tuple2(alice, key2)))
