...
class TransactionServiceIT extends LedgerTestSuite {
  ...
  // visibility tests
  {
    ...
    test(
      ...
      for {
        ...
        tree <- eventually {
          ledger.exercise(bob, trade.exerciseIouTrade_Accept(_, bobIou))
        }
    ...
    def onlyRequestingPartiesAsWitnesses(
    ...
    def allTxWitnesses(transactions: Vector[Transaction]): Set[String] =
      transactions
    ...
    def allTxTreesWitnesses(transactionTrees: Vector[TransactionTree]): Set[String] =
      transactionTrees
  ...
  // correctness tests
  {
    ...
    def comparableTransactions(transactions: Vector[Transaction]): Vector[Transaction] =
      transactions
        .map(t =>
          t.copy(
            commandId = "commandId",
    ...
    // Furthermore, makes sure that the order is not relevant for witness parties
    // Sort by transactionId as on distributed ledgers updates can occur in different orders
    def comparableTransactionTrees(
