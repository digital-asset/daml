...
class SqlLedgerSpec
  ...
  "SQL Ledger" should {
    ...
    "be able to be created from scratch with a given ledger id" in {
      val ledgerId = Ref.LedgerIdString.assertFromString("TheLedger")
    ...
    "refuse to create a new ledger when there is already one with a different ledger id" in {
      ...
      val ledgerF = for {
        _ <- SqlLedger(
          jdbcUrl = postgresFixture.jdbcUrl,
          ledgerId = Some(Ref.LedgerIdString.assertFromString("TheLedger")),
          timeProvider = TimeProvider.UTC,
          acs = ActiveContractsInMemory.empty,
          initialLedgerEntries = ImmArray.empty,
          queueDepth
        ...
        _ <- SqlLedger(
          jdbcUrl = postgresFixture.jdbcUrl,
          ledgerId = Some(Ref.LedgerIdString.assertFromString("AnotherLedger")),
          timeProvider = TimeProvider.UTC,
          acs = ActiveContractsInMemory.empty,
          initialLedgerEntries = ImmArray.empty,
          queueDepth
