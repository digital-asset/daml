...
import com.digitalasset.platform.index.store.entries.{ConfigurationEntry, LedgerEntry}
import com.digitalasset.platform.index.store.{DbType, FlywayMigrations, PersistenceEntry}
    ...
    "upload packages in an idempotent fashion, maintaining existing descriptions" in {
      ...
      for {
        ...
        loadedPackages.values.flatMap(_.sourceDescription.toList) should contain theSameElementsAs
          Seq(firstDescription) ++ Seq.fill(15)(secondDescription)
      }
    }
