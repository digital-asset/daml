...
/**
  * A post-execution finalizer to be invoked as the last stage of a pre-execution pipeline that
  ...
  *
  * It access the ledger state through [[LedgerStateAccess]] that doesn't provide fingerprints along values
...
class PostExecutionFinalizerWithFingerprintsFromValues[LogResult](
  ...
  /**
    ...
    * @param ledgerStateAccess Transactional access to the operations that can access actual ledger storage as part
    *                          of a transaction.
