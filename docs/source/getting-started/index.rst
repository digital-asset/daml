...
- `Node <https://nodejs.org/en/>`_ and the associated package manager ``npm``. Use the `Active LTS<https://nodejs.org/en/about/releases/>`_ version, which is ``v16``. Use ``node --version`` to check; if you have another version, see `this link <https://docs.npmjs.com/downloading-and-installing-node-js-and-npm>`_ for additional installation options.
