...
The social network users discussed in this guide are really a combination of all three of these concepts. Alice, Bob, and Charlie are all aliases that correspond to a single test user and a single party id each. As part of the installation process the ``Setup.daml`` file creates the initial three test users and a public party. This public party is for user management. Here, for instance, it supports creating additional users.
