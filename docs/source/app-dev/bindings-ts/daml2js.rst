...
The command ``daml codegen js`` generates JavaScript (and TypeScript) that can be used in conjunction with the `JavaScript Client Libraries <https://docs.daml.com/app-dev/bindings-ts/index.html>`_ for interacting with a Daml ledger via the `HTTP JSON API <https://docs.daml.com/json-api/index.html>`_.
