...
- The big title reading ``Main:Token`` is the identifier of the type of contract. ``Main`` is the module name, ``Token`` is the template name.
...
- The next set of columns show the contract arguments, with one column per field. As expected, field ``owner`` is ``'Alice'``. The single quotation marks indicate that ``Alice`` is a party.
